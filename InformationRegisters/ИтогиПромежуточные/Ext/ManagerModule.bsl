...
Процедура Обновить(Тур, Замещать=Ложь) Экспорт
	Если ТипЗнч(Тур) = Тип("СправочникСсылка.Чемпионаты") Тогда
		Туры	= СерверныйФункции.ТурыЧемпионата(Тур, Истина);
		Обновить(Туры, Замещать);
	ИначеЕсли ТипЗнч(Тур) = Тип("Массив") Тогда
		Для Каждого Элемент Из Тур Цикл
