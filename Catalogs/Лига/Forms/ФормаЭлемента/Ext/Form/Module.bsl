...
Процедура КомандыЛигиЗагрузить()
	Если ЗначениеЗаполнено(Объект.Страна) Тогда
		КомандыЛиги.ЗагрузитьЗначения(СерверныйФункции.КомандыСтраны(Объект.Страна));
	Иначе
...
Процедура ВидимостьДоступностьУстановить()
	...
	КонецЕсли;
	Элементы.КоличествоРотация.ТолькоПросмотр	= ?(Объект.КоличествоКоманд > 2, Объект.ОлимпийскаяСистема Или НЕ ЗначениеЗаполнено(Объект.Страна), Истина);
	Элементы.Квалификация.ТолькоПросмотр		= ?(Объект.КоличествоКоманд > 2, Объект.ОлимпийскаяСистема Или НЕ ЗначениеЗаполнено(Объект.Страна), Истина);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Объект.Ссылка.Пустая() Тогда
		Элементы.Наименование.СписокВыбора.ЗагрузитьЗначения(НаименованияПолучитьНаСервере());
	Иначе
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли СтрСравнить(ИмяСобытия, "Команда") = 0 Тогда
		КомандыЛигиЗагрузить();
	КонецЕсли;
КонецПроцедуры
...
Процедура КомандыЗаполнитьНаСервере()
	Если НЕ Объект.Ссылка.Пустая() Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 6
		                      ...
		                      |ГДЕ
		                      |	ГОД(ТурнирнаяТаблица.Период) >= &Год
		                      ...
		                      |
		                      |УПОРЯДОЧИТЬ ПО
		                      |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Период) + СУММА(ТурнирнаяТаблица.КоличествоОчков) УБЫВ");
		Запрос.УстановитьПараметр("Лига",			Объект.Ссылка);
		Запрос.УстановитьПараметр("Год",			Год(ТекущаяДатаСеанса()) - 11);
...
&НаСервереБезКонтекста
Функция НаименованияПолучитьНаСервере()
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	Лига.Наименование КАК Наименование
	                      |ИЗ
	                      |	Справочник.Лига КАК Лига
	                      |ГДЕ
	                      |	Лига.ПометкаУдаления = ЛОЖЬ
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	Лига.Наименование
	                      |
...
Процедура НаименованиеПриИзменении(Элемент)
	Если Объект.Ссылка.Пустая() И СтрНайти2(Объект.Наименование, "кубок") > 0 Тогда
