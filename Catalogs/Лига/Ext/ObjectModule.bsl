...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Владелец) Тогда
		Владелец = НаСервереПовторно.Футбол();
	КонецЕсли;
КонецПроцедуры

Процедура ПередЗаписью(Отказ)
	Если Ссылка.Пустая() Тогда
		Если ЗначениеЗаполнено(Страна) И НЕ ОлимпийскаяСистема Тогда
			ЧемпионатНацио = СерверныйФункции.ЛигаСтраны(Страна);
			Если ЗначениеЗаполнено(ЧемпионатНацио) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрШаблон("Уже есть национальный чемпионат (%1)", ЧемпионатНацио));
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ОлимпийскаяСистема Или НЕ ЗначениеЗаполнено(Страна) Тогда
		ОбщегоНазначения.УстановитьЗначение(КоличествоРотация, 0);
	КонецЕсли;
	Если Отказ Тогда
		//
	ИначеЕсли ЗначениеЗаполнено(Страна) Тогда
		КомандыСвернуть();
	ИначеЕсли Команды.Количество() > 0 Тогда
		Команды.Очистить();
	КонецЕсли;
КонецПроцедуры
