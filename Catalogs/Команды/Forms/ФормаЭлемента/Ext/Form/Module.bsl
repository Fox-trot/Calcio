...
Процедура ФотоПолучить() Экспорт
	...
	ИначеЕсли ТекущиеДанные.Свойство("Игрок") И ТекущийИгрок = ТекущиеДанные.Игрок Тогда
		Если Кеш.НайтиПоЗначению(ТекущийИгрок) = Неопределено Тогда
			ФотоПолучитьНаСервере();
		КонецЕсли;
	ИначеЕсли ТекущиеДанные.Свойство("Игрок") Тогда
...
Процедура ФотоПолучитьНаСервере()
	...
	Если Элемент = Неопределено Тогда
		ИЗО	= ИзображениеПолучить(ТекущийИгрок);
...
Процедура МатчиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Элемент.ТекущиеДанные.Свойство("Матч") Тогда
		Показать(Элемент.ТекущиеДанные.Матч, СтандартнаяОбработка);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Объект.Ссылка.Пустая() Тогда
		...
		ИначеЕсли Параметры.РежимВыбора
		И ЗначениеЗаполнено(Параметры.ПараметрыВыбора)
		Тогда
			Если Параметры.ПараметрыВыбора.Свойство("Ссылка") Тогда
				Страны.ЗагрузитьЗначения(СтраныПолучить(Параметры.ПараметрыВыбора.Ссылка));
			КонецЕсли;
		Иначе
...
&НаСервере
Процедура СтраныЗагрузитьНаСервере()
	Если ЗначениеЗаполнено(Объект.Город) Тогда
		Страны.ЗагрузитьЗначения(СтраныПолучить(Объект.Город));
	ИначеЕсли Страны.Количество() = 0 Тогда
		Объект.Город = ГородНайти(Объект.Наименование, Страны);
		Если ЗначениеЗаполнено(Объект.Город) Тогда
...
&НаКлиенте
Процедура СтраныЗагрузитьАсинх() Экспорт
	СтраныЗагрузитьНаСервере();
КонецПроцедуры
...
Процедура ПриОткрытииНаСервере()
	...
	Если ЗначениеЗаполнено(Чемпионат) И Лиги.НайтиПоЗначению(Чемпионат.Владелец) = Неопределено Тогда
...
Функция СтраныПолучить(Данные=Неопределено)
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      ...
	                      |ГДЕ
	                      |	ЧемпионатыКоманды.Команда В(&Данные)");
...
Процедура ДоступностьВидимостьУстановить(Перечитать=Ложь)
	Если Объект.Ссылка.Пустая() Тогда
		...
		Элементы.СтраницаЧемпионы.Доступность		= Ложь;
	Иначе
		...
		Если Перечитать Тогда
			Прочитать();
		КонецЕсли;
		Элементы.СтраницаЧемпионы.Доступность		= Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли СтрСравнить(ИмяСобытия, "Матч") = 0 Тогда
...
Процедура Исключить(Команда)
	...
	ИначеЕсли Элементы.ТекущийСостав.ТекущиеДанные = Неопределено Тогда
		//
	ИначеЕсли Элементы.ТекущийСостав.ТекущиеДанные.Свойство("Игрок") И ИсключитьНаСервере(Элементы.ТекущийСостав.ТекущиеДанные.Игрок) Тогда
		ТекущийСоставОбновитьНаСервере();
	КонецЕсли;
КонецПроцедуры
...
Функция ПостОбработка(РезультатЗапроса)
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка;
	КонецЕсли;
	Возврат Новый Структура("Показатель,Регистратор");
КонецФункции
...
Функция СезонНаСервере()
	...
	Если ЗначениеЗаполнено(Чемпионат) Тогда
		ОлимпийскаяСистема = СерверныйФункции.ЛигаОлимпийскаяСистема(Чемпионат.Владелец);
		...
		Для Каждого ТекЭлемент Из Стат Цикл
			...
			ИначеЕсли СтрСравнить(ТекЭлемент.Ключ, "Игр с начала сезона") = 0 Тогда
				СыграноМатчей = ТекЭлемент.Значение;
			КонецЕсли;
		КонецЦикла;
		
		Если ?(ОлимпийскаяСистема, СыграноМатчей > 1, СыграноМатчей > 0)  Тогда
			Запрос	= Новый Запрос("ВЫБРАТЬ
			      	               ...
			      	               |	И (Противники.Команда = &МояКоманда
			      	               |			ИЛИ ТурнирнаяТаблица.Команда = &МояКоманда)
			      	               |;
			      	               |
			      	               |////////////////////////////////////////////////////////////////////////////////
			      	               |ВЫБРАТЬ ПЕРВЫЕ 1
			      	               |	""Самый крупный счет"" КАК Показатель,
			      	               |	ТурнирнаяТаблица.Регистратор КАК Регистратор
			      	               |ИЗ
			      	               ...
			      	               |////////////////////////////////////////////////////////////////////////////////
			      	               |ВЫБРАТЬ ПЕРВЫЕ 1
			      	               |	""Самая крупная победа домашняя"" КАК Показатель,
			      	               |	ТурнирнаяТаблица.Регистратор КАК Регистратор
			      	               |ИЗ
			      	               |	ТурнирнаяТаблица КАК ТурнирнаяТаблица
			      	               |ГДЕ
			      	               |	ТурнирнаяТаблица.МояКоманда = ИСТИНА
			      	               ...
			      	               |////////////////////////////////////////////////////////////////////////////////
			      	               |ВЫБРАТЬ ПЕРВЫЕ 1
			      	               |	""Самая крупная победа гостевая"" КАК Показатель,
			      	               |	ТурнирнаяТаблица.Регистратор КАК Регистратор
			      	               |ИЗ
			      	               |	ТурнирнаяТаблица КАК ТурнирнаяТаблица
			      	               |ГДЕ
			      	               |	ТурнирнаяТаблица.МояКоманда2 = ИСТИНА
			...
			Запрос.УстановитьПараметр("МояКоманда",			Объект.Ссылка);
			Выборка = Запрос.ВыполнитьПакет();
			
			Для Значение = ?(СыграноМатчей > 1, 1, 4) По 4 Цикл
				Матч = ПостОбработка(Выборка.Получить(Значение));
				Если ЗначениеЗаполнено(Матч.Регистратор) Тогда
					ЗаполнитьЗначенияСвойств(Сезон.Добавить(), Новый Структура("Ключ,Значение", Матч.Показатель, Матч.Регистратор));
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура СезонОбновить(Команда=Неопределено)
	...
	Иначе
		Элементы.СтраницаСезон.Заголовок = СтрШаблон("Сезон (%1)", Слово);
		Предыдущие5Обновить();
	КонецЕсли;
КонецПроцедуры
...
Процедура Предыдущие5Обновить()
	ЭлементыУдалить(Элементы.ГруппаПредыдущие5.ПодчиненныеЭлементы);
	Запрос	= Новый Запрос("ВЫБРАТЬ
...
&НаСервере
Процедура ЭлементыУдалить(Викимз)
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если Элементы.СтраницаСезон = ТекущаяСтраница Тогда
		Если Сезон.Количество() = 0 Тогда
			СезонОбновить();
		КонецЕсли;
	ИначеЕсли Элементы.СтраницаБиоритмы = ТекущаяСтраница Тогда
		Если График2.КоличествоСерий = 0
		И ТекущийСоставКоличество > 0
		Тогда
			График2Обновить();
		КонецЕсли;
	ИначеЕсли Элементы.СтраницаЧемпионы = ТекущаяСтраница Тогда
...
Процедура ТекущийСоставВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Элемент.ТекущиеДанные.Свойство("Игрок") Тогда
		Показать(Элемент.ТекущиеДанные.Игрок, СтандартнаяОбработка);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	ОбщегоНазначения.УстановитьЗначение(Изображение, ИзображениеПолучить(Объект.Ссылка));
...
Процедура ТекущийСоставПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТекущийСостав.ТекущиеДанные.Свойство("Игрок") И ИсключитьНаСервере(Элементы.ТекущийСостав.ТекущиеДанные.Игрок) Тогда
		ТекущийСоставОбновитьНаСервере();
	КонецЕсли;
КонецПроцедуры
...
Процедура Уволить(Команда)
	Если Элементы.ТекущийСостав.ТекущиеДанные = Неопределено Тогда
		//
	ИначеЕсли Элементы.ТекущийСостав.ТекущиеДанные.Свойство("Игрок") И УволитьНаСервере(Элементы.ТекущийСостав.ТекущиеДанные.Игрок) Тогда
		ТекущийСоставОбновитьНаСервере();
	КонецЕсли;
КонецПроцедуры
...
Функция УволитьНаСервере(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Игроки") Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |ВЫБРАТЬ ПЕРВЫЕ 1
		                      |	ВЫБОР
		                      |		КОГДА МатчСоставыКоманд.Ссылка.Тур.Владелец.ДатаОкончания ЕСТЬ NULL
		                      |				И ТурнирнаяТаблица.Тур.Владелец.ДатаОкончания ЕСТЬ NULL
		                      |			ТОГДА ПеремещенияИгроков.Период
		                      |		КОГДА МатчСоставыКоманд.Ссылка.Тур.Владелец.ДатаОкончания ЕСТЬ NULL
		                      |			ТОГДА ДОБАВИТЬКДАТЕ(ТурнирнаяТаблица.Тур.Владелец.ДатаОкончания, ДЕНЬ, 1)
		                      |		ИНАЧЕ ДОБАВИТЬКДАТЕ(ТурнирнаяТаблица.Тур.Владелец.ДатаОкончания, ДЕНЬ, 1)
		                      |	КОНЕЦ КАК Период,
		                      |	ПеремещенияИгроков.Команда КАК Команда
		                      |ИЗ
		                      |	ПеремещенияИгроков КАК ПеремещенияИгроков
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
		                      |		ПО ПеремещенияИгроков.Команда = ТурнирнаяТаблица.Команда
		                      |			И ПеремещенияИгроков.Период >= ТурнирнаяТаблица.Период
		...
		Если Выборка.Следующий() Тогда
			...
			ЗаполнитьЗначенияСвойств(Запись, Новый Структура("Период,Игрок", Выборка.Период, Ссылка));
			...
			КонецЕсли;
		КонецЕсли;
		Если Ответ И Ссылка.ПометкаУдаления Тогда
...
Процедура ГородНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	ПодключитьОбработчикОжидания("СтраныЗагрузитьАсинх", 0.1, Истина);
КонецПроцедуры

&НаСервереБезКонтекста
Функция ГородНайти(Наименование, Страны)
	Возврат СерверныйФункции.ГородНайти(Наименование, ?(Страны.Количество() = 0, Неопределено, Страны.Получить(0).Значение));
КонецФункции
...
Функция УстановитьСерию(Граф, Элемент)
	Серия	= Граф.УстановитьСерию(НаСервереПовторно.ФЭИ(Элемент));
...
Процедура ЧемпионатНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	НаКлиенте.ОбработкаКомандыОткрытьФорму("Справочник.Чемпионаты.ФормаВыбора", Новый Структура("РежимВыбора,ТекущаяСтрока,Отбор", Истина, Чемпионат, Новый Структура("Ссылка", Чемпионаты.ВыгрузитьЗначения())), Элемент, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура НаименованиеПриИзмененииНаСервере()
	Если НЕ ПустаяСтрока(Объект.Наименование) И Объект.Ссылка.Пустая() Тогда
		...
		мСлова = Разделить(Объект.Наименование, ?(СтрНайти(Объект.Наименование, ",") = 0, " ", ","));
		Для Каждого Слово Из мСлова Цикл 
			Город = ГородНайти(Слово, Страны);
...
Функция ЧемпионыПолучить()
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               ...
	      	               |ГДЕ
	      	               |	Итоги.Команда = &Команда
	      	               |	И ВЫБОР
	      	               |			КОГДА Чемпионаты.Владелец.ОлимпийскаяСистема = ИСТИНА
	      	               |				ТОГДА Итоги.Место = 1
	      	               |			ИНАЧЕ Итоги.Место <= 1 + Чемпионаты.Владелец.Квалификация
	      	               |		КОНЕЦ
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	ЕстьКартинка УБЫВ,
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока Выбор.Следующий() Цикл
			ЗаполнитьЗначенияСвойств(Итоги.Добавить(), Выбор);
		КонецЦикла;
	КонецЦикла;
	Возврат Ответ;
КонецФункции
...
Процедура ЧемпионыОбновитьНаСервере()
	...
	Список = ЧемпионыПолучить();
	Для Каждого Выборка Из Список Цикл
		ЭлементСписка = Чемпионы.Добавить(Выборка.Ссылка, Выборка.Представление);
		
		Элемент = Элементы.Добавить("Декорация" + Выборка.Код, Тип("ПолеФормы"), Элементы.ГруппаЧемпионы);
		Если Выборка.ЕстьКартинка Тогда
...
&НаКлиенте
Процедура ЧемпионыОбновить(Команда)
	ЧемпионыОбновитьНаСервере();
КонецПроцедуры
...
&НаКлиенте
Процедура СезонВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ТекущиеДанные = Сезон.Получить(ВыбраннаяСтрока);
	Если ТипЗнч(ТекущиеДанные.Значение) = Тип("ДокументСсылка.Матч") Тогда
		Показать(ТекущиеДанные.Значение, СтандартнаяОбработка);
	ИначеЕсли ТекущиеДанные.Значение > "0" И СтрНайти("Проигрышей,Ничьих,Побед", ТекущиеДанные.Ключ) > 0 Тогда
		СтандартнаяОбработка = Ложь;
		Игры = ИгрыПолучить(?(СтрСравнить(ТекущиеДанные.Ключ, "Проигрышей") = 0, "<", ?(СтрСравнить(ТекущиеДанные.Ключ, "Ничьих") = 0, "=", ">")));
		Если Игры.Количество() = 1 Тогда
			ПослеВыбора(Игры.Получить(0));
		ИначеЕсли Игры.Количество() > 1 Тогда
			Игры.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ПослеВыбора", ЭтотОбъект), "Выберите матч");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Функция ИгрыПолучить(Знак)
	Игры = Новый СписокЗначений;
	Запрос	= Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 11
	      	               |	ТурнирнаяТаблица.Регистратор КАК Регистратор,
	      	               |	ТурнирнаяТаблица.Регистратор.Представление КАК Представление
	      	               |ИЗ
	      	               |	РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	      	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК Противники
	      	               |		ПО ТурнирнаяТаблица.Регистратор = Противники.Регистратор
	...
	Запрос.УстановитьПараметр("Команда",			Объект.Ссылка);
	Запрос.УстановитьПараметр("Чемпионат",			Чемпионат);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "<=", Знак);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Игры.Добавить(Выборка.Регистратор, Выборка.Представление);
	КонецЦикла;
	Возврат Игры;
КонецФункции
