...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Объект.Ссылка.Пустая() Тогда
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(Страны) И Страны.НайтиПоЗначению(Объект.Город.Владелец) = Неопределено Тогда
...
Процедура ДоступностьВидимостьУстановить(Перечитать=Ложь)
	Если Объект.Ссылка.Пустая() Тогда
		Элементы.ФормаТрансферы.Видимость			= Ложь;
		Элементы.ГруппаТекущийСостав.Доступность	= Ложь;
		Элементы.СтраницаБиоритмы.Доступность		= Ложь;
	Иначе
		Элементы.ФормаТрансферы.Видимость			= Истина;
		Элементы.ГруппаТекущийСостав.Доступность	= Истина;
		Элементы.СтраницаБиоритмы.Доступность		= Истина;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли СтрСравнить(ИмяСобытия, "ПеремещенияИгроков") = 0 Тогда
		ПодключитьОбработчикОжидания("ТекущийСоставОбновить", 0.1, Истина);
	КонецЕсли;
КонецПроцедуры
...
Функция РазмерНорм(Размер)
	Возврат (Размер >= НаСервереПовторно.РазмерКартинкиМинимальный());
КонецФункции
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	НаКлиенте.Оповещение("Команда", Объект.Ссылка, ЭтаФорма);
КонецПроцедуры
...
Процедура График2ОбновитьНаСервере()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	МАКСИМУМ(ЕСТЬNULL(Перемещения.Амплуа, Игроки.Амплуа)) КАК Амплуа,
	                      |	МАКСИМУМ(НЕ МатчСоставыКоманд.Игрок ЕСТЬ NULL) КАК Команда
	                      |ПОМЕСТИТЬ ВТ
	                      |ИЗ
	                      ...
	                      |			И МатчСоставыКоманд.Ссылка.Дата >= &ДатаН
	                      |			И МатчСоставыКоманд.Ссылка.Проведен = ИСТИНА) КАК МатчСоставыКоманд
	...
	Если ЗначениеЗаполнено(Амплуа) Тогда
		Запрос.Текст ="ВЫБРАТЬ
		              ...
		              |			Справочник.Игроки КАК Игроки
		              |		ГДЕ
		              |			ГОД(Игроки.ДатаРождения) >= 1900) КАК Игроки
		              |		ПО Перемещения.Игрок = Игроки.Ссылка
		              |ГДЕ
		              |	Перемещения.Команда = &Команда
...
&НаСервереБезКонтекста
Функция ФЭИ(Элемент)
	Возврат НаСервереПовторно.ФЭИ(Элемент);
КонецФункции

&НаСервереБезКонтекста
Функция Цвет(Фаза)
	Возврат НаСервереПовторно.Цвет(Фаза);
КонецФункции
...
Процедура НаименованиеПриИзмененииНаСервере()
	Если НЕ ПустаяСтрока(Объект.Наименование) И Объект.Ссылка.Пустая() Тогда
		Объект.Наименование	= СтрУдалить(Объект.Наименование, "¶");
		мСлова = Разделить(СтрЗаменить(СтрЗаменить(Объект.Наименование, ",", " "), "-", " "));
...
&НаКлиенте
Процедура ТекущийСоставОбновитьНаКлиенте(Команда)
	ТекущийСоставОбновить();
КонецПроцедуры

&НаКлиенте
Процедура Трансферы(Команда)
	НаКлиенте.ОбработкаКомандыОткрытьФорму("РегистрСведений.ПеремещенияИгроков.Форма.ФормаПоследнее", Новый Структура("Отбор", Новый Структура("Команда", Объект.Ссылка)), ЭтаФорма);
КонецПроцедуры
