...
Функция ЧемпионыПолучить(Команда)
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 6
	      	               |	Итоги.Чемпионат.Владелец КАК Ссылка,
	      	               |	Итоги.Чемпионат.Владелец.Наименование КАК Наименование,
	      	               |	СУММА(1) КАК Количество,
	      	               |	МАКСИМУМ(РАЗМЕРХРАНИМЫХДАННЫХ(Итоги.Чемпионат.Владелец.Изображение) >= 64) КАК ЕстьКартинка
	      	               |ИЗ
	      	               ...
	      	               |	Итоги.Чемпионат.Владелец.Наименование
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	ЕстьКартинка УБЫВ,
	      	               |	Количество УБЫВ,
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Элемент = Новый Структура("Ссылка,Наименование,Количество,Картинка", Выборка.Ссылка, Выборка.Наименование, Выборка.Количество, "");
		Если Выборка.ЕстьКартинка Тогда
			Рисунок = ИзображениеПолучить(Выборка.Ссылка);
			Элемент.Картинка = Рисунок;
		КонецЕсли;
...
&НаСервере
Процедура ЭлементыСкрыть()
	Для Каждого Элемент Из Элементы.ГруппаЧемпионы.ПодчиненныеЭлементы Цикл
...
Процедура ЧемпионыОбновитьНаСервере()
	...
	Для Каждого Чемпион Из Список Цикл
		ЭлементСписка = Чемпионы.Добавить(Чемпион.Ссылка, Чемпион.Ссылка.УникальныйИдентификатор());
		
		Для Каждого Элемент Из Элементы.ГруппаЧемпионы.ПодчиненныеЭлементы Цикл
			Если НЕ Элемент.Видимость Тогда
				Если ПустаяСтрока(Чемпион.Картинка) Тогда
					Элемент.Вид			= ВидПоляФормы.ПолеНадписи;
					Элемент.Заголовок	= СтрШаблон("(%1)", Чемпион.Количество);
					ЗаполнитьЗначенияСвойств(ЭтаФорма, Новый Структура(Элемент.ПутьКДанным, Чемпион.Ссылка));
				Иначе
					Элемент.Вид			= ВидПоляФормы.ПолеКартинки;
					Элемент.Заголовок	= СтрШаблон("%1 (%2)", Чемпион.Наименование, Чемпион.Количество);
					ЗаполнитьЗначенияСвойств(ЭтаФорма, Новый Структура(Элемент.ПутьКДанным, Чемпион.Картинка));
				КонецЕсли;
				Элемент.Подсказка	= ЭлементСписка.Представление;
				Элемент.РасширеннаяПодсказка.Заголовок = ЭлементСписка.Значение;
