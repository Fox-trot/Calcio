...
Процедура СезонВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ТекущиеДанные.Значение > "0" И СтрНайти("Проигрышей,Ничьих,Побед", ТекущиеДанные.Ключ) > 0 Тогда
		СтандартнаяОбработка = Ложь;
		_Матчи = МатчиПолучить(?(СтрСравнить(ТекущиеДанные.Ключ, "Проигрышей") = 0, "<", ?(СтрСравнить(ТекущиеДанные.Ключ, "Ничьих") = 0, "=", ">")));
		Если _Матчи.Количество() = 1 Тогда
			ПослеВыбора(_Матчи.Получить(0));
		ИначеЕсли _Матчи.Количество() > 1 Тогда
			_Матчи.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ПослеВыбора", ЭтотОбъект), "Выберите матч");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Функция МатчиПолучить(Знак)
	_Матчи = Новый СписокЗначений;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		_Матчи.Добавить(Выборка.Регистратор, Выборка.Представление);
	КонецЦикла;
	Возврат _Матчи;
КонецФункции
