...
&НаСервереБезКонтекста
Функция ЧемпионыПолучить(Команда)
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 11
	      	               ...
	      	               |	МАКСИМУМ(РАЗМЕРХРАНИМЫХДАННЫХ(Чемпионаты.Владелец.Изображение) >= 64) КАК ЕстьКартинка,
	      	               ...
	      	               |СГРУППИРОВАТЬ ПО
	      	               |	Чемпионаты.Владелец,
	      	               |	Чемпионаты.Владелец.Код,
	      	               |	Чемпионаты.Владелец.Наименование
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	ЕстьКартинка УБЫВ,
	      	               |	Количество УБЫВ,
...
Процедура СезонВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Значение = Сезон.Получить(ВыбраннаяСтрока).Значение;
	Если ТипЗнч(Значение) = Тип("ДокументСсылка.Матч") Тогда
