...
Функция СезонНаСервере()
	...
	Если ЗначениеЗаполнено(Чемпионат) Тогда
		...
		Если СыграноМатчей > 1 Тогда
			...
			ЗаполнитьЗначенияСвойств(Сезон.Добавить(), Новый Структура("Ключ,Значение", "Самый крупный счет", ПостОбработка(Выборка.Получить(1))));
			ЗаполнитьЗначенияСвойств(Сезон.Добавить(), Новый Структура("Ключ,Значение", "Самая крупная победа домашняя", ПостОбработка(Выборка.Получить(2))));
			ЗаполнитьЗначенияСвойств(Сезон.Добавить(), Новый Структура("Ключ,Значение", "Самая крупная победа гостевая", ПостОбработка(Выборка.Получить(3))));
			ЗаполнитьЗначенияСвойств(Сезон.Добавить(), Новый Структура("Ключ,Значение", "Последний матч", ПостОбработка(Выборка.Получить(4))));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЧемпионатНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	НаКлиенте.ОбработкаКомандыОткрытьФорму("Справочник.Чемпионаты.ФормаВыбора", Новый Структура("Отбор", Новый Структура("Ссылка", Чемпионаты.ВыгрузитьЗначения())), Элемент, СтандартнаяОбработка);
КонецПроцедуры
