...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Параметры.ЗначенияЗаполнения.Свойство("Страна") Тогда
			Объект.Город = ГородПодобрать(Параметры.ЗначенияЗаполнения.Страна);
		КонецЕсли;
	Иначе
...
Процедура СтраныЗагрузить()
	Если ЗначениеЗаполнено(Объект.Город) Тогда
		Страны.ЗагрузитьЗначения(Страны(Объект.Город));
	Иначе
		Объект.Город = ГородНайти(Объект.Наименование);
		Если ЗначениеЗаполнено(Объект.Город) Тогда
			СтандартнаяОбработка = Ложь;
		ИначеЕсли Объект.Ссылка.Пустая() Тогда
			Страны.ЗагрузитьЗначения(Страны());
		Иначе
			Страны.ЗагрузитьЗначения(Страны(Объект.Ссылка));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция Страны(Данные=Неопределено)
