...
Процедура ФотоПолучить() Экспорт
	Если Элементы.ТекущийСостав.ТекущиеДанные = Неопределено Тогда
		//
	ИначеЕсли ТекущийИгрок = Элементы.ТекущийСостав.ТекущиеДанные.Игрок Тогда
		//
	Иначе
		ТекущийИгрок = Элементы.ТекущийСостав.ТекущиеДанные.Игрок;
		ФотоПолучитьНаСервере(Элементы.ТекущийСостав.ТекущиеДанные.Игрок);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ФотоПолучитьНаСервере(Ссылка)
	Элемент	= Кеш.НайтиПоЗначению(Ссылка);
	Если Элемент = Неопределено Тогда
		ИЗО	= ИзображениеПолучить(Ссылка);
