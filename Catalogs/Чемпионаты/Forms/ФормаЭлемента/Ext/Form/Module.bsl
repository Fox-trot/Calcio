...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ТипЗнч(Параметр) = Тип("СправочникСсылка.Чемпионаты") И Параметр = Объект.Ссылка Тогда
		ПодключитьОбработчикОжидания("Перечитать", 0.1, Истина);
		ПодключитьОбработчикОжидания("ВидимостьДоступность", 0.2, Истина);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВидимостьДоступностьУстановить()
	Если ЕстьИстория(Объект.Ссылка) Тогда
		...
		Элементы.Даты.ТолькоПросмотр		= НЕ РедактированиеДоступно;
	...
	Иначе
		...
		Элементы.Даты.ТолькоПросмотр		= Ложь;
	...
	КонецЕсли;
	Элементы.КомандыКомандаГород.Видимость			= ЗначениеЗаполнено(Объект.Владелец.Страна);
	Элементы.КомандыКомандаГородВладелец.Видимость	= НЕ ЗначениеЗаполнено(Объект.Владелец.Страна);
...
Функция Нонсенсы()
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	      	               |	НРЕГ(Команды.Наименование) КАК Наименование
	      	               |ИЗ
	      	               |	Справочник.Команды КАК Команды
	      	               |ГДЕ
	      	               |	Команды.Наименование < ""a""");
...
Функция ИмпортНаСервере(Гиперссылка)
	...
	Иначе
		Нонсенс	= Нонсенсы();
		мСлова	= Новый Массив;
		Исключения	= Разделить("матч,клуб,мячи,очки,метка,группа,счет,финал,№,команда,форма", ",");
		Для Каждого Строка Из Разделить(Гиперссылка, "¶") Цикл
			Если СтрНайти("A0 A1 A2 B0 B1 B2 QF SF F1", Лев(Строка, 2)) > 0 Тогда
				Продолжить;
			ИначеЕсли СтрНайти(Строка, ",") > 0 Тогда
				Продолжить;
			ИначеЕсли СтрСравнить(Лев(Строка, 6), "Группа") = 0 Тогда Продолжить;
			ИначеЕсли СтрНайти(Строка, Символ(9)) = 0 Тогда
				Слова = Разделить(СтрЗаменить(Строка, " – ", ","), ",", Истина);
			Иначе
				Слова = Разделить(Строка, Символ(9), Истина);
			КонецЕсли;
			Для Каждого Слово Из Слова Цикл
				...
				ИначеЕсли Исключения.Найти(НРег(Слово)) <> Неопределено Тогда
					//
				ИначеЕсли СтрНайти(Объект.Наименование, Слово) > 0 Тогда
					Исключения.Добавить(НРег(Слово));
				ИначеЕсли СтрокаНачинаетсяЧислом(Слово) Тогда
					Если Нонсенс.Найти(НРег(Слово)) = Неопределено Тогда
						Исключения.Добавить(НРег(Слово));
					ИначеЕсли мСлова.Найти(Слово) = Неопределено Тогда
...
Функция ДатаОкончания(Дата)
	Возврат ?(Объект.Владелец.КоличествоКоманд > 2, КонецМесяца(ДобавитьМесяц(Дата, 9)), Дата);
КонецФункции
...
&НаСервере
Процедура ИтогиПереписатьНаСервере(Ссылка, Принудительно=Ложь)
	РегистрыСведений.Итоги.Обновить(Ссылка, Истина, Принудительно);
	РедактированиеУстановить();
КонецПроцедуры

&НаКлиенте
Процедура ИтогиНаКлиенте(Принудительно=Ложь)
	ИтогиПереписатьНаСервере(Объект.Ссылка, Принудительно);
	Прочитать();
	ВидимостьДоступностьУстановить();
	
	НаКлиенте.Оповещение("Чемпионат", Объект.Ссылка);
КонецПроцедуры
...
&НаКлиенте
Процедура ИтогиЗаписать(Команда)
	ИтогиНаКлиенте(Истина);
КонецПроцедуры
...
Функция УРЛ(Команда=Неопределено)
	...
	Иначе
		...
		Если СтрНайти(УРЛ, "%1") > 0 Тогда
			УРЛ	= СтрШаблон(УРЛ, ФорматЧГ(Объект.ДатаОкончания));
		КонецЕсли;
	КонецЕсли;
	Возврат УРЛ;
КонецФункции
...
Процедура ИмпортИгроковНаСервере()
	Если ТекущаяДатаСеанса() > Объект.ДатаНачала И ТекущаяДатаСеанса() < Объект.ДатаОкончания Тогда
		КомОбъект = Неопределено;
		Для Каждого ТекСтрока Из Объект.Команды Цикл
			УРЛ = УРЛ(ТекСтрока.Команда);
			Если ЭтоУРЛ(УРЛ) Тогда
				Если СтрНайти(УРЛ, ФорматЧГ(Объект.ДатаОкончания)) > 0 Тогда
					Если НЕ СерверныйФункции.ИмпортИгроков(УРЛ, ТекСтрока.Команда, КомОбъект) Тогда
						КомОбъект = Неопределено;
					КонецЕсли;
				Иначе
					ОбщегоНазначения.СообщитьОбОшибке(ТекСтрока.Команда,, ТекСтрока.Команда);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ИмпортИгроков(КодОтвета, ПараметрКоманды) Экспорт
...
&НаКлиенте
Процедура КомандаИмпортИгроков(Команда)
	ПоказатьВопрос(Новый ОписаниеОповещения("ИмпортИгроков", ЭтотОбъект), СтрШаблон("Будут загружены все команды (%1) Продолжить?", Объект.Команды.Количество()), РежимДиалогаВопрос.ОКОтмена, 60, КодВозвратаДиалога.Отмена, "Импорт игроков");
КонецПроцедуры

&НаКлиенте
Процедура КомандыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Элементы.Команды.ТолькоПросмотр Тогда
		Показать(Элемент.ТекущиеДанные.Команда, СтандартнаяОбработка);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура СезонВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Показать(Элемент.ТекущиеДанные.Значение, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура ИнформацияОбновить()
	...
	Если Итоги.Следующий() Тогда
		СезонДобавить("Сыграно матчей", Итоги.СыграноМатчей);
		Если Объект.Владелец.ОлимпийскаяСистема Тогда
			СезонДобавить("Забито голов", Итоги.ЗабитоГолов);
		Иначе
			СезонДобавить("Побед хозяев", Итоги.ПобедХозяев);
			СезонДобавить("Ничьих", Итоги.Ничьих);
			СезонДобавить("Побед гостей", Итоги.СыграноМатчей - Итоги.ПобедХозяев - Итоги.Ничьих);
			
			СезонДобавить("Забито голов", Итоги.ЗабитоГолов);
			СезонДобавить("Забито голов хозяевами", Итоги.ЗабитоГоловХозяевами);
			СезонДобавить("Забито голов гостями", Итоги.ЗабитоГолов - Итоги.ЗабитоГоловХозяевами);
			
			СезонДобавить("Голов в среднем за игру (хозяева)",	Окр(Итоги.ЗабитоГоловХозяевами / Итоги.СыграноМатчей, 1));
			СезонДобавить("Голов в среднем за игру (гости)", Окр((Итоги.ЗабитоГолов - Итоги.ЗабитоГоловХозяевами) / Итоги.СыграноМатчей, 1));
		КонецЕсли;
		
		Если Итоги.СыграноМатчей > 1 Тогда
			СезонДобавить("Голов в среднем за игру", Окр(Итоги.ЗабитоГолов / Итоги.СыграноМатчей, 1));
		КонецЕсли;
		
		Если Итоги.ЗрителейВсего > 0 Тогда
			СезонДобавить("Зрителей всего", Итоги.ЗрителейВсего);
			Если Итоги.МатчейСоЗрителями > 1 Тогда
				СезонДобавить("Зрителей в среднем за игру", Окр(Итоги.ЗрителейВсего / Итоги.МатчейСоЗрителями, 0));
			КонецЕсли;
		КонецЕсли;	
		
		Если Итоги.СыграноМатчей > 1 Тогда
			Для Итератор = 2 По 4 Цикл
				Матч			= ПостОбработка(Выборка.Получить(Итератор));
				Если Итератор = 2 И ЗначениеЗаполнено(Матч) Тогда
					СезонДобавить("Самый крупный счет", Матч);
				ИначеЕсли Итератор = 3 И ЗначениеЗаполнено(Матч) Тогда
					СезонДобавить("Самая крупная победа домашняя", Матч);
				ИначеЕсли Итератор = 4 И ЗначениеЗаполнено(Матч) Тогда
					СезонДобавить("Самая крупная победа гостевая", Матч);
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли Итоги.СыграноМатчей = 1 И Объект.Владелец.ОлимпийскаяСистема Тогда
			Матч			= ПостОбработка(Выборка.Получить(2));
			Если ЗначениеЗаполнено(Матч) Тогда
				СезонДобавить("Матч", Матч);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура СезонДобавить(Ключ, Значение)
	ЗаполнитьЗначенияСвойств(Сезон.Добавить(), Новый Структура("Ключ,Значение", Ключ, Значение));
КонецПроцедуры
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	НаКлиенте.Оповещение("Чемпионат", Объект.Ссылка);
	Если ТурыНужны(Объект.Ссылка) Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ВопросПослеЗаписи", ЭтотОбъект), "Создать туры?", РежимДиалогаВопрос.ОКОтмена, 60, КодВозвратаДиалога.Отмена, "Туры чемпионата");
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Функция ТурыНужны(Ссылка)
	Если Ссылка.Владелец.ОлимпийскаяСистема Тогда
		Возврат Ложь;
	КонецЕсли;
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
...
&НаКлиенте
Процедура ВопросПослеЗаписи(КодОтвета, ПараметрКоманды) Экспорт
	Если КодОтвета = КодВозвратаДиалога.ОК Тогда
		ТурыЗаполнитьНаСервере();
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ТурыЗаполнитьНаСервере()
	Справочники.Туры.Заполнить(Объект.Ссылка);
КонецПроцедуры

&НаКлиенте
Процедура ТурыСоздать(Команда)
	ТурыЗаполнитьНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура СтатистикаОбновить(Команда)
	ИнформацияОбновить();
КонецПроцедуры

&НаКлиенте
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	Если Элементы.СтрнаицаСтатистика = ТекущаяСтраница
	И Сезон.Количество() = 0
	Тогда
		ИнформацияОбновить();
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Если Модифицированность И НЕ РедактированиеДоступно Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Включите возможность редактирования", Отказ);
	КонецЕсли;
КонецПроцедуры
...
Процедура ФотоПолучитьНаСервере()
	...
	Если Элемент = Неопределено Тогда
		КомандаИзображение	= СерверныйФункции.ИзображениеПолучить(ТекущийКлуб);
		Кеш.Добавить(ТекущийКлуб, КомандаИзображение);
	Иначе
		КомандаИзображение	= Элемент.Представление;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ФотоПолучить() Экспорт
	ТекущиеДанные	= Элементы.Команды.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		КомандаИзображение	= "";
		ТекущийКлуб = Неопределено;
	ИначеЕсли ТекущийКлуб = ТекущиеДанные.Команда Тогда
		Если Кеш.НайтиПоЗначению(ТекущийКлуб) = Неопределено Тогда
			ФотоПолучитьНаСервере();
		КонецЕсли;
	Иначе
		ТекущийКлуб = ТекущиеДанные.Команда;
		ФотоПолучитьНаСервере();
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура КомандыКомандаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Страна) Тогда
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("Справочник.Команды.ФормаВыбора", Новый Структура("Страна,Отбор", Страна, Новый Структура("Ссылка", КомандыСтраны(Страна))), Элемент);
	КонецЕсли;
КонецПроцедуры
