...
&НаСервере
Функция УРЛ()
	Ответ = ?(СтрНайти(НРег(Объект.Наименование), "англ") = 0, "", СтрШаблон("https://football.kulichki.net/england/%1/", Формат(Год(Объект.ДатаОкончания), "ЧГ=0")));
	Возврат ?(ПустаяСтрока(Ответ), СерверныйФункции.КомментарийКакУРЛ(Объект.Комментарий), Ответ);
КонецФункции
