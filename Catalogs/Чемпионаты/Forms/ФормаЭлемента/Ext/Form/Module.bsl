...
&НаСервереБезКонтекста
Процедура ИтогиПромежуточныеОбновитьНаСервере(Ссылка, Замещать=Ложь)
	Если НЕ Ссылка.Пустая() Тогда
		РегистрыСведений.ИтогиПромежуточные.Обновить(Ссылка, Замещать);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ИтогиПромежуточныеОбновить(Команда)
	ИтогиПромежуточныеОбновитьНаСервере(Объект.Ссылка);
КонецПроцедуры

&НаКлиенте
Процедура ИтогиПромежуточныеПереписать(Команда)
	ИтогиПромежуточныеОбновитьНаСервере(Объект.Ссылка, Истина);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ИтогиПромежуточныеОчиститьНаСервере(Ссылка)
	Если НЕ Ссылка.Пустая() Тогда
		РегистрыСведений.ИтогиПромежуточные.Очистить(Ссылка);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ИтогиПромежуточныеОчистить(Команда)
	ИтогиПромежуточныеОчиститьНаСервере(Объект.Ссылка);
КонецПроцедуры
