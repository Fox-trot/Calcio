...
&НаСервере
Функция ЧемпионатЗакончен()
	Возврат РегистрыСведений.Итоги.ЧемпионатЗакончен(Объект.Ссылка);
КонецФункции
...
Функция ИмпортНаСервере(Гиперссылка)
	...
	Для Каждого Слово Из мСлова Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если Объект.Команды.Количество() < КоличествоКоманд
	И Объект.Команды.Количество() + Виктимз.Количество() = КоличествоКоманд
	Тогда
