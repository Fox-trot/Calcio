...
Процедура ИтогиПереписатьНаСервере(Ссылка, Принудительно=Ложь)
	Если ЗначениеЗаполнено(СамыйКрупныйСчет) Тогда
...
Процедура ИнформацияОбновить()
	...
	Если Итоги.Следующий() Тогда
		...
		Элементы.ГоловВСреднемЗаИгру.Видимость = (Итоги.СыграноМатчей > 1);
		Если Элементы.ГоловВСреднемЗаИгру.Видимость Тогда
			ГоловВСреднемЗаИгру			= Окр(Итоги.ЗабитоГолов / Итоги.СыграноМатчей, 1);
		КонецЕсли;
		
		ЗрителейВсего				= Итоги.ЗрителейВсего;
		Элементы.ЗрителейВСреднемЗаИгру.Видимость = (Итоги.ЗрителейВсего > 0);
		Если Элементы.ЗрителейВСреднемЗаИгру.Видимость Тогда
			ЗрителейВСреднемЗаИгру		= ?(Итоги.МатчейСоЗрителями = 0, "", Окр(Итоги.ЗрителейВсего / Итоги.МатчейСоЗрителями, 0));
		КонецЕсли;	
		
		Элементы.ГруппаКрупныйСчет.Видимость = (Итоги.СыграноМатчей > 1);
		Если Элементы.ГруппаКрупныйСчет.Видимость Тогда
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	И НЕ ЗначениеЗаполнено(СамыйКрупныйСчет)
	Тогда
		ИнформацияОбновить();
	КонецЕсли;
КонецПроцедуры
