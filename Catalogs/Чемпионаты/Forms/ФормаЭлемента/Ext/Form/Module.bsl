...
Функция ИмпортНаСервере(Гиперссылка)
	...
	Иначе
		мСлова	= Новый Массив;
		Исключения	= Разделить("матч,клуб,мячи,очки,метка,группа,тур,счет", ",");
		Для Каждого Тимз Из Разделить(Гиперссылка, "¶") Цикл
			Если СтрНайти(Тимз, ",") > 0 Тогда
				Продолжить;
			ИначеЕсли Лев(Тимз, 2) = "A0" Тогда
				Продолжить;
			ИначеЕсли СтрНайти(Тимз, Символ(9)) = 0 Тогда
				Слова = Разделить(СтрЗаменить(Тимз, " – ", ","), ",", Истина);
			Иначе
	...
	Для Каждого Слово Из мСлова Цикл
		...
		//ИначеЕсли СтрСравнить(Слово, "дата, время") = 0 Тогда
...
Процедура ИтогиПереписать(Команда)
	...
	Прочитать();
	НаКлиенте.Оповещение("Чемпионат");
КонецПроцедуры

&НаКлиенте
Процедура ИтогиЗаписать(Команда)
	ИтогиПереписатьНаСервере(Объект.Ссылка, Истина);
	Прочитать();
	НаКлиенте.Оповещение("Чемпионат");
КонецПроцедуры
...
Процедура ИнформацияОбновить()
	...
	Если Итоги.Следующий() Тогда
		...
		Если Итоги.ЗрителейВсего > 0 И Итоги.МатчейСоЗрителями > 1 Тогда
