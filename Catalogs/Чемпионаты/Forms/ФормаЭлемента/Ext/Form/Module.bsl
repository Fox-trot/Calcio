	...
	Если ТекущаяДатаСеанса() > Объект.ДатаНачала И ТекущаяДатаСеанса() < Объект.ДатаОкончания Тогда
		...
		Для Каждого ТекСтрока Из Объект.Команды Цикл
			...
			Если ЭтоУРЛ(УРЛ) И СтрНайти(УРЛ, ФорматЧГ(Объект.ДатаОкончания)) > 0 Тогда
				Если НЕ ?(Год(Объект.ДатаОкончания) >= 2022, СерверныйФункции.ИмпортИгроковНаСервере(УРЛ, ТекСтрока.Команда, КомОбъект), СерверныйФункции.ИмпортИгроковНаСервере2(УРЛ, ТекСтрока.Команда)) Тогда
					КомОбъект = Неопределено;
				КонецЕсли;
			Иначе
