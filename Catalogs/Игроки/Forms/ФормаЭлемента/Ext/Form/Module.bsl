...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Амплуа) Тогда
		ВидСпорта = Объект.Амплуа.Владелец;
	Иначе
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	НаКлиенте.Оповещение("Игрок", Объект.Ссылка, ЭтаФорма);
КонецПроцедуры
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ОбщегоНазначения.УстановитьЗначение(ВидСпорта, ВидСпорта(ДанныеИгрока.Команда), Истина);
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	И СтрСравнить(ИмяСобытия, "Игрок") = 0
	И ТипЗнч(Параметр) = Тип("СправочникСсылка.Игроки") И Параметр = Объект.Ссылка
	Тогда
		Прочитать();
	КонецЕсли;
КонецПроцедуры
