...
Процедура ВидимостьДоступностьУстановить()
	Если ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Города") Тогда
		Элементы.Город.ТолькоПросмотр				= НЕ РедактированиеДоступно;
...
Процедура ВладелецПриИзмененииНаСервере()
	...
	ИначеЕсли ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Команды") Тогда
		...
		Если ЗначениеЗаполнено(Объект.Город) Тогда
			Страна	= Объект.Владелец.Город.Владелец;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Выборка.Следующий() Тогда
			ОбщегоНазначения.СообщитьОбОшибке("Уже есть стадион с таким названием", Отказ, Объект.Ссылка, "Объект.Наименование");
		Иначе	//Если ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Команды") Тогда
			...
			Если Выборка.Следующий() Тогда
				Если ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Команды") Тогда
					ОбщегоНазначения.СообщитьОбОшибке(СтрШаблон("У команды уже есть стадион %1", Выборка.Ссылка),, Объект.Ссылка, "Объект.Владелец");
				Иначе
					ОбщегоНазначения.СообщитьОбОшибке(СтрШаблон("У города уже есть стадион %1", Выборка.Ссылка),, Объект.Ссылка, "Объект.Владелец");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	Если Объект.Ссылка.Пустая() Тогда
		ВладелецПриИзмененииНаСервере();
	ИначеЕсли НЕ ЕстьИстория(Объект.Ссылка) Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	И ТипЗнч(Параметры.Владелец) = Тип("СправочникСсылка.Команды")
	Тогда
		Объект.Владелец	= Параметры.Владелец;
		Страна			= Параметры.Владелец.Город.Владелец;
	
	ИначеЕсли ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Города") Тогда
		Страна = Объект.Владелец;
	ИначеЕсли ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Команды") Тогда
		Страна	= Объект.Владелец.Город.Владелец;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	НаКлиенте.Оповещение("Стадион", Объект.Ссылка);
КонецПроцедуры
...
&НаКлиенте
Процедура КомандаВладелец(Команда)
	Объект.Владелец = Объект.Город;
КонецПроцедуры
