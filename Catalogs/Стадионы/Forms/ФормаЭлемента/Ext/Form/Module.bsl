...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если Объект.Ссылка.Пустая() Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Страны КАК Страны
		                      |			ПО Города.Владелец = Страны.Владелец
		...
		Если Выборка.Следующий() Тогда
			ОбщегоНазначения.СообщитьОбОшибке("Уже есть стадион с таким названием",, Объект.Ссылка, "Наименование");
		Иначе	//Если ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Команды") Тогда
			...
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				ОбщегоНазначения.СообщитьОбОшибке("У команды/города уже есть один стадион",, Объект.Ссылка, "Владелец");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
