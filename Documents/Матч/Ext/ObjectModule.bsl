...
Функция СчетПолучить(Проведение) Экспорт
	Ответ = "";
	Если Команды.Количество() = 2 Тогда
		...
		Если Проведение Тогда
			Если Хозяева.Пенальти + Гости.Пенальти = 0 Тогда
				Ответ = СтрШаблон("%1 - %2 %3:%4", Хозяева.Команда, Гости.Команда, Формат(Хозяева.КоличествоГолов, "ЧН=0; ЧГ=0"), Формат(Гости.КоличествоГолов, "ЧН=0; ЧГ=0"));
			Иначе
				Ответ = СтрШаблон("%1 - %2 %3:%4 (%5:%6)", Хозяева.Команда, Гости.Команда, Формат(Хозяева.КоличествоГолов, "ЧН=0; ЧГ=0"), Формат(Гости.КоличествоГолов, "ЧН=0; ЧГ=0"), Формат(Хозяева.Пенальти, "ЧН=0; ЧГ=0"), Формат(Гости.Пенальти, "ЧН=0; ЧГ=0"));
			КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ЗначениеЗаполнено(Тур) Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |			И МатчКоманды.Ссылка.Тур.ОлимпийскаяСистема = ЛОЖЬ) КАК МатчКоманды
		                      |		ПО ТурнирнаяТаблица.Тур = МатчКоманды.Тур
		...
		Пока НЕ Отказ И Выборка.Следующий() Цикл
			Если Выборка.Количество = 2 И Выборка.Ссылка <> Ссылка Тогда
		...
		Если НЕ Отказ Тогда
			...
			Если Выборка.Следующий() Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрШаблон("Неверный счет: %1", СчетПолучить(Истина)));
			КонецЕсли;
		КонецЕсли;
	Иначе
