...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Хозяева <> Неопределено И ?(ЗначениеЗаполнено(Тур), НЕ Тур.ОлимпийскаяСистема, Ложь) Тогда
		ОбщегоНазначения.УстановитьЗначение(Хозяева.Пенальти, 0);
		ОбщегоНазначения.УстановитьЗначение(Гости.Пенальти, 0);
	КонецЕсли;
...
Функция ДатаНачала(Тур)
	...
	Запрос.УстановитьПараметр("Тур",				Тур);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() И Выборка.Дата < ТекущаяДатаСеанса() Тогда
