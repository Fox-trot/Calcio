...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		Отказ = СерверныйФункции.СообщитьОбОшибке("Недостаточно команд");
	КонецЕсли;
	Если НЕ Отказ Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
		                      |	ТурнирнаяТаблицаОбороты.Команда КАК Команда
		                      |ИЗ
		                      |	РегистрНакопления.ТурнирнаяТаблица.Обороты(&ДатаНач, &ДатаКон, , Команда В (&Команды)) КАК ТурнирнаяТаблицаОбороты");
		...
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			Отказ = СерверныйФункции.СообщитьОбОшибке("Такой матч уже есть в базе");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	Если НачалоДня(Дата) > НачалоДня(ТекущаяДата()) Тогда
		Отказ = СерверныйФункции.СообщитьОбОшибке("Дата неверна");
	КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	КонецЦикла;
	Если Команды.Количество() = 2 Тогда
		Темпо	= Команды[0].Команда;
		Команды[0].Команда	= Команды[1].Команда;
		Команды[1].Команда	= Темпо;
	КонецЕсли;
КонецПроцедуры
