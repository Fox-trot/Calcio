	...
	ИначеЕсли ЗначениеЗаполнено(Тур) Тогда
		...
		Пока НЕ Отказ И Выборка.Следующий() Цикл
			Если Выборка.Ссылка <> Ссылка Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрШаблон("Такой матч уже есть в базе (%1) %2", Выборка.Ссылка, Тур), Отказ, Выборка.Ссылка);
			КонецЕсли;
		КонецЦикла;
	Иначе
		мКоманды	= Команды.Выгрузить().ВыгрузитьКолонку("Команда");
		...
		Пока НЕ Отказ И Выборка.Следующий() Цикл
			Если Выборка.Ссылка <> Ссылка Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрШаблон("Такой матч уже есть в базе (%1)", Выборка.Ссылка), Отказ, Выборка.Ссылка);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Для Каждого тСтрока Из Команды Цикл
		...
		Запись.Тур				= Тур;
		Запись.КоличествоОчков	= КоличествоОчковРасчитать(Секондо, КоличествоОчков);
...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	Если ТипЗнч(Параметр) = Тип("Структура") Тогда
		Возврат СерверныйФункции.СтадионНайти(Параметр, ЭтотМатчТурФинал());
	КонецЕсли;
