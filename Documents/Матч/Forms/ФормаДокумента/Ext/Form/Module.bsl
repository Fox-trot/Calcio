...
Процедура АнонсОбновитьНаСервере()
	...
	КонецЦикла;
	КакЕсть	= (ИгрСНачалаСезона < 5);			// недостаточно игр с начала сезона
	ИгрСНачалаСезона	= 0;
	Выборка.Сбросить();
...
Процедура КомандыПриИзмененииНаСервере()
	...
	Для Каждого ТекСтрока Из Объект.Команды Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ОбщегоНазначения.КартинкиЗагрузить(Элементы.График2Команда.СписокВыбора, Кеш);
	...
	Если НЕ Ризалт.Пустой() Тогда
		...
		Если График2Группировать Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				КонецЦикла;
				График2.УстановитьЗначение(Точка, Серия1, Выборка.Фаза1, Расшифровка);
				График2.УстановитьЗначение(Точка, Серия2, Выборка.Фаза2, Расшифровка);
				График2.УстановитьЗначение(Точка, Серия3, Выборка.Фаза3, Расшифровка);
			КонецЦикла;
		Иначе
			Выборка = Ризалт.Выбрать();
			Пока Выборка.Следующий() Цикл
				Точка	= График2.УстановитьТочку(Выборка.Точка);
				График2.УстановитьЗначение(Точка, Серия1, Выборка.Фаза1, Выборка.Точка);
				График2.УстановитьЗначение(Точка, Серия2, Выборка.Фаза2, Выборка.Точка);
				График2.УстановитьЗначение(Точка, Серия3, Выборка.Фаза3, Выборка.Точка);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция ЧемпионатыПолучить(Дата, Данные=Неопределено)
	Запрос = Новый Запрос("ВЫБРАТЬ
	...
	Если ТипЗнч(Данные) = Тип("ДанныеФормыКоллекция") Тогда
		Запрос.Текст	= "ВЫБРАТЬ
		            	  ...
		            	  |УПОРЯДОЧИТЬ ПО
		            	  |	Ссылка";
		Запрос.УстановитьПараметр("Данные",			Данные.Выгрузить(, "Команда").ВыгрузитьКолонку(0));
...
Процедура ЧемпионатНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Значения	= ЧемпионатыПолучить(Объект.Дата, ВидСпорта);
...
Процедура СоставыИмпорт(Команда=Неопределено)
	...
	ИначеЕсли Команда = Неопределено Или Объект.СоставыКоманд.Количество() = 0 Тогда
	...
	Если Слова.Количество() > 0 Тогда
		...
		Для Каждого Слово Из Слова Цикл
			...
			ИначеЕсли СтрНайти(Слово, "(") > 0 Тогда
				...
				КонецЦикла;
			Иначе
				сИгрок = СерверныйФункции.ИгрокНайти(Слово, Объект.Ссылка);
