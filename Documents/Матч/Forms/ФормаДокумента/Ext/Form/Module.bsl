...
Процедура АнонсОбновитьНаСервере()
	Хозяева		= Хозяева();
	ИзображениеХозяева	= СерверныйФункции.ИзображениеПолучить(Хозяева);
	Гости		= Гости();
	ИзображениеГости	= СерверныйФункции.ИзображениеПолучить(Гости);
...
Процедура КомандыПриИзменении(Элемент=Неопределено)
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.Тур) Тогда
		Объект.Тур		= СерверныйФункции.Тур(Хозяева(), Объект.Дата);
	КонецЕсли;
