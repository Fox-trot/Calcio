...
&НаСервере
Функция ВидСпортаПолучить()
	Если ЗначениеЗаполнено(Объект.Тур) Тогда
		Возврат СерверныйФункции.ВидСпорта(Объект.Тур);
	ИначеЕсли ЗначениеЗаполнено(Чемпионат) Тогда
		Возврат СерверныйФункции.ВидСпорта(Чемпионат);
	ИначеЕсли ЗначениеЗаполнено(Хозяева) Тогда
		Возврат СерверныйФункции.ВидСпорта(Хозяева);
	КонецЕсли;
	Возврат СерверныйФункции.Футбол();
КонецФункции

&НаСервере
Процедура ВидСпортаУстановить()
	ОбщегоНазначения.УстановитьЗначение(ВидСпорта, ВидСпортаПолучить());
КонецПроцедуры
...
Функция КомандыТура()
	...
	Иначе
		...
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |ИЗ
		                      |	Справочник.Команды КАК Команды
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица.Обороты(&ДатаН, &ДатаК, , ) КАК ТурнирнаяТаблица
		...
		Запрос.УстановитьПараметр("ВидСпорта",		ВидСпортаПолучить());
...
Функция Тур()
	...
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Ответ) Тогда
		Ответ = СерверныйФункции.Тур(ВидСпорта, Объект.Дата);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
