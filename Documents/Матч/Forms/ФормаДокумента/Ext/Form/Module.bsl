...
Процедура АнонсОбновитьНаСервере()
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ИграУстановить(?(Выборка.Хозяева, Элементы.ГруппаПредыдущие5Хозяева, Элементы.ГруппаПредыдущие5Гости), Выборка);
	КонецЦикла;
КонецПроцедуры
...
&НаСервере
Процедура ИграУстановить(Родитель, Выборка)
...
Процедура ЧемпионатУстановить()
	ОбщегоНазначения.УстановитьЗначение(Чемпионат, Объект.Тур.Владелец);
КонецПроцедуры
