...
Процедура СоставыЗаполнитьНаСервере()
	...
	Если Объект.Команды.Количество() = 2 Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	Игрок
		               |АВТОУПОРЯДОЧИВАНИЕ";
		Запрос.УстановитьПараметр("Гости", Объект.Команды[1].Команда);
	КонецЕсли;
...
Процедура ПрошлыеВстречиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ОткрытьФорму("Документ.Матч.ФормаОбъекта", Новый Структура("Ключ", Элемент.ТекущиеДанные.Регистратор), ЭтаФорма);
КонецПроцедуры

&НаСервере
Процедура АнонсОбновитьНаСервере()
	//ТабДок.ПолучитьОбласть(
	//ТабДок.Вывести("Анонс");
	//
	//Для Каждого тСтрока Из Объект.Команды Цикл
	...
	//	Запрос.УстановитьПараметр("Чемпионат",	Объект.Тур.Владелец);
	//	Запрос.УстановитьПараметр("Дата",		Объект.Дата);
	...
	//		Если Каунт = 0 Тогда
	//			Каунт	= 1;
	//			Победа	= Выборка.КоличествоОчков;
	//		ИначеЕсли Выборка.КоличествоОчков = Победа Тогда
	//			Каунт	= Каунт + 1;
	//		Иначе
	//			Прервать;
	//		КонецЕсли;
	...
	//		ТабДок.Вывести(Строка(тСтрока.Команда) + " последние " + Строка(Каунт) + " матча " + ?(Победа=0, "проиграла", ?(Победа=1, "сыграла вничью", "выиграла")));
	//	КонецЕсли;
	//КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура АнонсОбновить(Команда)
	АнонсОбновитьНаСервере();
КонецПроцедуры
