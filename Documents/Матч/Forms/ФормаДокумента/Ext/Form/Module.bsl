...
Функция СтадионНайти(Тим, Тур=Неопределено)
	...
	Если ЗначениеЗаполнено(Тур) И Тур.ОлимпийскаяСистема Тогда
		мСтадионы = СерверныйФункции.СтадионНайти(Тур.Владелец, Истина);
		Для Каждого Стадион Из мСтадионы Цикл
			Ответ.Добавить(Стадион);
		КонецЦикла;
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Тогда
		...
		//ОбновлятьЭлементы.Добавить(Элементы.Шапка);
		//ОбновлятьЭлементы.Добавить(Элементы.Страницы);
