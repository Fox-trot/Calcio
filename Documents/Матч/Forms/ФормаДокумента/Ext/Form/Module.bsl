...
Функция СтадионНайти(Тим)
	Если ЗначениеЗаполнено(Объект.Тур) Тогда
		Ответ = РеквизитФормыВЗначение("Объект").СтадионНайти(Новый Структура("Команда,Тур", Тим, Объект.Тур), Истина);
...
Процедура СтадионыЗагрузитьНаСервере(Знач Клуб=Неопределено)
	...
	Если Модифицированность И НЕ Объект.Проведен Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если ЗначениеЗаполнено(Клуб) И Стадионы.Количество() > 1 Тогда
		Элементы.Стадион.СписокВыбора.Очистить();
		Штад = СерверныйФункции.СтадионНайти(Клуб, Ложь);
		Если ЗначениеЗаполнено(Штад) Тогда
			Элементы.Стадион.СписокВыбора.Добавить(Штад);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
