...
Функция СоставКоманды(Команда)
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      ...
	                      |			ПО Игроки.Ссылка = СоставКоманды.Игрок
	                      |		ПО Участники.Игрок = Игроки.Ссылка
	                      |ГДЕ
	                      |	Участники.Команда = &Команда
	                      |	И Участники.Ссылка В
	                      |			(ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |					ПО
	                      |						СоставКоманды.Ссылка = Матч.Ссылка
	                      |			ГДЕ
...
Функция СтадионНайти(Тим, Тур=Неопределено)
	...
	//ИначеЕсли ЗначениеЗаполнено(Чемпионат.Владелец) И Чемпионат.Владелец.ОлимпийскаяСистема Тогда
...
Процедура СтадионНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если Объект.Команды.Количество() > 0 Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Стадион) Тогда
			...
			КонецЕсли;
			Если Стадионы.НайтиПоЗначению(Стадион) = Неопределено Тогда
