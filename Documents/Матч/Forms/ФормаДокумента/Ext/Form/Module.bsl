...
Процедура ГрафикОбновитьНаСервере()
	...
	Если КомандыДве(Истина) Тогда
		...
		Если Объект.Ссылка.Проведен Тогда
			График.ОбластьЗаголовка.Расположение = РасположениеОбластиЗаголовкаДиаграммы.Нет;
		Иначе
			График.ОбластьЗаголовка.Расположение = РасположениеОбластиЗаголовкаДиаграммы.Авто;
			...
			Если мСтроки.Количество() = 0 Или Модифицированность Тогда
				График.ОбластьЗаголовка.Текст	= "";
			Иначе
				ТекСтрока = мСтроки.Получить(0);
				График.ОбластьЗаголовка.Текст		= СтрШаблон("%1 - %2", ТекСтрока.Фаза, ТекСтрока.ФазаП);
