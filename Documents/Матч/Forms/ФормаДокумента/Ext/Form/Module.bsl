...
Процедура ГрафикОбновитьНаСервере()
	...
	Если КомандыДве(Истина) Тогда
		Для Каждого тКоманда Из Объект.Команды Цикл
			мСтроки = ГрафикПолучитьНаСервере(тКоманда.Команда, Амплуа);
...
&НаСервере
Функция ГрафикПолучитьНаСервере(Команда, Роль=Неопределено)
	...
	Если ЗначениеЗаполнено(Роль) Тогда
		...
		Запрос.УстановитьПараметр("Амплуа",		Роль);
	КонецЕсли;
	...
	Если Ризалт.Пустой() Тогда
		...
		Ризалт = Запрос.Выполнить();
	КонецЕсли;
	Возврат Ризалт.Выгрузить().ВыгрузитьКолонку(?(ЗначениеЗаполнено(Роль), 0, "ДатаРождения"));
КонецФункции
...
Процедура КомандыПриИзмененииНаСервере()
	...
	Для Каждого тСтрока Из Объект.Команды Цикл
		Если ЗначениеЗаполнено(тСтрока.Команда) Тогда
			...
			И (Модифицированность Или НЕ ЗначениеЗаполнено(Объект.Стадион) И НЕ Объект.Проведен)
			Тогда
			...
			Элементы.График2Команда.СписокВыбора.Добавить(тСтрока.Команда);
			Если НЕ ЗначениеЗаполнено(График2Команда) Тогда
				График2Команда = тСтрока.Команда;
			КонецЕсли;
...
Процедура График2ОбновитьНаСервере()
	...
	КонецЕсли;
	Запрос.УстановитьПараметр("Дата",		Объект.Дата);
	Запрос.УстановитьПараметр("Игроки",		Объект.СоставыКоманд.Выгрузить(Новый Структура("Команда", График2Команда), "Игрок").ВыгрузитьКолонку(0));
	...
	КонецЕсли;
	Если График2Группировать Тогда
		ВыборкаА = Ризалт.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаА.Следующий() Цикл
			Точка	= График2.УстановитьТочку(ВыборкаА.Амплуа);
			Выборка = ВыборкаА.Выбрать();
			Для Фаза = 1 По 3 Цикл
				Выборка.Сбросить();
				КолИтого	= 0;
				ВесИтого	= 0;
				Пока Выборка.Следующий() Цикл
					КолИтого	= КолИтого + 1;
					ВесИтого	= ВесИтого + ФазаРасчитать2(Выборка.Количество, Фаза);
				КонецЦикла;
				Серия	= График2.УстановитьСерию(ФЭИ(Фаза));
				Серия.Цвет	= Цвет(Фаза);
				График2.УстановитьЗначение(Точка, Серия, ?(КолИтого = 0, 0, Окр(ВесИтого / КолИтого, 1)));
			КонецЦикла;
		КонецЦикла;
	Иначе
