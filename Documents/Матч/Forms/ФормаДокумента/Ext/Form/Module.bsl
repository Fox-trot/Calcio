...
Процедура ГрафикОбновить(Команда)
	...
	ИначеЕсли СтрСравнить(Команда.Имя, "ГрафикОбновить") = 0 Тогда
		ГрафикОбновитьНаСервере();
	ИначеЕсли СтрСравнить(Команда.Имя, "АнонсОбновить") = 0 Тогда
		АнонсОбновитьНаСервере();
	ИначеЕсли СтрСравнить(Команда.Имя, "СоставыЗаполнить") = 0 Тогда
...
Процедура СтадионНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если Объект.Команды.Количество() > 0 Тогда
		Стадион = Стадион(Объект.Команды.Получить(0).Команда);
		Если ЗначениеЗаполнено(Стадион) И Элемент.СписокВыбора.НайтиПоЗначению(Стадион) = Неопределено Тогда
