...
Процедура КомандыПриИзмененииНаСервере()
	...
	Для Каждого тСтрока Из Объект.Команды Цикл
		Если ЗначениеЗаполнено(тСтрока.Команда) Тогда
			...
			Если тСтрока.НомерСтроки = 1 И НЕ ЗначениеЗаполнено(Объект.Стадион) Тогда
		...
		ИначеЕсли тСтрока.НомерСтроки = 1 И ЗначениеЗаполнено(Объект.Стадион) Тогда
...
Процедура КомандыПриИзменении(Элемент=Неопределено)
	Если Объект.Команды.Количество() = 0 Тогда
		Если ЗначениеЗаполнено(Объект.Стадион) Тогда
...
Функция МатчиПоследние5(Тим, Сколько)
	...
	Выборка = СерверныйФункции.МатчиПоследние5(Тим, Чемпионат(), Сколько);
	Для Каждого Детали Из Выборка Цикл
		Если НачалоДня(Детали.Период) > НачалоДня(Объект.Дата) Тогда
			Виктимз.Вставить(0, Детали);
		Иначе
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Для Каждого Виктим Из Виктимз Цикл
		Выборка.Удалить(Виктим);
	КонецЦикла;
	Возврат Выборка;
КонецФункции
