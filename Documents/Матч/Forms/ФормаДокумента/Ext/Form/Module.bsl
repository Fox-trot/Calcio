...
Процедура АнонсОбновитьНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Элементы.Результативность.МаксимальноеЗначение	= Окр(?(ИгрСНачалаСезона=0, 0, Окр(КоличествоГолов / ИгрСНачалаСезона, 1)) + ?(ИгрСНачалаСезонаГости=0, 0, Окр(КоличествоГоловГости / ИгрСНачалаСезонаГости, 1)), 0);
