...
Процедура ВидимостьДоступностьУстановить(Перечитать=Ложь)
	...
	Элементы.ОтчетДербиКомандаДерби.Доступность	= (НЕ ПустаяСтрока(ЭтоДерби));
КонецПроцедуры
...
Функция СтадионНайти(Тим, Тур=Неопределено)
	Если ЗначениеЗаполнено(Чемпионат) И Чемпионат.Владелец.ОлимпийскаяСистема
	И ЗначениеЗаполнено(Тур) И Тур.ОлимпийскаяСистема
	Тогда
		//Ответ = РеквизитФормыВЗначение("Объект").СтадионНайти(Тур, Истина);
		Ответ = РеквизитФормыВЗначение("Объект").СтадионНайти(Чемпионат, Истина);
	Иначе
...
Процедура СтадионНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если Объект.Команды.Количество() > 0 Тогда
		Стадион	= 	СтадионНайти(Объект.Команды.Получить(0).Команда, Объект.Тур);
...
Процедура СоставыКомандИгрокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Игроки.ЗагрузитьЗначения(ИгрокиПолучить(Объект.Дата, КомандыПолучить()));
КонецПроцедуры
...
Процедура График2ОбновитьНаСервере()
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 30
	                      ...
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	ЕСТЬNULL(Роли.Код, Игроки.Амплуа.Код),
	                      |	Игрок
	                      |АВТОУПОРЯДОЧИВАНИЕ");
	Запрос.УстановитьПараметр("Дата",		Объект.Дата);
	Запрос.УстановитьПараметр("Ссылка",		Объект.Ссылка);
...
Процедура ЭтоДерби()
	...
	Город		= ?(ПустаяСтрока(ЭтоДерби), Неопределено, ГородНайти());
КонецПроцедуры
...
Процедура ФотоПолучить() Экспорт
	НаКлиенте.ФотоПолучить(ЭтаФорма, Элементы.СоставыКоманд.ТекущиеДанные);
КонецПроцедуры
