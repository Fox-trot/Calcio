...
Функция СоставКоманды(Команда)
	...
	Запрос.УстановитьПараметр("ДатаН", 		ДобавитьМесяц(Объект.Дата, -1));
...
&НаСервере
Функция Чемпионат()
	Если ЗначениеЗаполнено(Объект.Тур) И НЕ ЗначениеЗаполнено(Чемпионат) Тогда
		Чемпионат	= СерверныйФункции.ЧемпионатНайти(Объект.Тур);
		Если НЕ ЗначениеЗаполнено(Чемпионат) И НЕ Объект.Проведен Тогда
...
Функция ТекстЗапросаПоХронологии()
	Текст	= СтрЗаменить("ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 5
	     	              |	ИСТИНА КАК Хозяева,
	     	              ...
	     	              |УПОРЯДОЧИТЬ ПО
	     	              |	Хозяева УБЫВ,
	     	              |	Период", "5", Формат(Макс(1, КоличествоМатчейВАнонсе()), "ЧГ=0"));
...
Функция Победа(Чемпионат)
	Возврат НаСервереПовторно.Победа(Чемпионат);
КонецФункции

&НаСервереБезКонтекста
Функция ЦветТекста(Чемпионат, КоличествоОчков=0)
	Если КоличествоОчков = 0 Тогда
		Возврат	ЦветаСтиля.ЦветОтрицательногоЧисла;
	ИначеЕсли КоличествоОчков = Победа(Чемпионат) Тогда
...
Процедура МатчУстановить(Родитель, Выборка)
	...
	Иначе
		Элемент.Заголовок	= "W";
	КонецЕсли;
	Элемент.ЦветТекста	= ЦветТекста(Чемпионат, Выборка.КоличествоОчков);
...
Процедура АнонсОбновитьНаСервере2()
	...
	Для Каждого тСтрока Из Объект.Команды Цикл
		...
		Для Каждого Детали Из Выборка Цикл
			Область = Макет.ПолучитьОбласть("Анонс|Команды");
		...
		Если Итератор > 0 Тогда
			Для Каждого Детали Из Выборка Цикл
				Область = Макет.ПолучитьОбласть("Детали1|" + Детали.Результат);
				Область.Параметры.Заполнить(Детали);
				Предыдущие5.Присоединить(Область);
				Итератор = Итератор + 1;
			КонецЦикла;
			Пока Итератор <= Сколько Цикл
				Предыдущие5.Присоединить(Макет.ПолучитьОбласть("Детали1|NN"));
				Итератор = Итератор + 1;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	Предыдущие5.ФиксацияСверху	= 3;
	Предыдущие5.ФиксацияСлева	= Сколько + 2;
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция Цвет(Элемент)
	Возврат НаСервереПовторно.Цвет(Элемент);
КонецФункции
...
Процедура ГрафикОбновитьНаСервере()
	...
	Если КомандыДве(Истина) Тогда
		...
		Хозяева	= Хозяева();
		ТекСтрока	= Новый Структура("Фаза,ФазаП", 31, 30);
		Пока Выборка.Следующий() Цикл
			Точка	= График.УстановитьТочку(Выборка.Команда);
			График.УстановитьЗначение(Точка, Серия1, Окр(Выборка.Фаза1, 1));
			График.УстановитьЗначение(Точка, Серия2, Окр(Выборка.Фаза2, 1));
			График.УстановитьЗначение(Точка, Серия3, Окр(Выборка.Фаза3, 1));
			Если Объект.Проведен Или Модифицированность Тогда
				Детали	= Выборка.Выбрать();
				Пока Детали.Следующий() Цикл
					Если Выборка.Команда = Хозяева Тогда
						ТекСтрока.Фаза	= ТекСтрока.Фаза + Детали.Фаза1 + Детали.Фаза2 * 1.2 + Детали.Фаза3 * 1.1
						+ ?(Детали.Фаза1 >= 1 И Детали.Фаза2 >= 1 И Детали.Фаза3 >= 1, 1,
						+ ?(Детали.Фаза1 <= -1 И Детали.Фаза2 <= -1 И Детали.Фаза3 <= -1, -1, 0))
						+ ?(Детали.Фаза1 >= 0 И Детали.Фаза2 >= 0 И Детали.Фаза3 >= 0 И Детали.Фаза1 + Детали.Фаза2 + Детали.Фаза3 > 0, 0.34,
						+ ?(Детали.Фаза1 <= 0 И Детали.Фаза2 <= 0 И Детали.Фаза3 <= 0 И Детали.Фаза1 + Детали.Фаза2 + Детали.Фаза3 < 0, -0.34, 0));
					Иначе
						ТекСтрока.ФазаП	= ТекСтрока.ФазаП + Детали.Фаза1 + Детали.Фаза2 * 1.2 + Детали.Фаза3 * 1.1
						+ ?(Детали.Фаза1 >= 1 И Детали.Фаза2 >= 1 И Детали.Фаза3 >= 1, 1,
						+ ?(Детали.Фаза1 <= -1 И Детали.Фаза2 <= -1 И Детали.Фаза3 <= -1, -1, 0))
						+ ?(Детали.Фаза1 >= 0 И Детали.Фаза2 >= 0 И Детали.Фаза3 >= 0 И Детали.Фаза1 + Детали.Фаза2 + Детали.Фаза3 > 0, 0.34,
						+ ?(Детали.Фаза1 <= 0 И Детали.Фаза2 <= 0 И Детали.Фаза3 <= 0 И Детали.Фаза1 + Детали.Фаза2 + Детали.Фаза3 < 0, -0.34, 0));
			...
			КонецЕсли;
		КонецЦикла;
		Если НЕ Объект.Проведен И НЕ Модифицированность Тогда
			Отчет	= Отчеты.Прогноз.Создать();
			Отчет.Чемпионат = Чемпионат;
			Выборка	= Отчет.ДанныеПолучить();
			мСтроки	= Выборка.НайтиСтроки(Новый Структура("Матч", Объект.Ссылка));
			ТекСтрока = мСтроки.Получить(0);
		КонецЕсли;
		График.ОбластьЗаголовка.Текст		= СтрШаблон("%1 - %2", Окр(ТекСтрока.Фаза, 1), Окр(ТекСтрока.ФазаП, 1));
		График.ОбластьЗаголовка.ЦветТекста	= ЦветТекста(Чемпионат, ?(ТекСтрока.Фаза - ТекСтрока.ФазаП <= -5, 0, ?(ТекСтрока.Фаза - ТекСтрока.ФазаП < 5, 1, Победа(Чемпионат))));
	КонецЕсли;
КонецПроцедуры
...
Функция ГрафикПолучитьНаСервере()
	...
	Запрос.УстановитьПараметр("Команда",	КомандыПолучить());
	Запрос.УстановитьПараметр("Хозяева",	Хозяева());
	Если Объект.СоставыКоманд.Количество() = 0 Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |			ТОГДА 2
		               |		КОГДА Перемещения.Игрок.ПометкаУдаления = ЛОЖЬ
		               |			ТОГДА 1
		               |		ИНАЧЕ 0
		               |	КОНЕЦ КАК Порядок
		               |ПОМЕСТИТЬ Игроки
		               ...
		               |					ГДЕ
		               |						Составы.Ссылка.ПометкаУдаления = ЛОЖЬ
		               |						И Составы.Ссылка.Тур.Владелец = &Чемпионат
		               |					СГРУППИРОВАТЬ ПО
		               |						Составы.Команда
		               |					ИМЕЮЩИЕ
		...
		Запрос.УстановитьПараметр("Чемпионат",	Чемпионат);
	Иначе
		Запрос.УстановитьПараметр("Игроки",		Объект.СоставыКоманд.Выгрузить(, "Игрок").ВыгрузитьКолонку(0));
	КонецЕсли;
	Если ЗначениеЗаполнено(Амплуа) Тогда
		Запрос.УстановитьПараметр("Амплуа",		Амплуа);
	Иначе
		Запрос.Текст = СтрУдалить(Запрос.Текст, "И ЕСТЬNULL(Перемещения.Игрок.Амплуа.Ссылка, Игроки.Амплуа) = &Амплуа");
	КонецЕсли;
...
Процедура СоставыЗаполнить(Команда=Неопределено)
	Если Объект.Ссылка.Пустая() Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Данные ещё не записаны");
	ИначеЕсли Объект.Команды.Количество() = 0 Тогда
...
Процедура СтадионПриИзмененииНаСервере()
	Если ЗначениеЗаполнено(Объект.Тур) И НЕ Объект.Тур.ОлимпийскаяСистема Тогда
...
Процедура СтадионНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Если Стадионы.Количество() = 0 Тогда
		ПодключитьОбработчикОжидания("СтадионыЗагрузитьАсинх", 0.1, Истина);
	КонецЕсли;
	Если Элементы.Стадион.СписокВыбора.Количество() = 1
	И НЕ ЗначениеЗаполнено(Объект.Стадион)
	Тогда
		СтандартнаяОбработка	= Ложь;
		Модифицированность		= Истина;
		Объект.Стадион			= Элементы.Стадион.СписокВыбора.Получить(0).Значение;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		Если МатчиВсеЗаписаны() Тогда
			ОбщегоНазначения.СообщитьОбОшибке("Матчи все уже записаны", Отказ);
		КонецЕсли;
...
Процедура ПриОткрытииНаСервере(Отказ)
	ТурПриИзмененииНаСервере(Истина);
	Если Объект.Ссылка.Пустая() Тогда
		//
	Иначе
...
Процедура ПриОткрытии(Отказ)
	...
	ИначеЕсли НЕ Объект.Ссылка.Пустая() И НЕ АнонсВсеИгры Тогда
...
Процедура ВидимостьДоступностьУстановить(Перечитать=Ложь)
	...
	Если ЭтоДерби() Тогда
		Элементы.ОтчетДерби.Доступность	= Истина;
		Элементы.ДербиСоздать.Видимость	= Ложь;
		Элементы.ЭтоДерби.Видимость		= Истина;
	Иначе
		Элементы.ОтчетДерби.Доступность	= Ложь;
		Элементы.ДербиСоздать.Видимость	= ЭтоДербиВозможно();
		Элементы.ЭтоДерби.Видимость		= Ложь;
	КонецЕсли;
	Элементы.КоличествоЗрителей.Доступность	= (НачалоДня(Объект.Дата) <= ТекущаяДатаСеанса());
...
&НаСервере
Функция СтадионНайти(Клуб)
	Если ЗначениеЗаполнено(Объект.Тур) Тогда
		Ответ = РеквизитФормыВЗначение("Объект").СтадионНайти(Новый Структура("Команда,Тур", Клуб, Объект.Тур), Истина);
		Если Ответ.Количество() = 0 Тогда
			Ответ = СерверныйФункции.СтадионНайти(Чемпионат(), Истина);
		КонецЕсли;
	Иначе
		Ответ = РеквизитФормыВЗначение("Объект").СтадионНайти(, Истина);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Процедура ПредыдущиеВстречиОбновитьНаСервере()
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Макет = ПолучитьОбщийМакет("ТурнирнаяТаблица");
	ПредыдущиеВстречи.Вывести(Макет.ПолучитьОбласть("ВстречиШ"));
	Пока Выборка.Следующий() Цикл
		Область = Макет.ПолучитьОбласть(?(Выборка.Хозяева, "ВстречиХ", "Встречи"));
...
Процедура ПредыдущиеМатчиОбновитьНаСервере()
	...
	ИначеЕсли МатчиФильтр = 3 Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	Противники.Период КАК Дата,
		               |	МатчКоманды.Ссылка.Счет КАК Счет,
		               |	МатчКоманды.Ссылка КАК Матч,
		               |	Противники.Тур.Владелец КАК Чемпионат,
		               |	МАКСИМУМ(МатчКоманды.Команда = &Хозяева) КАК Хозяева
		               |ИЗ
		               |	Документ.Матч.Команды КАК МатчКоманды
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК Противники
		               |		ПО МатчКоманды.Ссылка = Противники.Регистратор
		               |			И МатчКоманды.НомерСтроки <> Противники.НомерСтроки
		               |ГДЕ
		               |	Противники.Период МЕЖДУ &ДатаН И &ДатаК
		               |	И Противники.Команда В(&Команды)
		               |	И (МатчКоманды.Команда.Город, Противники.Команда.Город) В
		               |			(ВЫБРАТЬ
		               |				МИНИМУМ(Города.Ссылка),
		               |				МАКСИМУМ(Города.Ссылка)
		               |			ИЗ
		               ...
		               |			ИМЕЮЩИЕ
		               |				МИНИМУМ(Города.Ссылка) = МАКСИМУМ(Города.Ссылка))
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	МатчКоманды.Ссылка,
		               |	МатчКоманды.Ссылка.Счет,
		               |	Противники.Период,
		               |	Противники.Тур.Владелец
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Дата УБЫВ";
	КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Макет = ПолучитьОбщийМакет("ТурнирнаяТаблица");
	ПредыдущиеМатчи.Вывести(Макет.ПолучитьОбласть("ВстречиШ"));
	Пока Выборка.Следующий() Цикл
		Область = Макет.ПолучитьОбласть(?(Выборка.Хозяева, "ВстречиХ", "Встречи"));
		Область.Параметры.Заполнить(Выборка);
...
&НаСервере
Процедура ТурПриИзмененииНаСервере(ЭтоСтарт=Ложь)
	Клубы = КомандыТураПолучить();
	КомандыТура.ЗагрузитьЗначения(Клубы);
	Элементы.КомандыКоманда.СписокВыбора.ЗагрузитьЗначения(Клубы);
...
&НаСервере
Процедура СтадионыЗагрузитьНаСервере(Знач Клуб=Неопределено)
	Если Клуб = Неопределено И Объект.Команды.Количество() > 0 Тогда
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Клуб) И Стадионы.Количество() > 0 Тогда
		Элементы.Стадион.СписокВыбора.Очистить();
		Итератор = 0;
		Для Каждого Элемент Из Стадионы Цикл
			Если Элемент.Значение.ПометкаУдаления Тогда
				//
			ИначеЕсли Элемент.Значение.Владелец = Клуб Тогда
				Элементы.Стадион.СписокВыбора.Добавить(Элемент.Значение);
				Прервать;
			ИначеЕсли ТипЗнч(Элемент.Значение.Владелец) = Тип("СправочникСсылка.Команды") Тогда
				Продолжить;
			ИначеЕсли Элемент.Значение.Город = Клуб.Город Тогда
				Элементы.Стадион.СписокВыбора.Добавить(Элемент.Значение);
...
&НаКлиенте
Процедура СтадионыЗагрузитьАсинх() Экспорт
	СтадионыЗагрузитьНаСервере();
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ТипЗнч(Параметр) = Тип("СправочникСсылка.Стадионы") Тогда
		ПодключитьОбработчикОжидания("СтадионыЗагрузитьАсинх", 0.1, Истина);
	ИначеЕсли СтрСравнить(ИмяСобытия, "Дерби") = 0 Тогда
		Если ЭтоДерби() Тогда
			Перечитать();
		КонецЕсли;
	КонецЕсли;
...
Процедура График2ОбновитьНаСервере()
	...
	Если График2Группировать Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ЕСТЬNULL(Перемещения.Амплуа.Ссылка, Игроки.Амплуа) КАК Точка,
		               |	Перемещения.Игрок КАК Игрок,
		               |	ОКР(ВЫБОР
		               |			КОГДА Игроки.Фаза1 <= 23 / 4
		               |				ТОГДА Игроки.Фаза1 * 25 / 23
...
Функция КомандыТураПолучить()
	Возврат РеквизитФормыВЗначение("Объект").КомандыТураПолучить(Чемпионат());
КонецФункции

&НаСервереБезКонтекста
Функция КоличествоМатчейВАнонсе()
	Возврат НаСервереПовторно.КоличествоМатчейВАнонсе();
КонецФункции
...
Функция СтарыйВидАнонсов()
	Возврат НаСервереПовторно.СтарыйВидАнонсов();
КонецФункции

&НаСервере
Функция ЭтоДерби()
	ЭтоДерби	= СерверныйФункции.ЭтоДерби(Объект.Ссылка);
	Город		= ?(ЭтоДерби, ГородНайти(), Неопределено);
	Возврат ЭтоДерби;
КонецФункции

&НаСервере
Функция ЭтоДербиВозможно()
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	СУММА(1) КАК Количество
	                      |ИЗ
	                      |	Справочник.Команды КАК Команды
	                      ...
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	Команды.Город.Владелец");
	Запрос.УстановитьПараметр("Команды",		КомандыПолучить());
	Выборка = Запрос.Выполнить().Выбрать();
	Возврат (Выборка.Следующий() И Выборка.Количество() = 1 И Выборка.Количество = 2);
КонецФункции
...
&НаКлиенте
Процедура ЧемпионатыЗагрузитьЗначения()
...
Процедура ЭтоДербиНажатие(Элемент)
	...
	Иначе
		ДанныеЗаполнения.Вставить("Команды", КомандыПолучить());
	КонецЕсли;
	
	НаКлиенте.ОбработкаКомандыОткрытьФорму("Отчет.Дерби.Форма", ДанныеЗаполнения);
КонецПроцедуры
...
&НаКлиенте
Процедура СтадионОчистка(Элемент, СтандартнаяОбработка)
	Если НЕ ЗначениеЗаполнено(Объект.Стадион) Тогда
		Если Элементы.Стадион.СписокВыбора.Количество() = 1 Тогда
			СтандартнаяОбработка = Ложь;
			Объект.Стадион = Элементы.Стадион.СписокВыбора.Получить(0).Значение;
		Иначе
			Стадионы.Очистить();
...
Процедура Импорт(Слова, ПараметрКоманды) Экспорт
	Если Слова = Неопределено Тогда
		//
	ИначеЕсли ПустаяСтрока(Слова) Тогда
		СоставыЗаполнитьНаСервере();
	ИначеЕсли СоставыИмпортНаСервере(Слова) Тогда
...
Функция СоставыИмпортНаСервере(Знач УРЛ)
	...
	КонецЕсли;
	Слова = Разделить(СтрЗаменить(УРЛ, ":", ","), ",", Истина);
		...
		Для Каждого Слово Из Слова Цикл
			...
			ИначеЕсли СтрНайти(Слово, "(") > 0 Тогда
				Для Каждого Словечко Из Разделить(Слово, "(") Цикл
					...
					Прервать;
				КонецЦикла;
			//ИначеЕсли СтрСравнить(Слово, Объект.Команды.Получить(0).Команда.Наименование) = 0 Тогда
			//	Игнор = Ложь;
			ИначеЕсли СтрСравнить(Слово, Объект.Команды.Получить(1).Команда.Наименование) = 0 Тогда
				Игнор = Ложь;
			ИначеЕсли СтрСравнить(Слово, "Запасные") = 0 Тогда
				Игнор = Истина;
			ИначеЕсли НЕ Игнор Тогда
				_Страна	= СерверныйФункции.СтранаНайти(Слово);
				Если ЗначениеЗаполнено(_Страна) Тогда
					Исключения.Добавить(НРег(Слово));
					Продолжить;
				КонецЕсли;
				ИгрокИмя = Слово;
				Если СтрокаЗаканчиваетсяЧислом(Слово) Тогда
					...
					КонецЦикла;
				КонецЕсли;
				сИгрок = ИгрокНайти(ИгрокИмя, Объект.Ссылка);
...
&НаСервереБезКонтекста
Функция ИгрокНайти(ИгрокИмя, Ссылка)
	Возврат СерверныйФункции.ИгрокНайти(ИгрокИмя, Ссылка);
КонецФункции

&НаКлиенте
Процедура СтадионСоздание(Элемент, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Хозяева()) Тогда
		НаКлиенте.ОбработкаКомандыОткрытьФорму("Справочник.Стадионы.ФормаОбъекта", Новый Структура("Владелец", Хозяева), ЭтаФорма, СтандартнаяОбработка);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция ДербиСоздатьНаСервере()
	Если Объект.Команды.Количество() = 2 Тогда
		Возврат РегистрыСведений.Дерби.Создать(Новый Структура("Команда,Противник", Хозяева(), Гости()));
	КонецЕсли;
	Возврат "Заполните таблицу 'Команды'";
КонецФункции

&НаКлиенте
Процедура ДербиСоздать(Команда)
	Нота = ДербиСоздатьНаСервере();
	Если ПустаяСтрока(Нота) Тогда
		ВидимостьДоступностьУстановить();
	Иначе
		ОбщегоНазначения.СообщитьОбОшибке(Нота,, Объект.Ссылка, ?(СтрНайти(Нота, "Команды") = 0, "", "Объект.Команды"));
	КонецЕсли;
КонецПроцедуры
