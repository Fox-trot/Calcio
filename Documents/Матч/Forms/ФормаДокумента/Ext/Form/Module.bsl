...
Процедура АнонсОбновитьНаСервере()
	...
	КонецЦикла;
	КакЕсть	= (ИгрСНачалаСезона < 10);			// недостаточно игр с начала сезона
	ИгрСНачалаСезона	= 0;
...
Процедура ЧемпионатНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Значения	= ЧемпионатыПолучить(Объект.Дата, ?(Объект.Команды.Количество() = 0, ВидСпорта, Объект.Команды.Получить(0)));
