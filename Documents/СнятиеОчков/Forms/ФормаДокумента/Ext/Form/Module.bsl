...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	НаКлиенте.Оповещение("СнятиеОчков", Чемпионат);
КонецПроцедуры
...
Процедура ЧемпионатУстановить()
	...
	Иначе
		...
		Если Чемпионаты.Количество() = 1 Тогда
...
&НаСервере
Функция КомандаНайти(Наименование)
	Возврат СерверныйФункции.КомандаНайти(Наименование, Чемпионат);
КонецФункции

&НаСервере
Процедура КомментарийПриИзмененииНаСервере()
	Если НЕ ЗначениеЗаполнено(Объект.Команда) Или Объект.КоличествоОчков = 0 Тогда
		мСлова = Разделить(Объект.Комментарий);
		Для Каждого Слово Из мСлова Цикл
			Если ЗначениеЗаполнено(Объект.Команда) И Объект.КоличествоОчков <> 0 Тогда
				Прервать;
			ИначеЕсли СтрНайти(Слово, """") > 0 Тогда
				Слово = СтрУдалить(Слово, """");
				Команда = КомандаНайти(Лев(Слово, СтрДлина(Слово) - 1));
				Если ЗначениеЗаполнено(Команда) Тогда
					Объект.Команда = Команда;
				КонецЕсли;
			ИначеЕсли Объект.КоличествоОчков = 0 Тогда
				КоличествоОчков = СтрокаКакЧисло(Слово);
				Если КоличествоОчков > 0 Тогда
					Объект.КоличествоОчков = -КоличествоОчков;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
