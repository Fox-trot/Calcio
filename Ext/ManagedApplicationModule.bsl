
Процедура ПриНачалеРаботыСистемы()
	НаКлиенте.КаталогаФайловУстановить();
	Если ОповещенияНаСегодня() Тогда
		ПодключитьОбработчикОжидания("ОповещенияПользователя", Часов());
	КонецЕсли;
	Если Синхронизировать() Тогда
		ПодключитьОбработчикОжидания("РегламентныеЗаданияЗапустить", 1, Истина);
	КонецЕсли;
КонецПроцедуры

Функция ОповещенияНаСегодня(ТолькоМатчи=Ложь)
	Ответ	= Ложь;
	События	= СерверныйФункции.ОповещенияНаСегодня(ТолькоМатчи);
	Для Каждого Событие Из События Цикл
		ПоказатьОповещениеПользователя(Событие.Ссылка, Событие.Ключ, Событие.Дата, Событие.Картинка, СтатусОповещенияПользователя.Важное, Событие.КлючУникальности);
		Если Событие.Повторить Тогда
			Ответ	= Истина;
		КонецЕсли;
	КонецЦикла;
	Если ТолькоМатчи И НЕ Ответ Тогда
		ОтключитьОбработчикОповещения("ОповещенияПользователя");
	КонецЕсли;
	Возврат Ответ;
КонецФункции

Процедура ОповещенияПользователя() Экспорт
	ОповещенияНаСегодня(Истина);
КонецПроцедуры

Функция Синхронизировать()
	Возврат (СтрНайти2(ПараметрЗапуска, "Синхронизировать") > 0);
КонецФункции

Процедура РегламентныеЗаданияЗапустить() Экспорт
	Если СерверныйФункции.ЗапуститьРегламентноеЗадание() Тогда
		ПодключитьОбработчикОжидания("ИнтерфейсОбновить", 30, Истина);
	Иначе
		ОбщегоНазначения.СообщитьОбОшибке("Не удалось запустить. Проверьте настройки регламентных заданий");
	КонецЕсли;
КонецПроцедуры

Процедура ИнтерфейсОбновить() Экспорт
	НаКлиенте.Оповещение("Матчи");
	НаКлиенте.Оповещение("ЧтениеНовостей");
КонецПроцедуры
