...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	...
	Если ТипЗнч(Параметр) = Тип("Структура") Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	Туры.ОлимпийскаяСистема = ИСТИНА
		               |		И МАКСИМУМ(Финал.Код) = Туры.Код КАК ЭтоФинал,
		               ...
		               |			ПО Города.Владелец = ВременнаяТаблица.Страна
		               |		ПО Стадионы.Город = Города.Ссылка
		               |ГДЕ
		               |	ВременнаяТаблица.ЭтоФинал = ЛОЖЬ
...
Функция ЧемпионатПолучитьПоследнее(Команды=Неопределено, ОлимпийскаяСистема=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	И ТурнирнаяТаблица.Тур.ОлимпийскаяСистема = &Система
	                      |
	                      |УПОРЯДОЧИТЬ ПО
...
Функция ЧемпионатыТекущие(ОлимпийскаяСистема=Ложь) Экспорт
	...
	Если ОлимпийскаяСистема Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	Справочник.Туры КАК Туры
		               |ГДЕ
		               |	Туры.Владелец.ДатаОкончания >= &Дата";
	КонецЕсли;
...
Функция ЭтотМатчТурФинал(Параметр) Экспорт
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               |	ИСТИНА КАК ЭтоФинал,
	      	               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Команда) КАК ф,
	      	               ...
	      	               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Команда) = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЧемпионатыКоманды.Команда) И
	      	               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Команда) >= 1");
...
Функция ТурНеПоследний(Чемпионат)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	                      |			ЧемпионатыКоманды.Ссылка КАК Ссылка,
	                      |			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЧемпионатыКоманды.Команда) * 2 - 2 КАК Количество
	                      |		ИЗ
	                      |			Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
	                      |		ГДЕ
	                      |			ЧемпионатыКоманды.Ссылка = &Чемпионат
	                      ...
	                      |		ГДЕ
	                      |			ТурнирнаяТаблица.Тур.Владелец = &Чемпионат
	                      |			И ТИПЗНАЧЕНИЯ(ТурнирнаяТаблица.Регистратор) = ТИП(Документ.Матч)) КАК ТурнирнаяТаблица
	                      |		ПО Туры.Код = ТурнирнаяТаблица.Код
	                      |ГДЕ
	                      |	Туры.Владелец = &Чемпионат
	                      |	И Туры.Код < ЧемпионатыКоманды.Количество");
	Запрос.УстановитьПараметр("Чемпионат",	Чемпионат);
	Выборка = Запрос.Выполнить().Выбрать();
...
Функция ТурнирнаяТаблица(Чемпионат, НомерСтроки=0) Экспорт
	...
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Чемпионаты КАК Чемпионаты
	|				ПО
	|					ЛигаКоманды.Ссылка = Чемпионаты.Владелец
	|		ГДЕ
	|			Чемпионаты.Ссылка = &Чемпионат) УБЫВ,
	|	Команда
	|АВТОУПОРЯДОЧИВАНИЕ");
	Запрос.УстановитьПараметр("Чемпионат",		Чемпионат);
	Запрос.УстановитьПараметр("Команда",		МояКомандаПолучить());
...
Функция ИгрокНайти(Наименование, Параметр=Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Параметр) = Тип("Дата") Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	Справочник.Игроки КАК Игроки
		               |ГДЕ
		               |	ГОД(Игроки.ДатаРождения) = ГОД(&ДатаРождения)
		               |	И (Игроки.Наименование = &Наименование
		               |			ИЛИ Игроки.Комментарий ПОДОБНО &Комментарий)
		               |
		               |УПОРЯДОЧИТЬ ПО
		               ...
		               |		ИНАЧЕ 2
		               |	КОНЕЦ,
		               |	Игроки.ДатаРождения = &ДатаРождения УБЫВ,
		               |	Игроки.ПометкаУдаления";
	...
	Иначе
		Слова	= Разделить(СтрЗаменить(Наименование, "-", " "));
		Если Слова.Количество() > 1 Тогда
			...
			Тогда
				Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
				               ...
				               |ГДЕ
				               |	ПеремещенияИгроков.Команда В(&Команда)
				               |	И Игроки.Наименование ПОДОБНО &Слово3
				               |	И Игроки.Наименование ПОДОБНО &Слово4
				               |	И (Игроки.Наименование ПОДОБНО &Слово1
				               |			ИЛИ Игроки.Наименование ПОДОБНО &Слово2)
				               |
				               |УПОРЯДОЧИТЬ ПО
				               |	Игроки.ПометкаУдаления,
				               |	Игроки.Наименование ПОДОБНО &Слово1
				               |		И Игроки.Наименование ПОДОБНО &Слово2 УБЫВ,
				               |	ПеремещенияИгроков.Период УБЫВ";
			Иначе
				Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
				               |	Игроки.Ссылка КАК Игрок,
				               ...
				               |ИЗ
				               |	Справочник.Игроки КАК Игроки
				               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПеремещенияИгроков.СрезПоследних(, ГОД(Игрок.ДатаРождения) > 1900) КАК ПеремещенияИгроков
				               |		ПО Игроки.Ссылка = ПеремещенияИгроков.Игрок
				               |ГДЕ
				               |	Игроки.Наименование ПОДОБНО &Слово3
				               |	И Игроки.Наименование ПОДОБНО &Слово4
				               |	И (Игроки.Наименование ПОДОБНО &Слово1
				               |			ИЛИ Игроки.Наименование ПОДОБНО &Слово2)
				               |
				               ...
				               |	Игроки.Наименование ПОДОБНО &Слово1
				               |		И Игроки.Наименование ПОДОБНО &Слово2 УБЫВ,
				               |	ГОД(Игроки.ДатаРождения) УБЫВ";
			КонецЕсли;
			Запрос.УстановитьПараметр("Слово1",		ЛевоеПравое(Слова.Получить(0)));
			Запрос.УстановитьПараметр("Слово2",		ЛевоеПравое(Слова.Получить(0), Истина));
			Если Слова.Количество() = 2 Тогда
				Запрос.УстановитьПараметр("Слово3",		ЛевоеПравое(Слова.Получить(1)));
				Запрос.УстановитьПараметр("Слово4",		ЛевоеПравое(Слова.Получить(1), Истина));
			Иначе
				Запрос.УстановитьПараметр("Слово3",		Обернуть(Слова.Получить(1)));
				Запрос.УстановитьПараметр("Слово4",		Обернуть(Слова.Получить(2)));
			КонецЕсли;
			Выборка	= Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				ЗаполнитьЗначенияСвойств(Ответ, Выборка);
...
Функция ЛевоеПравое(Слово, Правое=Ложь)
	Возврат Обернуть(?(СтрДлина(Слово) < 5, Слово, ?(Правое, Прав(Слово, Окр(СтрДлина(Слово) / 2)), Лев(Слово, Окр(СтрДлина(Слово) / 2)))));
КонецФункции
...
Функция ИмпортЧемпионата(Гиперссылка, Чемпионат) Экспорт
	Итого	= 0;
	Игры	= Цел(Чемпионат.Команды.Количество() / 2);
	Предел	= Чемпионат.Команды.Количество() * 2 - 2;
	Год		= Год(Чемпионат.ДатаОкончания);
	Справочники.Туры.Заполнить(Чемпионат);

	Если СтрНайти2(Гиперссылка, "games") = 0 Тогда
		Для Итератор = 1 По Предел Цикл
			...
			Если ЗначениеЗаполнено(Тур) Тогда
				...
				Если МатчиТура.Количество() = Игры Тогда
					Продолжить;
				ИначеЕсли ИмпортИгрПоГиперссылке(СтрШаблон(Гиперссылка + ?(Год > 2010, "%1/", "%1.htm"), ФорматЧГ(Итератор)), Тур) Тогда
					Итого	= Итого + 1;
					Попытка
...
Функция МатчНайти(Команды, Дата, Тур=Неопределено) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      ...
	                      |		КОГДА Матч.Тур = &Тур
	                      |			ТОГДА -1
	                      |		КОГДА НАЧАЛОПЕРИОДА(Матч.Дата, ДЕНЬ) <= &Дата
	                      |			ТОГДА 0
	                      ...
	                      |ГДЕ
	                      |	(Матч.Тур = &Тур
	                      |			ИЛИ Матч.Дата МЕЖДУ &Дата1 И &Дата2)
	...
	Запрос.УстановитьПараметр("Тур",		?(ЗначениеЗаполнено(Тур), Тур, Справочники.Туры.ПустаяСсылка()));
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Ссылка;
	КонецЕсли;
...
Функция СтруктураКакКоманда(Команда, Гол)
	Возврат Новый Структура("Команда,КоличествоГолов", Команда, Гол);
КонецФункции

Функция ИмпортИгрПоГиперссылке(Гиперссылка, Тур) Экспорт
	Если Год(Тур.ДатаОкончания) <= 2010 Тогда Возврат ИмпортИгрПоГиперссылке2010(Гиперссылка, Тур); КонецЕсли;
	...
	Для Каждого Слово Из Слова Цикл
		Если Итого > 0 И СтрНайти2(Слово, "таблица") > 0 Тогда
			Прервать;
		...
		ИначеЕсли Год(сИгра.Дата) < 2 Тогда
			Если СтрокаНачинаетсяЧислом(Слово) Тогда
				...
				Если Год(сИгра.Дата) < 2 Тогда
					сИгра.Тур = ТурПолучить(Слово, Параметры.Чемпионат, Параметры.Тур);
...
Функция ИмпортНаполнитель(Тур)
	Возврат Новый Структура("Тур,Дата,Счет,Гол1,Гол2,КоличествоЗрителей,Проведен,Комментарий,Команда1,Команда2,Стадион", Тур, Дата(1,1,1), "", -1, -1,0, Истина, "");
КонецФункции
	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгра.Команда1) И НЕ ЗначениеЗаполнено(сИгра.Команда2) Тогда
			...
			Если мСлова.Количество() >= 2 Тогда
				...
				ИначеЕсли ЗначениеЗаполнено(сИгра.Команда1) И ЗначениеЗаполнено(сИгра.Команда2) Тогда
					сИгра.Проведен	= Ложь;
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгра.Команда2) И ЗначениеЗаполнено(сИгра.Команда1) Тогда
			сИгра.Команда2	= КомандаНайти(Слово, Тур);
		...
		Тогда
			...
			КонецЕсли;
			Если Док.Команды.Количество() = 0 Тогда
				ЗаполнитьЗначенияСвойств(Док.Команды.Добавить(), СтруктураКакКоманда(сИгра.Команда1, Макс(0, сИгра.Гол1)));
				ЗаполнитьЗначенияСвойств(Док.Команды.Добавить(), СтруктураКакКоманда(сИгра.Команда2, Макс(0, сИгра.Гол2)));
			КонецЕсли;
			...
			Если Стадионы.Количество() >= 1 Тогда
				...
				//КонецЕсли;
			КонецЕсли;
			Если Док.Модифицированность() И ?(сИгра.Проведен, Док.ПроверитьЗаполнение(), Истина) Тогда
				Ошибка = МатчЗаписать(Док, сИгра.Проведен);
				Если ПустаяСтрока(Ошибка) Тогда
					Итого = Итого + 1;
				Иначе
					Сообщить(Ошибка);
					Логировать("СерверныйФункции.ИмпортИгрПоСтроке", Ошибка, Текст);
					Прервать;
				КонецЕсли;
			КонецЕсли;
			Зачистить(сИгра);
...
Процедура Зачистить(Виктим, ЭтоСобытие=Ложь, ОставитьДату=Ложь)
	Наполнитель = Новый Структура("ЭтоСобытие,Счет,Гол1,Гол2,КоличествоЗрителей,Проведен,Команды,Комментарий,Комментарий2,ТурКод,Стадион,Команда1,Команда2", ЭтоСобытие, "", -1, -1, 0, Истина, Новый Массив, "", "", 0);
	Если НЕ ОставитьДату Тогда
		Наполнитель.Вставить("Дата", Дата(1,1,1));
	КонецЕсли;
	ЗаполнитьЗначенияСвойств(Виктим, Наполнитель);
КонецПроцедуры

Функция ГолыОпределить(сИгра, Слова)
...
Функция ТурПолучить(Слово, Чемпионат, Тур)
	...
	И СтрНайти2(Слова.Получить(1), "тур") > 0
...
Функция КомментарийКакУРЛ(Комментарий) Экспорт
	...
	Для Строка = 1 По СтрЧислоСтрок(Комментарий) Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Возврат ?(СтрЧислоВхождений(Ответ, "%") = 2, СтрЗаменить(Ответ, "%1", ФорматЧГ(ТекущаяДатаСеанса())), Ответ);
КонецФункции
...
Функция ТурПодобрать(Матч)
	...
	Запрос.УстановитьПараметр("Комментарий",	"%" + Матч.Комментарий2 + "%");
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Ссылка;
	КонецЕсли;
