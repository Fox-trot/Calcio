...
Процедура ПервоначальноеЗаполнение() Экспорт
	...
	Для Каждого ТекЭлемент Из СписокШиндлера Цикл
		Если НайтиПоНаименованию(ТекЭлемент, "Города").Пустая() Тогда
	...
	Для Каждого ТекЭлемент Из СписокШиндлера Цикл
		Если НайтиПоНаименованию(ТекЭлемент, "Лига").Пустая() Тогда
	...
	Для Каждого ТекЭлемент Из СписокШиндлера Цикл
		Если КомандаНайти(ТекЭлемент.Наименование).Пустая() Тогда
			КомандыСоздать(ТекЭлемент.Наименование, ТекЭлемент.Город);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция КомандыТура(Тур) Экспорт
	...
	Если Тур.ОлимпийскаяСистема Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		ЛЕВОЕ СОЕДИНЕНИЕ Текущий КАК Текущий
		               |		ПО ЧемпионатыКоманды.Команда = Текущий.Команда
		               |ГДЕ
		               |	Туры1.Ссылка = &Тур
		               |	И ВЫБОР
		               ...
		               |					ИЛИ НЕ Текущий.Команда ЕСТЬ NULL
		               |		КОНЕЦ
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Команда
