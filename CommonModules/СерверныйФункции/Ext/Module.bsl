...
Функция ЭтоДерби(Матч) Экспорт
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ
	|	СУММА(МатчКоманды.НомерСтроки) + 10 * КОЛИЧЕСТВО(РАЗЛИЧНЫЕ МатчКоманды.Команда) + 100 * КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Команды.Город) КАК Количество
	|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Возврат (Выборка.Следующий() И Выборка.Количество = 123);
КонецФункции
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				...
				ИначеЕсли Выборка.Сдвиг <= КонецДня(Дата + Сутки()) + Часов(1.6) Тогда
					Событие.Дата		= "Завтра,ну почти " + Формат(Выборка.Сдвиг, "ДФ='d MMM HH:mm'");
				Иначе
