...
Функция ТурнирнаяТаблица(Чемпионат, НомерСтроки=0) Экспорт
	...
	Если НаСервереПовторно.ЧемпионатПустой(Чемпионат) Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ЧемпионатыКоманды.Команда КАК Команда,
		               |	0 КАК КоличествоИгр,
		               |	0 КАК КоличествоЗабито,
		               |	0 КАК КоличествоОчков,
		               |	0 КАК Выигрыш,
		               |	0 КАК Ничья,
		               |	0 КАК Проигрыш,
		               |	0 КАК КоличествоПропущено,
		               |	0 КАК Разница
		               |ИЗ
		               |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
		               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Лига.Команды КАК ЛигаКоманды
		               |		ПО ЧемпионатыКоманды.Команда = ЛигаКоманды.Команда
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Итоги КАК Итоги
		               |		ПО ЧемпионатыКоманды.Команда = Итоги.Команда
		               |			И (ГОД(ЧемпионатыКоманды.Ссылка.ДатаОкончания) = ГОД(Итоги.Чемпионат.ДатаОкончания) + 1)
		               |			И ЧемпионатыКоманды.Ссылка.Владелец = Итоги.Чемпионат.Владелец
		               |ГДЕ
		               |	ЧемпионатыКоманды.Ссылка = &Чемпионат
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	ЕСТЬNULL(Итоги.Место, 99),
		               |	ЧемпионатыКоманды.Команда = &Команда УБЫВ,
		               |	ЛигаКоманды.Команда ЕСТЬ NULL,
		               |	Команда
		               |АВТОУПОРЯДОЧИВАНИЕ";
			...
			Для Каждого Слово Из Разделить(СтрЗаменить(Слова, " : ", ":")) Цикл
				Если СтрНайти(Слово, ":") > 0 Тогда
					...
					ИначеЕсли Матч.Клубы.Количество() = 2 Тогда
						Цифры		= Разделить(СтрУдалить(Слово, "ДВ"), ":");
						Итератор	= 0;
