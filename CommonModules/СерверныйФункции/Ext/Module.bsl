	...
	Пока Выборка.Следующий() Цикл
		Если ?(Авто, Старье.Найти(Выборка.Ссылка) = Неопределено, Истина) Тогда
			...
			Если НЕ Выборка.ЭтоИгра Тогда
				Туча.Дата		= СтрШаблон("%1 (%2)", Формат(Выборка.Дата, "ДЛФ=D"), Возраст(Выборка.Дата));
				Туча.Картинка	= ИзображениеПолучить(Выборка.Ссылка, Истина);
			ИначеЕсли Выборка.Дата = Дата Или Выборка.Дата > ТекущаяДатаСеанса() Тогда
				Туча.Дата		= Формат(Выборка.Дата, ?(Дата=НачалоДня(Выборка.Дата), "ДФ=HH:mm", "ДФ='d MMMM'"));
			Иначе
				Продолжить;
			КонецЕсли;
			Ответ.Добавить(Туча);
