...
Функция НайтиПоНаименованию(Наименование, СправочникИмя="ВидыСпорта")
	Запрос = Новый Запрос(СтрЗаменить("ВЫБРАТЬ ПЕРВЫЕ 1
	                                  ...
	                                  |
	                                  |УПОРЯДОЧИТЬ ПО
	                                  |	Справочник.ПометкаУдаления", "ВидыСпорта", СправочникИмя));
...
Процедура ПервоначальноеЗаполнение() Экспорт
	...
	Для Каждого ТекЭлемент Из СписокШиндлера Цикл
		Если Амплуа(ТекЭлемент, НаСервереПовторно.Футбол()).Пустая() Тогда
			Объект = Справочники.Амплуа.СоздатьЭлемент();
			Объект.Наименование	= ТекЭлемент;
			Объект.Владелец		= НаСервереПовторно.Футбол();
	...
	Для Каждого ТекЭлемент Из СписокШиндлера Цикл
		...
		Если Команда.Пустая() Тогда
			Команда = КомандыСоздать(ТекЭлемент.Наименование, ТекЭлемент.Город, НаСервереПовторно.Футбол());
...
Функция КомандыСоздать(Наименование, Город="", ВидыСпорта=Неопределено) Экспорт
	...
	Объект.Владелец		= ?(ВидыСпорта=Неопределено, НаСервереПовторно.Футбол(), ВидыСпорта);
...
Функция ДанныеИгрока(Игрок, Дата=Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Игрок) Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |	ВЫБОР
		                      |		КОГДА Перемещения.Команда.Ссылка ЕСТЬ NULL
		                      |			ТОГДА 0
		                      |		ИНАЧЕ Перемещения.Номер
		                      |	КОНЕЦ КАК Номер
		                      |ИЗ
...
Функция КомандыСтраны(Параметр) Экспорт
	...
	Если ТипЗнч(Параметр) = Тип("СправочникСсылка.Лига") И ЗначениеЗаполнено(Параметр.Страна) Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |ГДЕ
		               |	ТурнирнаяТаблица.Тур.Владелец.Владелец.Страна В
		               |			(ВЫБРАТЬ
		               |				Лига.Страна
		               |			ИЗ
...
Функция КомандыТура(Тур, Все=Ложь) Экспорт
	...
	Если Тур.ОлимпийскаяСистема Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |	ЧемпионатыКоманды.Команда КАК Команда,
		               |	ЧемпионатыКоманды.НомерСтроки КАК НомерСтроки
		               |ИЗ
		               |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
		               ...
		               |			И ЧемпионатыКоманды.Команда = ТурнирнаяТаблица.Команда
		               |ГДЕ
		               |	ЕСТЬNULL(ТурнирнаяТаблица.Тур.Код, 1) >= Туры.Код
		               |	И ЕСТЬNULL(ТурнирнаяТаблица.КоличествоОчков, 1) >= Туры.Коэффициент
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ЧемпионатыКоманды.Команда,
		               |	ЧемпионатыКоманды.НомерСтроки
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	НомерСтроки";
	ИначеЕсли Все Тогда
...
Функция КомандаНайти(Наименование, Владелец=Неопределено) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	ЕСТЬNULL(СУММА(ТурнирнаяТаблица.НомерСтроки), 0) КАК НомерСтроки
	                      |ИЗ
	                      |	Справочник.Команды КАК Команды
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	                      ...
	                      |	НомерСтроки УБЫВ");
	Запрос.УстановитьПараметр("ВидСпорта",		?(ЗначениеЗаполнено(Владелец), Владелец, НаСервереПовторно.Футбол()));
	Запрос.УстановитьПараметр("Владелец",		Владелец);
	Если ТипЗнч(Владелец) = Тип("СправочникСсылка.Чемпионаты") Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               |	ЧемпионатыКоманды.Команда КАК Ссылка
		               |ИЗ
		               |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
		               |ГДЕ
		               |	ЧемпионатыКоманды.Ссылка = &Владелец
		               |	И (ЧемпионатыКоманды.Команда.Наименование = &Наименование
		               |			ИЛИ ЧемпионатыКоманды.Команда.Комментарий ПОДОБНО &Комментарий)
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	ЧемпионатыКоманды.Команда.Наименование = &Наименование УБЫВ,
		               |	ЧемпионатыКоманды.Команда.ПометкаУдаления";
	...
	ИначеЕсли ТипЗнч(Владелец) = Тип("СправочникСсылка.Страны") Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	Справочник.Команды КАК Команды
		               |ГДЕ
		               |	Команды.Город.Владелец = &Владелец
		               |	И (Команды.Наименование = &Наименование
		               |			ИЛИ Команды.Комментарий ПОДОБНО &Комментарий)
		               |
		               |УПОРЯДОЧИТЬ ПО
	...
	ИначеЕсли ТипЗнч(Владелец) = Тип("Строка") Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |			ИЛИ Команды.Наименование ПОДОБНО &Комментарий
		               |			ИЛИ Команды.Комментарий ПОДОБНО &Комментарий)
		               |	И (Команды.Город.Владелец.Наименование ПОДОБНО &Владелец
		               |			ИЛИ Команды.Город.Владелец.Комментарий ПОДОБНО &Владелец)
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Команды.Наименование = &Наименование УБЫВ,
		               |	Команды.ПометкаУдаления";
		Запрос.УстановитьПараметр("Владелец",		Обернуть(Владелец));
	КонецЕсли;
...
Функция ГородНайти(Наименование, Страна=Неопределено) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 2
	                      ...
	                      |			ИЛИ Города.Наименование ПОДОБНО &Комментарий
	                      |			ИЛИ Города.Комментарий ПОДОБНО &Комментарий)
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Команды.Город ЕСТЬ NULL,
	                      |	ВЫБОР
	                      |		КОГДА Города.Наименование = &Наименование
	                      |			ТОГДА 3
	                      |		КОГДА Города.Наименование ПОДОБНО &Комментарий
	                      |			ТОГДА 2
	                      |		ИНАЧЕ 0
	                      |	КОНЕЦ УБЫВ,
	                      |	Города.ПометкаУдаления");
	Если ТипЗнч(Наименование) = Тип("Массив") И ЗначениеЗаполнено(Страна) Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	И Команды.Город.Владелец = &Страна
		               |
		               |УПОРЯДОЧИТЬ ПО
...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |		ПО Стадионы.Ссылка = Матч.Стадион
	                      |			И (Матч.Проведен = ИСТИНА)
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Команды КАК Клубы
	                      |		ПО (Стадионы.Владелец = Клубы.Ссылка
	                      |				ИЛИ Стадионы.Город = Клубы.Город)
	                      |ГДЕ
	                      |	(Стадионы.Ссылка = &Параметр
	                      |			ИЛИ Стадионы.Владелец = &Параметр
	                      |			ИЛИ Стадионы.Наименование = &Наименование
	                      |			ИЛИ Стадионы.Комментарий ПОДОБНО &Комментарий
	                      |			ИЛИ Клубы.Наименование = &Наименование
	                      |			ИЛИ Стадионы.Город = &Параметр
	                      |			ИЛИ Стадионы.Город.Наименование = &Наименование)
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	Стадионы.Ссылка
	                      |
	                      ...
	                      |				ТОГДА 0
	                      |			КОГДА Стадионы.Владелец = Клубы.Ссылка
	                      |				ТОГДА 0
	                      |			КОГДА Клубы.Ссылка = &Параметр
	                      |				ТОГДА 0
	                      |			КОГДА Клубы.Город = &Параметр
	                      |				ТОГДА 0
	                      |			КОГДА Стадионы.Город = Клубы.Город
	                      |				ТОГДА 1
	                      |			КОГДА Стадионы.Наименование = &Наименование
	                      |				ТОГДА 1
	                      |			КОГДА Стадионы.Город.Наименование = &Наименование
	                      |				ТОГДА 2
	                      |			КОГДА Клубы.Наименование = &Наименование
	                      |				ТОГДА 2
	                      |			КОГДА Клубы.Город.Наименование = &Наименование
	                      |				ТОГДА 2
	                      |			КОГДА Стадионы.Комментарий ПОДОБНО &Комментарий
	                      |				ТОГДА 3
	                      |			ИНАЧЕ 9
	                      |		КОНЕЦ + ВЫБОР
	                      |			КОГДА Стадионы.Ссылка = Матч.Стадион
	                      |				ТОГДА -9
	...
	Если ТипЗнч(Параметр) = Тип("Структура") Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	Туры.Владелец.Владелец.Страна.Ссылка КАК Страна
		               |ПОМЕСТИТЬ ВТ
		               |ИЗ
		               |	Справочник.Туры КАК Туры
		               ...
		               |		ГДЕ
		               |			Туры.Ссылка = &Тур
		               |			И Туры.Владелец.Владелец.ОлимпийскаяСистема = ИСТИНА) КАК Финал
		               |		ПО Туры.Владелец = Финал.Владелец
		               |ГДЕ
		               |	Туры.ПометкаУдаления = ЛОЖЬ
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	Туры.Владелец.Владелец.Страна.Ссылка
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
		               ...
		               |				ТОГДА 4
		               |			КОГДА Команды.Ссылка = &Команда
		               |				ТОГДА 3
		               |			КОГДА Города.Ссылка = &Город
		               |				ТОГДА 2
		               |			КОГДА Города.Владелец = &Страна
		               |				ТОГДА 1
		               |			ИНАЧЕ 0
		               |		КОНЕЦ + ВЫБОР
		               |			КОГДА Стадионы.ПометкаУдаления = ЛОЖЬ
		               ...
		               |			ИНАЧЕ 0
		               |		КОНЕЦ + ВЫБОР
		               |			КОГДА Стадионы.Ссылка В
		               |					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |						Матч.Стадион
		               |					ИЗ
		               |						Документ.Матч КАК Матч
		               |					ГДЕ
		               |						Матч.ПометкаУдаления = ЛОЖЬ
		               ...
		               |	ГДЕ
		               |		ЕСТЬNULL(ВременнаяТаблица.ЭтоФинал, ЛОЖЬ) = ЛОЖЬ
		               |		И ВЫБОР
		               ...
		               |		ВЫБОР
		               |			КОГДА Стадионы.Наименование = &Наименование
		               |				ТОГДА 555 * Стадионы.КоличествоЗрителей
		               |			КОГДА Стадионы.Комментарий ПОДОБНО &Комментарий
		               |				ТОГДА 111 * Стадионы.КоличествоЗрителей
		               |			КОГДА Стадионы.ПометкаУдаления = ЛОЖЬ
		               |				ТОГДА Стадионы.КоличествоЗрителей
		               |			ИНАЧЕ 0
		               |		КОНЕЦ
		               |	ИЗ
		               |		Справочник.Стадионы КАК Стадионы
		               ...
		               |				Справочник.Стадионы КАК Стадионы
		               |			ГДЕ
		               |				Стадионы.КоличествоЗрителей >= ВЫБОР
		               |						КОГДА Стадионы.Город = &Город
		               |							ТОГДА 2022
		               |						КОГДА Стадионы.Город.Владелец = &Страна
		               |							ТОГДА 4044
		               |						ИНАЧЕ 8088
		               |					КОНЕЦ
		               |			
		               ...
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	МАКСИМУМ(Стадионы.Порядок) УБЫВ";
		Запрос.УстановитьПараметр("Команда",			Неопределено);
		Запрос.УстановитьПараметр("Тур",				Неопределено);
		Запрос.УстановитьПараметр("Город",				Неопределено);
		Запрос.УстановитьПараметр("Страна",				Неопределено);
		Запрос.УстановитьПараметр("Наименование",		"");
		Запрос.УстановитьПараметр("Комментарий",		"");
		ЗаполнитьЗначенияСвойств(Запрос.Параметры, Параметр);
		Если НЕ ПустаяСтрока(Запрос.Параметры.Наименование) Тогда
			Запрос.УстановитьПараметр("Наименование",		СокрЛП(Параметр.Наименование));
			Запрос.УстановитьПараметр("Комментарий",		Обернуть(Параметр.Наименование));
		КонецЕсли;
	Иначе
		Если ТипЗнч(Параметр) = Тип("СправочникСсылка.Чемпионаты") И Все Тогда
			Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			               |	Стадионы.Ссылка КАК Ссылка,
			               |	ВЫБОР
			               |		КОГДА Стадионы.ПометкаУдаления = ЛОЖЬ
		...
		ИначеЕсли ТипЗнч(Параметр) = Тип("СправочникСсылка.Туры") И Все Тогда
			Если ЗначениеЗаполнено(Параметр.Владелец.Владелец.Страна) Тогда
				Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
				               |	Стадионы.Ссылка КАК Ссылка,
				               |	ВЫБОР
				               |		КОГДА Стадионы.ПометкаУдаления = ЛОЖЬ
			...
			Иначе
				Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
				               ...
				               |		ПО Стадионы.Город = Команды.Город
				               |ГДЕ
				               |	ЧемпионатыКоманды.Ссылка.Владелец В
				               |			(ВЫБРАТЬ РАЗЛИЧНЫЕ
				               |				Туры.Владелец.Владелец
				               |			ИЗ
				               |				Справочник.Туры КАК Туры
		...
		//	               |УПОРЯДОЧИТЬ ПО
		//	               |	Порядок";
		ИначеЕсли Параметр = Неопределено И Все Тогда
			Запрос.Текст = "ВЫБРАТЬ
			               |	Стадионы.Ссылка КАК Ссылка
			               |ИЗ
			               |	Справочник.Стадионы КАК Стадионы
			               |ГДЕ
			               |	Стадионы.КоличествоЗрителей >= 1
			               |
			               |УПОРЯДОЧИТЬ ПО
			               |	Стадионы.ПометкаУдаления";
		КонецЕсли;
		Запрос.УстановитьПараметр("Параметр",			Параметр);
		Запрос.УстановитьПараметр("Наименование",		СокрЛП(Параметр));
		Запрос.УстановитьПараметр("Комментарий",		Обернуть(Параметр));
	КонецЕсли;
	Если СтрДлина(СокрЛП(Запрос.Параметры.Комментарий)) < 5 Тогда
...
Функция Чемпион(Чемпионат) Экспорт
	Если НачалоМесяца(Чемпионат.ДатаОкончания) > ТекущаяДатаСеанса() Тогда Возврат Справочники.Команды.ПустаяСсылка(); КонецЕсли;
	Запрос = Новый Запрос("ВЫБРАТЬ
...
Функция ЧемпионатНайти(Параметр, Дата=Неопределено) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	И ЧемпионатыКоманды.Команда В(&Параметр)
	                      |
	                      |ОБЪЕДИНИТЬ
	                      |
	                      |ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	1
	                      |ИЗ
	                      |	РегистрНакопления.ТурнирнаяТаблица.Обороты(&ДатаН, &ДатаК, , Команда В (&Параметр)) КАК ТурнирнаяТаблица
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Туры КАК Туры
	                      |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Чемпионаты КАК Чемпионаты
	...
	Если ТипЗнч(Параметр) = Тип("СправочникСсылка.Лига") Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	1
		               |ИЗ
		               |	РегистрНакопления.ТурнирнаяТаблица.Обороты(&ДатаН, &ДатаК, , Тур.Владелец.Владелец = &Параметр) КАК ТурнирнаяТаблица
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Порядок";
	КонецЕсли;
...
Функция ТекущийГод(Команда) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      |	ГОД(Чемпионаты.ДатаОкончания) КАК Дата
	                      |ИЗ
	                      |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Чемпионаты КАК Чемпионаты
	                      |		ПО ЧемпионатыКоманды.Ссылка = Чемпионаты.Ссылка
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат ФорматЧГ(Выборка.Дата);
	КонецЕсли;
	Возврат ФорматЧГ(ТекущаяДатаСеанса());
КонецФункции

Функция ЧемпионатыАктивные(Все=Ложь) Экспорт
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               ...
	      	               |			ПО Туры.Владелец = Итоги.Чемпионат
	      	               ...
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	МАКСИМУМ(МатчКоманды.Команда = &Команда) УБЫВ,
	      	               |	Ссылка
	      	               |АВТОУПОРЯДОЧИВАНИЕ");
	Если Все Тогда
		Запрос.Текст = "ВЫБРАТЬ
	      	               ...
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	МАКСИМУМ(МатчКоманды.Команда = &Команда) УБЫВ,
	      	               |	МАКСИМУМ(Туры.ОлимпийскаяСистема),
	      	               |	Ссылка
	      	               |АВТОУПОРЯДОЧИВАНИЕ";
	КонецЕсли;
...
Функция ЧемпионатыКоманды(Команда, ОлимпийскаяСистема=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      |	ТурнирнаяТаблица.Тур.Владелец КАК Ссылка,
	                      |	ТурнирнаяТаблица.Тур.Владелец.ДатаОкончания КАК Дата
	                      |ИЗ
	                      |	РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	                      |ГДЕ
	                      |	ТурнирнаяТаблица.Команда В(&Команда)
	                      |	И ТурнирнаяТаблица.Тур.Владелец.Владелец.ОлимпийскаяСистема = ЛОЖЬ
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Дата УБЫВ");
	Запрос.УстановитьПараметр("Команда",	?(ЗначениеЗаполнено(Команда), Команда, МояКомандаПолучить()));
...
Функция ЧемпионатыСтраны(Страна, ОлимпийскаяСистема=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	Чемпионаты.Ссылка КАК Ссылка
	                      |ИЗ
	                      |	Справочник.Чемпионаты КАК Чемпионаты
	                      |ГДЕ
	                      |	Чемпионаты.Владелец.Страна = &Страна
						  //|	И Чемпионаты.Владелец.ОлимпийскаяСистема = ЛОЖЬ
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Чемпионаты.ДатаОкончания УБЫВ");
	Запрос.УстановитьПараметр("Страна",	?(ЗначениеЗаполнено(Страна), Страна, МояКомандаПолучить().Город.Владелец));
	Если ОлимпийскаяСистема Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	Чемпионаты.Ссылка КАК Ссылка
		               |ИЗ
		               ...
		               |ГДЕ
		               |	Чемпионаты.Владелец.Страна = &Страна
		               |	И Чемпионаты.Владелец.ОлимпийскаяСистема = ИСТИНА
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Чемпионаты.ДатаОкончания УБЫВ";
	КонецЕсли;
...
Функция ЧемпионатыТекущие(Все=Ложь) Экспорт
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	      	               |	Чемпионаты.Ссылка КАК Ссылка
	      	               |ИЗ
	      	               |	Справочник.Чемпионаты КАК Чемпионаты
	      	               ...
	      	               |	И Чемпионаты.Владелец.ОлимпийскаяСистема = ЛОЖЬ
	      	               |	И НЕ Чемпионаты.Владелец.Команды.Ссылка ЕСТЬ NULL
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	Ссылка
	      	               |АВТОУПОРЯДОЧИВАНИЕ");
...
Функция ТурТекущий(Чемпионат) Экспорт
...
Функция ВидСпорта(Ссылка) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.Города") Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
		                      ...
		                      |
		                      |УПОРЯДОЧИТЬ ПО
		                      |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Команды.Ссылка) УБЫВ");
		Запрос.УстановитьПараметр("Город",			Ссылка);
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			Возврат Выборка.Ссылка;
		КонецЕсли;
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.Матч") Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      |	МатчКоманды.Команда.Владелец КАК Ссылка
		                      |ИЗ
		                      |	Документ.Матч.Команды КАК МатчКоманды
		                      |ГДЕ
...
Функция ВидыСпорта(Наименование="Футбол") Экспорт
	Если НЕ ПустаяСтрока(Наименование) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Возврат НаСервереПовторно.Футбол();
КонецФункции
...
Функция ТурнирнаяТаблица2(Чемпионат) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	МИНИМУМ(ТурнирнаяТаблица.Команда) КАК Команда,
	                      |	ТурнирнаяТаблица.КоличествоОчков КАК КоличествоОчков,
	                      |	ЦЕЛ(ТурнирнаяТаблица.КоличествоГолов) КАК КоличествоГолов,
	                      |	ТурнирнаяТаблица.Пенальти КАК Пенальти,
	                      |	МАКСИМУМ(ТурнирнаяТаблица.Команда2) КАК Команда2,
	                      |	ТурнирнаяТаблица.КоличествоОчков2 КАК КоличествоОчков2,
	                      |	ЦЕЛ(ТурнирнаяТаблица.КоличествоГолов2) КАК КоличествоГолов2,
	                      ...
	                      |	ТурнирнаяТаблица.Проведен КАК Проведен
	                      |ИЗ
	                      |	(ВЫБРАТЬ
	                      |		ТурнирнаяТаблица.Тур КАК Тур,
	                      ...
	                      |				ИНАЧЕ ТурнирнаяТаблица.КоличествоГолов * 1.001
	                      ...
	                      |		СУММА(ВЫБОР
	                      |				КОГДА Противник.НомерСтроки = 1
	                      |					ТОГДА Противник.КоличествоГолов
	                      |				ИНАЧЕ Противник.КоличествоГолов * 1.001
	                      |			КОНЕЦ) КАК КоличествоГолов2,
	                      |		СУММА(Противник.Пенальти) КАК Пенальти2,
	                      |		МИНИМУМ(ТурнирнаяТаблица.Регистратор) КАК Первая,
	                      |		МАКСИМУМ(ТурнирнаяТаблица.Регистратор) КАК Вторая,
	                      |		МАКСИМУМ(ТурнирнаяТаблица.Регистратор.Проведен) КАК Проведен,
	                      ...
	                      |				ТОГДА ИСТИНА
	                      |			ИНАЧЕ ТурнирнаяТаблица.КоличествоГолов + ТурнирнаяТаблица.Пенальти > ТурнирнаяТаблица.КоличествоГолов2 + ТурнирнаяТаблица.Пенальти2
	                      |		КОНЕЦ
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	ТурнирнаяТаблица.Тур,
	                      |	ТурнирнаяТаблица.ТурКод,
	                      |	ТурнирнаяТаблица.КоличествоОчков,
	                      |	ТурнирнаяТаблица.КоличествоГолов,
	                      |	ТурнирнаяТаблица.Пенальти,
	                      |	ТурнирнаяТаблица.КоличествоОчков2,
	                      |	ТурнирнаяТаблица.КоличествоГолов2,
	                      |	ТурнирнаяТаблица.Пенальти2,
	                      |	ТурнирнаяТаблица.Первая,
	                      |	ТурнирнаяТаблица.Вторая,
	                      |	ТурнирнаяТаблица.Проведен,
	                      |	ТурнирнаяТаблица.КоличествоМатчей
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	ТурКод УБЫВ,
	                      |	Первая
	                      ...
	                      |ПО
	                      |	Тур");
	Если НаСервереПовторно.ПоказыватьПредстоящиеМатчи() Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ТурнирнаяТаблица.Тур КАК Тур,
		               |	ТурнирнаяТаблица.ТурКод КАК ТурКод,
		               |	МИНИМУМ(ТурнирнаяТаблица.Команда) КАК Команда,
		               ...
		               |	ТурнирнаяТаблица.Вторая КАК Вторая,
		               |	ТурнирнаяТаблица.КоличествоМатчей КАК КоличествоМатчей,
		               |	МАКСИМУМ(ТурнирнаяТаблица.Проведен2) КАК Проведен
		               |ИЗ
		               |	(ВЫБРАТЬ
		               |		Матч.Тур КАК Тур,
		               ...
		               |						И МатчКоманды.НомерСтроки = 1
		               |					ТОГДА МатчКоманды.КоличествоГолов
		               |				КОГДА Матч.Проведен = ИСТИНА
		               |					ТОГДА МатчКоманды.КоличествоГолов * 1.001
		               |				ИНАЧЕ 0
		               |			КОНЕЦ) КАК КоличествоГолов,
		               |		СУММА(ВЫБОР
		               |				КОГДА Матч.Проведен = ИСТИНА
		               |					ТОГДА МатчКоманды.Пенальти
		               |				ИНАЧЕ 0
		               |			КОНЕЦ) КАК Пенальти,
		               |		Противник.Команда КАК Команда2,
		               |		ЕСТЬNULL(СУММА(ТурнирнаяТаблицаП.КоличествоОчков), 0) КАК КоличествоОчков2,
		               |		СУММА(ВЫБОР
		               |				КОГДА Противник.Ссылка.Проведен = ИСТИНА
		               |						И Противник.НомерСтроки = 1
		               |					ТОГДА Противник.КоличествоГолов
		               |				КОГДА Противник.Ссылка.Проведен = ИСТИНА
		               |					ТОГДА Противник.КоличествоГолов * 1.001
		               |				ИНАЧЕ 0
		               |			КОНЕЦ) КАК КоличествоГолов2,
		               |		СУММА(ВЫБОР
		               |				КОГДА Противник.Ссылка.Проведен = ИСТИНА
		               |					ТОГДА Противник.Пенальти
		               |				ИНАЧЕ 0
		               |			КОНЕЦ) КАК Пенальти2,
		               |		МИНИМУМ(МатчКоманды.Ссылка) КАК Первая,
		               |		МАКСИМУМ(МатчКоманды.Ссылка) КАК Вторая,
		               |		КОЛИЧЕСТВО(РАЗЛИЧНЫЕ МатчКоманды.Ссылка) КАК КоличествоМатчей,
		               |		МАКСИМУМ(Матч.Проведен) КАК Проведен2,
		               ...
		               |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Матч.Команды КАК Противник
		               |				ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблицаП
		               |				ПО Противник.Ссылка = ТурнирнаяТаблицаП.Регистратор
		               |					И Противник.Команда = ТурнирнаяТаблицаП.Команда
		               |			ПО МатчКоманды.Ссылка = Противник.Ссылка
		               |				И МатчКоманды.НомерСтроки <> Противник.НомерСтроки
		               |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Матч КАК Матч
		               |			ПО МатчКоманды.Ссылка = Матч.Ссылка
		               |	ГДЕ
		               |		МатчКоманды.Ссылка.Тур.Владелец = &Чемпионат
		               |		И МатчКоманды.Ссылка.Тур.ОлимпийскаяСистема = ИСТИНА
		               |		И МатчКоманды.Ссылка.ПометкаУдаления = ЛОЖЬ
		               |	
		               |	СГРУППИРОВАТЬ ПО
		               |		Противник.Команда,
		               |		Матч.Тур,
		               |		Матч.Тур.Код,
		               |		МатчКоманды.Команда) КАК ТурнирнаяТаблица
		               |ГДЕ
		               |	ВЫБОР
		               |			КОГДА ТурнирнаяТаблица.КоличествоОчков > ТурнирнаяТаблица.КоличествоОчков2
		               |				ТОГДА ИСТИНА
		               |			КОГДА ТурнирнаяТаблица.Проведен = ИСТИНА
		               |				ТОГДА ТурнирнаяТаблица.КоличествоГолов + ТурнирнаяТаблица.Пенальти > ТурнирнаяТаблица.КоличествоГолов2 + ТурнирнаяТаблица.Пенальти2
		               |			КОГДА ТурнирнаяТаблица.Проведен2 = ИСТИНА
		               |				ТОГДА ТурнирнаяТаблица.КоличествоГолов + ТурнирнаяТаблица.Пенальти >= ТурнирнаяТаблица.КоличествоГолов2 + ТурнирнаяТаблица.Пенальти2
		               |			ИНАЧЕ ТурнирнаяТаблица.КоличествоМатчей = 1
		               |					ИЛИ ТурнирнаяТаблица.Проведен2 = ЛОЖЬ
		               |		КОНЕЦ
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ТурнирнаяТаблица.Тур,
		               ...
		               |	ТурнирнаяТаблица.Первая,
		               |	ТурнирнаяТаблица.Вторая,
		               |	ЦЕЛ(ТурнирнаяТаблица.КоличествоГолов),
		               |	ЦЕЛ(ТурнирнаяТаблица.КоличествоГолов2)
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	ТурКод УБЫВ,
		               |	Проведен
		               |ИТОГИ
		               |	МАКСИМУМ(КоличествоМатчей),
		               |	МАКСИМУМ(Проведен)
		               |ПО
		               |	Тур";
	КонецЕсли;
...
Функция КомментарииПолучить(Чемпионат, Команда=Неопределено) Экспорт
	...
	|	СнятиеОчков.Ссылка КАК Регистратор,
	|	ВЫБОР
	...
	|	СнятиеОчков.Команда КАК Команда,
	|	СнятиеОчков.КоличествоОчков КАК КоличествоОчков
	|ИЗ
	|	Документ.СнятиеОчков КАК СнятиеОчков
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	|		ПО СнятиеОчков.Ссылка = ТурнирнаяТаблица.Регистратор
	|ГДЕ
	|	СнятиеОчков.Тур.Владелец = &Чемпионат
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата,
	|	Команда");
...
Функция БуферНовостей(КакМассив=Ложь) Экспорт
	Возврат ?(КакМассив, Разделить(Константы.БуферНовостей.Получить(), Символы.ПС), Константы.БуферНовостей.Получить());
КонецФункции
...
Функция МестоВЧемпионате(Команда, Чемпионат=Неопределено) Экспорт
	...
	Пока НЕ ШалостьУдалась И Выборка.Следующий() Цикл
		Ответ = Ответ + 1;
		Если Ответ = 1 И Выборка.КоличествоИгр = 0 Тогда
			Прервать;
		ИначеЕсли Выборка.Команда = Команда Тогда
...
Функция Амплуа(Наименование="Вратарь", ВидСпорта=Неопределено) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	Амплуа.Владелец = &Владелец
	|	И Амплуа.Наименование = &Наименование
	|
	|УПОРЯДОЧИТЬ ПО
	|	Амплуа.ПометкаУдаления");
	Запрос.УстановитьПараметр("Наименование",		СокрЛП(Наименование));
	Запрос.УстановитьПараметр("Владелец",			?(ЗначениеЗаполнено(ВидСпорта), ВидСпорта, НаСервереПовторно.Футбол()));
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Ссылка;
	КонецЕсли;
...
Функция ИгрокНайти(Наименование, Параметр=Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Параметр) = Тип("Дата") Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ВТ.Игрок КАК Игрок,
		               |	ЕСТЬNULL(Перемещения.Амплуа.Ссылка, ВТ.Амплуа) КАК Амплуа
		               |ИЗ
		               |	(ВЫБРАТЬ ПЕРВЫЕ 1
		               |		Игроки.Ссылка КАК Игрок,
		               ...
		               |			КОГДА Игроки.Наименование ПОДОБНО &Комментарий
		               |				ТОГДА 6
		               |			КОГДА Игроки.Наименование ПОДОБНО &Комментарий2
		               |				ТОГДА 2
		               |			ИНАЧЕ 0
		               |		КОНЕЦ + ВЫБОР
		               |			КОГДА Игроки.Комментарий ПОДОБНО &Комментарий
		               ...
		               |		И (Игроки.Наименование ПОДОБНО &Наименование
		               |				ИЛИ Игроки.Наименование ПОДОБНО &Комментарий
		               |				ИЛИ Игроки.Наименование ПОДОБНО &Комментарий2
		               |				ИЛИ Игроки.Комментарий ПОДОБНО &Комментарий
	...
	ИначеЕсли ТипЗнч(Параметр) = Тип("Структура") Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	Порядок,
		               |	Игроки.ПометкаУдаления";
		Запрос.УстановитьПараметр("Команда",			Параметр.Команды);
		Запрос.УстановитьПараметр("Дата",				Параметр.Дата);
	КонецЕсли;
	Запрос.УстановитьПараметр("Наименование",		СокрЛП(Наименование));
	Запрос.УстановитьПараметр("Комментарий",		Обернуть(Наименование));
...
Функция ПолучитьЭлементыПоИмени(Гиперссылка, Имя="td", КомОбъект=Неопределено) Экспорт
	Текст	= СкачатьПоСсылке(Гиперссылка, КомОбъект);
...
Функция МатчиПоследние5(Команда, Чемпионат, Дата=Неопределено, Сколько=0, НомерСтроки=0) Экспорт
	Запрос	= Новый Запрос(СтрЗаменить("ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 5
	      	                           |	ТурнирнаяТаблица.Период КАК Период,
	      	                           |	ТурнирнаяТаблица.КоличествоОчков КАК КоличествоОчков,
	      	                           |	ТурнирнаяТаблица.Регистратор КАК Регистратор
	      	                           |ПОМЕСТИТЬ ВТ
	      	                           ...
	      	                           |
	      	                           |УПОРЯДОЧИТЬ ПО
	      	                           |	Период", "5", ФорматЧГ(Макс(1, ?(Сколько=0, НаСервереПовторно.КоличествоМатчейВАнонсе(), Сколько)))));
...
Функция ИмпортЧемпионата(Гиперссылка, Чемпионат) Экспорт
	...
	Иначе
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |
		                      |ИМЕЮЩИЕ
		                      |	1 + 2 * ЕСТЬNULL(КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Регистратор), 0) < КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЧемпионатыКоманды.Команда)
		                      |
		                      |УПОРЯДОЧИТЬ ПО
		                      |	Туры.Код");
		Запрос.УстановитьПараметр("Чемпионат",		Чемпионат);
...
Функция МатчНайти(Команды, Дата, Тур=Неопределено) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	И МатчКоманды.НомерСтроки = 1
	                      |	И Противники.Команда = &Противник
	                      |	И Противники.НомерСтроки = 2
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Проведен УБЫВ,
	                      |	ПометкаУдаления,
	                      |	РАЗНОСТЬДАТ(Матч.Дата, &Дата, ДЕНЬ) УБЫВ");
	Если Команды.Количество() = 2 Тогда
		Запрос.УстановитьПараметр("Команда",	Команды.Получить(0));
		Запрос.УстановитьПараметр("Противник",	Команды.Получить(1));
		Если ЗначениеЗаполнено(Тур) Тогда
			...
			Запрос.УстановитьПараметр("Тур",		Тур);
		Иначе
			Запрос.УстановитьПараметр("Дата1",		НачалоДня(Дата - Сутки(2)));
			Запрос.УстановитьПараметр("Дата2",		КонецДня(Дата + Сутки(2)));
		КонецЕсли;
	Иначе
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               |	Матч.Ссылка КАК Ссылка,
...
Процедура ДобавитьВремя(Дата, Время, Сдвиг=0)
	Попытка
...
Функция ИмпортИгрПоГиперссылке(Гиперссылка, Тур, КомОбъект=Неопределено) Экспорт
	...
	Зачистить(сИгра);
	Слова	= ПолучитьЭлементыПоИмени(Гиперссылка,, КомОбъект);
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли СтрЧислоСтрок(Слово) > 2 Тогда
			Продолжить;
		ИначеЕсли Итого >= Параметры.ВсегоИгр Или Итого > 1 И СтрНайти2(Слово, "таблица") > 0 Тогда
			Прервать;
		ИначеЕсли СтрЧислоСтрок(Слово) = 2 Тогда
		...
		И СтрНайти2(Слово, "тур") > 0
		Тогда
			Если СтрНайти(Слово, сИгра.Тур.Код) = 0 Тогда
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгра.Команда2) И НЕ ЗначениеЗаполнено(сИгра.Команда1) Тогда
			...
			КонецЕсли;
			Если сИгра.Проведен Тогда
				Продолжить;
			КонецЕсли;
			
		ИначеЕсли ПустаяСтрока(сИгра.Счет)
		И (СтрЧислоВхождений(Слово, ":") = 2 Или (СтрЧислоВхождений(Слово, ":") = 1 И СтрНайти2(Слово, "(ТП)") > 3))
		Тогда
		...
		Тогда
			...
			Если Матч.Проведен
			И НачалоДня(Матч.Дата) = НачалоДня(сИгра.Дата)
			Тогда
				Зачистить(сИгра);
				Продолжить;
...
Функция ИмпортНаполнитель(Тур=Неопределено)
...
Функция ИмпортИгрПоСтроке2(Текст, Тур=Неопределено) Экспорт
	Ответ	= 0;
	Матч	= Новый Структура("Дата,Проведен,Клубы,Команды,Тур", Дата(1, 1, 1), Ложь, Новый Массив, Новый Массив, Тур);
	Игнор	= Разделить("Метка Группа Тур Дата,время Счет");
	Искать	= Истина;
	КлубИмя	= "";
	Уникум = ЗначениеЗаполнено(Тур) И НЕ Тур.ОлимпийскаяСистема;
	Для Каждого Слова Из Разделить(СтрЗаменить(Текст, "¶", " "), "	", Истина) Цикл
			...
			Для Каждого Слово Из Разделить(СтрЗаменить(Слова, " : ", ":")) Цикл
				Если СтрНайти(Слово, ":") > 0 Тогда
					...
					ИначеЕсли СтрНайти(КлубИмя, "(") = 0 Тогда
						Клуб = КомандаНайти(КлубИмя, Тур);
					Иначе
						мСтроки	= Разделить(КлубИмя, "(", Истина);
						Клуб = КомандаНайти(мСтроки.Получить(0), СтрУдалить(мСтроки.Получить(1), ")"));
						Если НЕ ЗначениеЗаполнено(Клуб) Тогда
							Клуб = КомандаНайти(КлубИмя, Тур);
						КонецЕсли;
					КонецЕсли;
					Если ЗначениеЗаполнено(Клуб) Тогда
						Матч.Клубы.Добавить(Клуб);
					ИначеЕсли НЕ ПустаяСтрока(КлубИмя) Тогда
						Сообщить(КлубИмя);
					КонецЕсли;
					КлубИмя	= "";
					...
					Если Матч.Команды.Количество() = 2 Тогда
						...
						Для Каждого Команда Из Матч.Команды Цикл
							Команда.Пенальти = СтрокаКакЧисло(Цифры.Получить(Итератор));
							Итератор = Итератор + 1;
						КонецЦикла;
					ИначеЕсли Матч.Клубы.Количество() = 2 Тогда
						Если СтрНайти(Слова, "– : –") = 0 Тогда
							Цифры		= Разделить(Слово, ":");
							Итератор	= 0;
							Для Каждого Клуб Из Матч.Клубы Цикл
								Матч.Команды.Добавить(Новый Структура("Команда,КоличествоГолов,Пенальти", Клуб, СтрокаКакЧисло(Цифры.Получить(Итератор)), 0));
								Итератор = Итератор + 1;
							КонецЦикла;
							Матч.Проведен = Истина;
						Иначе
							Для Каждого Клуб Из Матч.Клубы Цикл
								Матч.Команды.Добавить(Новый Структура("Команда,КоличествоГолов,Пенальти", Клуб, 0, 0));
							КонецЦикла;
						КонецЕсли;
					КонецЕсли;
				ИначеЕсли Слово = "–" Тогда
					...
					КонецЕсли;
					КлубИмя = "";
				ИначеЕсли СтрСравнить(Слово, "ДВ") = 0 Тогда
					//
				ИначеЕсли СтрСравнить(Слово, "Т") = 0 Тогда
					//
				Иначе
					КлубИмя = ?(ПустаяСтрока(КлубИмя), Слово, КлубИмя + " " + Слово);
				КонецЕсли;
			КонецЦикла;
			Если Матч.Команды.Количество() = 2 Тогда
				Ссылка = МатчНайти(Матч.Клубы, Матч.Дата, Тур);
				Если ЗначениеЗаполнено(Ссылка) Тогда
					Док	= Ссылка.ПолучитьОбъект();
					Если Док.ПометкаУдаления Тогда
						Док.УстановитьПометкуУдаления(Ложь);
					КонецЕсли;
					ОбщегоНазначения.УстановитьЗначение(Док.Дата, Матч.Дата);
					Для Каждого Элемент Из Док.Команды Цикл
						Команда = Матч.Команды.Получить(Элемент.НомерСтроки - 1);
						ОбщегоНазначения.УстановитьЗначение(Элемент.Команда, Команда.Команда);
						ОбщегоНазначения.УстановитьЗначение(Элемент.КоличествоГолов, Команда.КоличествоГолов);
						ОбщегоНазначения.УстановитьЗначение(Элемент.Пенальти, Команда.Пенальти);
					КонецЦикла;
				Иначе
					Док = МатчСоздать(Искать);
					Док.Дата	= Матч.Дата;
					...
					КонецЦикла;
				КонецЕсли;
				ОбщегоНазначения.УстановитьЗначение(Док.Тур, Тур, Истина);
				Если Док.Модифицированность() Или НЕ Док.Проведен Тогда
					Если Док.ПроверитьЗаполнение() Тогда
						Попытка
							Док.ДополнительныеСвойства.Вставить("Импорт", Истина);
							Док.Записать(?(Матч.Проведен, РежимЗаписиДокумента.Проведение, РежимЗаписиДокумента.Запись));
							Ответ = Ответ + 1;
						Исключение
		...
		ИначеЕсли Игнор.Найти(Слова) <> Неопределено Тогда
			//
		ИначеЕсли СтрокаЗаканчиваетсяЧислом(Слова) Тогда
			Если СтрДлина(СтрУдалить(Слова, " ")) = 15 Тогда
				Матч.Дата = СтрокаКакДата(Лев(Слова, 10));
			Иначе
				Игнор.Добавить(Слова);
			КонецЕсли;
		Иначе
			Игнор.Добавить(Слова);
		КонецЕсли;
...
Функция ИмпортИгрПоСтроке3(Текст, Тур, Разделитель="¶") Экспорт
	Если ТипЗнч(Тур) = Тип("СправочникСсылка.Туры") Тогда
		сИгра	= ИмпортНаполнитель(Тур);
	ИначеЕсли ТипЗнч(Тур) = Тип("СправочникСсылка.Чемпионаты") Тогда
		сИгра	= ИмпортНаполнитель();
	Иначе
		Возврат Ложь;
	КонецЕсли;
	...
	Зачистить(сИгра);
	Слова	= Разделить(Текст, Разделитель);
	Для Каждого Слово Из Слова Цикл
		Если ПустаяСтрока(Слово) Тогда
			Продолжить;
		ИначеЕсли СтрНайти(Слово, "|") > 1 Тогда
			мСлова	= Разделить(Слово, "|");
			Если мСлова.Количество() > 1 Тогда
				сИгра.Тур = ТурНайти(мСлова.Получить(1), Тур);
			КонецЕсли;
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгра.Тур) Тогда
			Продолжить;
		ИначеЕсли Год(сИгра.Дата) < 2000 Тогда
			Если СтрДлина(Слово) >= 13 Тогда
				мСлова	= Разделить(СтрУдалить(Слово, ","));
				Если мСлова.Количество() > 0 Тогда
					сИгра.Дата	= СтрокаКакДата(мСлова.Получить(0));
					Если мСлова.Количество() > 1 И ЗначениеЗаполнено(сИгра.Дата) Тогда
						ДобавитьВремя(сИгра.Дата, мСлова.Получить(1));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгра.Команда1) Тогда
			сИгра.Команда1 = КомандаНайти(Слово, сИгра.Чемпионат);
		ИначеЕсли сИгра.Гол1 < 0 Тогда
			сИгра.Гол1	= СтрокаКакЧисло(Слово);
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгра.Команда2) Тогда
			сИгра.Команда2 = КомандаНайти(Слово, сИгра.Чемпионат);
		ИначеЕсли сИгра.Гол2 < 0 Тогда
...
Функция ИмпортМатча(Гиперссылка, Матч, КомОбъект=Неопределено) Экспорт
	Ответ	= Ложь;
	...
	сМатч.Вставить("Клубы",		Новый Массив);
	Команда1	= "";
	Команда2	= "";
	Тренеры		= Ложь;
	События		= Новый Массив;
	Комментарий	= "";
	ОлимпийскаяСистема = ЗначениеЗаполнено(Матч.Тур) И Матч.Тур.ОлимпийскаяСистема;
	
	Слова	= ПолучитьЭлементыПоИмени(Гиперссылка,, КомОбъект);
	Для Каждого Слово Из Слова Цикл
		Если НЕ ЗначениеЗаполнено(сМатч.Дата) Тогда
			Если СтрЧислоСтрок(Слово) = 1 И СтрНайти2(Слово, "стадион") > 0 Тогда
				ШапкаЗаполнить(сМатч, Слово);
			ИначеЕсли СтрЧислоСтрок(Слово) = 1 И СтрНайти2(Слово, "(ТП)") > 0 Тогда
				ШапкаЗаполнить(сМатч, Лев(Слово, СтрНайти2(Слово, "(ТП)") - 1));
			КонецЕсли;
				...
				Если КомандыЗаполнить(сМатч, Слово) Тогда
					Для Каждого ТекЭлемент Из сМатч.Команды Цикл
						Если ПустаяСтрока(Команда1) Тогда
							...
							сМатч.Клубы.Добавить(ТекЭлемент.Команда);
							сМатч.Игроки1	= ИгрокиПолучить(ТекЭлемент.Команда, сМатч.Дата);
							Если ТипЗнч(сМатч.Стадион) = Тип("СправочникСсылка.Стадионы") И ЗначениеЗаполнено(сМатч.Стадион) Тогда
								//
							ИначеЕсли ЗначениеЗаполнено(Матч.Тур) И НЕ ОлимпийскаяСистема Тогда
								сМатч.Стадион = СтадионНайти(Новый Структура("Команда,Тур", ТекЭлемент.Команда, Матч.Тур));
							ИначеЕсли ЗначениеЗаполнено(сМатч.Страна) И ОлимпийскаяСистема Тогда
								сМатч.Стадион = СтадионНайти(ТекЭлемент.Команда);
							Иначе
								Стадионы = СтадионНайти(ТекЭлемент.Команда, Истина);
								Если Стадионы.Количество() = 1 Тогда
									сМатч.Стадион = Стадионы.Получить(0);
								КонецЕсли;
							КонецЕсли;
		...
		ИначеЕсли СтрЧислоСтрок(Слово) = 1 Тогда
			...
			ИначеЕсли СоставыКомандДобавить(Слово, 1, сМатч) Тогда
				Продолжить;
			Иначе
				Игрок = ИгрокНайти(Слово, Новый Структура("Команды,Дата", сМатч.Клубы, сМатч.Дата));
				Если сМатч.Клубы.Найти(Игрок.Команда) = Неопределено Тогда
					//
				Иначе
...
Функция КомандыЗаполнить(Матч, Слова)
	...
	Для Каждого Слово Из Разделить(СтрЗаменить(Слова, " - ", "="), "=") Цикл
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(Матч.Команда1) Тогда
			Если СтрЧислоСтрок(Слово) = 1 Тогда
...
Функция ИгрокиПолучить(Команда, Дата) Экспорт
	Ответ	= Новый Соответствие;
	Запрос	= Новый Запрос("ВЫБРАТЬ
...
Функция КомментарийПричесать(Знач Комментарий) Экспорт
	Если СтрЧислоСтрок(Комментарий) > 1 Тогда
		Комментарий	= СтрПолучитьСтроку(Комментарий, СтрЧислоСтрок(Комментарий));
	КонецЕсли;
	
	мСлова	= Разделить(Комментарий, ".");
	Слово	= мСлова.Получить(мСлова.ВГраница());
	Если Разделить(Слово).Количество() = 2
	//И СтрЧислоВхождений(Комментарий, Слово) = 1
	Тогда
		Если СтрНайти(Комментарий, Слово + ".") > 0 Тогда
			Слово = Слово + ".";
		КонецЕсли;
		Комментарий	= СтрЗаменить(Комментарий, Слово, Выделить(Слово));
	КонецЕсли;
	Для Каждого Слово Из Разделить("Фото - Getty Images.,Фото - Getty Images,Предыстория.,Предыстория,Ход матча.,Ход матча,Ход игры", ",") Цикл
		Если СтрНайти2(Комментарий, Слово) > 0 Тогда
			Комментарий	= СтрЗаменить(Комментарий, Слово, Разрыв(Истина));
		КонецЕсли;
	КонецЦикла;
...
Процедура ШапкаЗаполнить(Матч, Слова)
	...
	Ультима	= "";
	Для Каждого Слово Из Разделить(Слова, ".") Цикл
		Если Год(Матч.Дата) < 2 И СтрокаПохожаНаДату(Слово) Тогда
			Матч.Дата = КакДата(Слово, ?(ЗначениеЗаполнено(Матч.Тур), Матч.Тур, 0));
			
		ИначеЕсли СтрНайти2(Слово, "стадион") > 0 Тогда
			СтадионИмя	= СокрЛП(СтрУдалить(СтрУдалить(Слово, ("Стадион """)), """"));
			Если НЕ ЗначениеЗаполнено(Стадион) Тогда
				Если ЗначениеЗаполнено(Матч.Тур) Тогда
					СтруктураПоиска = Новый Структура("Наименование,Тур", СтадионИмя, Матч.Тур);
					Если ЗначениеЗаполнено(Матч.Страна) Тогда
						СтруктураПоиска.Вставить("Страна", Матч.Страна);
						Если НЕ ПустаяСтрока(Ультима) Тогда
							СтруктураПоиска.Вставить("Город", ГородНайти(Ультима, Матч.Страна));
						КонецЕсли;
					ИначеЕсли НЕ ПустаяСтрока(Ультима) Тогда
						СтруктураПоиска.Вставить("Город", ГородНайти(Ультима));
					КонецЕсли;
					Стадион	= СтадионНайти(СтруктураПоиска);
				//Иначе
				//	Стадион	= СтадионНайти(СтадионИмя);
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Стадион) Тогда
					Если НЕ ПустаяСтрока(Ультима) И ЗначениеЗаполнено(Матч.Страна) Тогда
						...
						Если ЗначениеЗаполнено(Город) Тогда
							Стадион = СтадионНайти(Город);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			Если ЗначениеЗаполнено(Стадион) Тогда
				ОбщегоНазначения.УстановитьЗначение(Матч.Стадион, Стадион);
			Иначе
				ОбщегоНазначения.УстановитьЗначение(Матч.Стадион, СтадионИмя);
			КонецЕсли;
		...
		ИначеЕсли СтрНайти2(Слово, "зрител") > 0 Тогда
			...
			КонецЕсли;
		ИначеЕсли СтрНайти(Слово, ":") > 0
		И СтрокаСодержитЧисло(Слово)
		Тогда
			Если КомандыЗаполнить(Матч, Слово) Тогда
				Для Каждого ТекЭлемент Из Матч.Команды Цикл
					Если Матч.Клубы.Количество() = 0 Тогда
						Матч.Клубы.Добавить(ТекЭлемент.Команда);
					ИначеЕсли Матч.Клубы.Количество() = 1 Тогда
						Матч.Клубы.Добавить(ТекЭлемент.Команда);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		Иначе
			Ультима	= СокрЛП(Слово);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Функция МатчСохранить(Матч) Экспорт
	Ответ	= Ложь;
	
	Док = Матч.Матч.ПолучитьОбъект();
	Если Док.ПометкаУдаления Тогда
		Док.УстановитьПометкуУдаления(Ложь);
	КонецЕсли;
	Если НачалоДня(Док.Дата) = Матч.Дата Тогда
		ЗаполнитьЗначенияСвойств(Док, Матч,, "Дата");
	Иначе
		ЗаполнитьЗначенияСвойств(Док, Матч);
	КонецЕсли;
	Если Док.Команды.НайтиСтроки(Новый Структура("Команда,КоличествоГолов", Матч.Команда1, Матч.Команды.Получить(0).КоличествоГолов)).Количество() = 0
		Или Док.Команды.НайтиСтроки(Новый Структура("Команда,КоличествоГолов", Матч.Команда2, Матч.Команды.Получить(1).КоличествоГолов)).Количество() = 0
		Тогда
		Док.Команды.Очистить();
		Для Каждого ТекЭлемент Из Матч.Команды Цикл
	...
	КонецЕсли;
	Док.СоставыКоманд.Очистить();
	Для Каждого ТекЭлемент Из Матч.СоставыКоманд Цикл
...
Функция КакДата(Слово, Тур)
	...
	Иначе
		...
		Для Каждого Строка Из Разделить(Слово) Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если Мезе > 0 И Мезе < 13 И Жьорно > 0 И Жьорно <= 31 Тогда
			Попытка
				Если ТипЗнч(Тур) = Тип("СправочникСсылка.Туры") И ЗначениеЗаполнено(Тур) Тогда
					Ответ = Дата(Год(?(Тур.Владелец.ДатаНачала > ТекущаяДатаСеанса(), Тур.Владелец.ДатаНачала, Тур.Владелец.ДатаОкончания)), Мезе, Жьорно);
				Иначе
					Ответ = Дата(?(Тур < 2, Год(ТекущаяДатаСеанса()), Тур), Мезе, Жьорно);
				КонецЕсли;
				Если Ответ > ДобавитьМесяц(Тур.Владелец.ДатаОкончания, 1) Тогда
					Ответ = Дата(Год(Тур.Владелец.ДатаНачала), Мезе, Жьорно);
				ИначеЕсли Ответ < Тур.Владелец.ДатаНачала Тогда
					Ответ = Дата(Год(Тур.Владелец.ДатаОкончания), Мезе, Жьорно);
				КонецЕсли;
			Исключение КонецПопытки;
			Если Год(Ответ) < 2 И ЧислоЧетное(Год(Тур.Владелец.ДатаОкончания)) Тогда
				Попытка
					Ответ = Дата(Год(Тур.Владелец.ДатаОкончания), Мезе, Жьорно);
				Исключение КонецПопытки;
			КонецЕсли;
...
Функция ТурНайти(КодИлиНаименование, Чемпионат) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	Туры.ПометкаУдаления");
	Запрос.УстановитьПараметр("Чемпионат",		Чемпионат);
	Запрос.УстановитьПараметр("Код",			КодИлиНаименование);
	Если ТипЗнч(КодИлиНаименование) = Тип("Строка") Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               |	Туры.Ссылка КАК Ссылка
		               |ИЗ
		               |	Справочник.Туры КАК Туры
		               ...
		               |УПОРЯДОЧИТЬ ПО
		               |	Туры.ПометкаУдаления";
		Запрос.УстановитьПараметр("Наименование",	КодИлиНаименование);
	КонецЕсли;
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Ссылка;
...
Функция ТурИлиЧемпионат(Чемпионат)
	Если ТипЗнч(Чемпионат) = Тип("СправочникСсылка.Чемпионаты") Тогда
		Тур		= ?(НЕ Чемпионат.Владелец.ОлимпийскаяСистема И НаСервереПовторно.ВыводитьТурВместоЧемпионата() И НЕ НаСервереПовторно.ЧемпионатПустой(Чемпионат), ТурТекущий(Чемпионат), Чемпионат);
	Иначе
		Тур		= ?(НЕ Чемпионат.ОлимпийскаяСистема И НаСервереПовторно.ВыводитьТурВместоЧемпионата() И НЕ НаСервереПовторно.ЧемпионатПустой(Чемпионат), ТурТекущий(Чемпионат), Чемпионат);
	КонецЕсли;
	Возврат ?(ЗначениеЗаполнено(Тур), Тур, Чемпионат);
КонецФункции

Процедура ЗаголовокВывести(ТабДокумент, Параметры) Экспорт
	ТабДокумент.Вывести(ЗаголовокПолучить(Параметры));
	Если Параметры.Свойство("Чемпион")
	И НЕ НаСервереПовторно.ВыводитьЛоготипыКоманд()
	Тогда
		Чемпион= Чемпион(Параметры.Чемпионат);
		Если ЗначениеЗаполнено(Чемпион) Тогда
			Изображение	= ИзображениеПолучить(Чемпион, Истина);
			Если Изображение.Вид <> ВидКартинки.Пустая Тогда
				Макет  = ПолучитьОбщийМакет("ТурнирнаяТаблица");
				ОбластьМакета = Макет.ПолучитьОбласть("Заголовок|NN");
				ТабДокумент.Присоединить(ОбластьМакета);
				ОбластьМакета.ТекущаяОбласть.Картинка = Изображение;
				ТабДокумент.Присоединить(ОбластьМакета);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаголовокПолучить(Параметры) Экспорт
	...
	КонецЕсли;
	Макет  = ПолучитьОбщийМакет("ТурнирнаяТаблица");
	ОбластьМакета = Макет.ПолучитьОбласть("Заголовок|Начало2");
...
Процедура ПередЗаписьюНаСервереИзображение(ТекущийОбъект, Адрес, Отказ) Экспорт
	...
	ИначеЕсли ПустаяСтрока(Адрес) И ЗначениеЗаполнено(ТекущийОбъект.Изображение) Тогда
		ТекущийОбъект.Изображение = NULL;
	КонецЕсли;
КонецПроцедуры

Процедура Логировать(ИмяСобытия, Ошибка, Данные=Неопределено, Уровень=Неопределено) Экспорт
	Если НаСервереПовторно.ЛогированияУровеньДоступен(Уровень) Тогда
...
Функция ПервоеДороже(Слова, Первое=Ложь)
	...
	Иначе
		Если мСлова.Количество() = 1 Тогда
			Ответ	= СокрЛП(Разделить(Слова, "|").Получить(0));
		Иначе
			Если мСлова.Количество() >= 2 Тогда
...
Функция СтадионыЧемпионата(Чемпионат) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      ...
	                      |				ИЛИ ЧемпионатыКоманды.Команда.Город = Стадионы.Владелец)
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      |			МатчКоманды.Команда КАК Команда,
	                      |			МатчКоманды.Ссылка.Стадион КАК Стадион
	                      |		ИЗ
	                      |			Документ.Матч.Команды КАК МатчКоманды
	                      |		ГДЕ
	                      |			МатчКоманды.Ссылка.Тур.Владелец = &Чемпионат
	                      |			И МатчКоманды.Ссылка.Проведен = ИСТИНА
	                      |			И (МатчКоманды.Ссылка.Стадион.Ссылка ЕСТЬ NULL) = ЛОЖЬ) КАК МатчКоманды
	                      |		ПО ЧемпионатыКоманды.Команда = МатчКоманды.Команда
	                      |ГДЕ
	                      |	ЧемпионатыКоманды.Ссылка = &Чемпионат
	                      |	И ВЫБОР
	                      |			КОГДА МатчКоманды.Стадион ЕСТЬ NULL
	                      |				ТОГДА Стадионы.ПометкаУдаления = ЛОЖЬ
	                      |			ИНАЧЕ Стадионы.Ссылка = МатчКоманды.Стадион
	                      |		КОНЕЦ
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Стадион
...
Функция СтрокаКакКоманды(Знач Слова, Тимз, ВидСпорта)
	Ответ	= Новый Массив;
	Слова	= СокрЛП(СтрУдалить(Слова, Разделить("(N) (A) (H)")));
	Солянка	= "";
	мСлова	= Новый Массив;
	Для Каждого Слово Из Разделить(Слова) Цикл
	...
	Для Каждого Слово Из мСлова Цикл
		...
		Если Команда = Неопределено Тогда
			Для Каждого Тим Из Тимз Цикл
				Если СтрНайти2(Тим.Ключ, Слово) > 0 И Ответ.Найти(Тим.Значение) = Неопределено Тогда
					Команда	= Тим.Значение;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция ТурПодобрать(Матч)
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |			И Чемпионаты.Владелец.Комментарий ПОДОБНО &Комментарий
	                      |		
	                      |		СГРУППИРОВАТЬ ПО
	                      |			Чемпионаты.Ссылка
	                      |		
	                      ...
	                      |		
	                      |		УПОРЯДОЧИТЬ ПО
	                      |			ДатаОкончания УБЫВ) КАК ВложенныйЗапрос
	                      |		ПО Туры.Владелец = ВложенныйЗапрос.Ссылка
	                      |ГДЕ
	                      |	Туры.Код = &ТурКод
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("Комментарий",	Обернуть(Матч.Комментарий2));
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Ссылка;
	КонецЕсли;
...
Функция ПрочитатьДату(Слово, ФорматДаты=Неопределено) Экспорт
	Ответ = Дата(1, 1, 1);
	Попытка
		Ответ = ПрочитатьДатуJSON(Слово, ?(ФорматДаты=Неопределено, ФорматДатыJSON.ISO, ФорматДаты));
	Исключение КонецПопытки;
	Возврат Ответ;
КонецФункции
...
Функция ИмпортИгрПоКалендарю(Текст, Команда) Экспорт
	...
	Если ЗначениеЗаполнено(Команда) Тогда
		...
		Тимз.Вставить(Команда.Наименование + Символы.ПС + Команда.Комментарий, Команда);
		Стадион		= СтадионНайти(Команда);
		Стадионы.Вставить(Стадион.Наименование, Стадион);
	КонецЕсли;
	Сдвиг	= ТекущаяДатаСеанса() - ТекущаяУниверсальнаяДата();
	Искать	= Истина;
	...
	Для НомерСтроки = 1 По СтрЧислоСтрок(Текст) Цикл
		...
		Если Исключения.Найти(Слово) <> Неопределено Тогда
			//
		ИначеЕсли СтрСравнить(Первое, "BEGIN") = 0 Тогда
			Если СтрСравнить(ПервоеДороже(Слово), "VEVENT") = 0 Тогда
				Зачистить(Элемент, Истина);
			Иначе
				Шаг = 0;
				Пока СтрНайти(Слово, ":") > 0 Цикл
					Слово	= СтрПолучитьСтроку(Текст, НомерСтроки + Шаг);
					Если СтрСравнить(ПервоеДороже(Слово), "VEVENT") = 0 Тогда
						Прервать;
					ИначеЕсли Исключения.Найти(Слово) = Неопределено Тогда
						Исключения.Добавить(Слово);
					КонецЕсли;
					Шаг = Шаг + 1;
				КонецЦикла;
			КонецЕсли;
		...
		ИначеЕсли СтрСравнить(Первое, "DTSTART") = 0 Тогда
			Элемент.Дата	= ПрочитатьДату(ПервоеДороже(Слово));
		...
		ИначеЕсли СтрСравнить(Первое, "SUMMARY") = 0 Тогда
			...
			Если Элемент.Команды.Найти(Команда) = Неопределено Тогда
				Зачистить(Элемент);
			КонецЕсли;
		...
		ИначеЕсли СтрСравнить(Первое, "DESCRIPTION") = 0 Тогда
			//Если Ложь Тогда
				Элемент.Комментарий = Причесать(ПервоеДороже(Слово));
				ЗаполнитьЗначенияСвойств(Элемент, ОписаниеПолучить(ПервоеДороже(Слово)));
			//КонецЕсли;
		ИначеЕсли СтрСравнить(Первое, "END") = 0 И СтрСравнить(ПервоеДороже(Слово), "VEVENT") = 0 Тогда
			Если Элемент.Дата < ТекущаяДатаСеанса() Тогда
				//
			ИначеЕсли Элемент.Команды.Количество() = 2 Тогда
				...
				ИначеЕсли ЗначениеЗаполнено(Матч) Тогда
					...
					КонецЕсли;
					Если ПустаяСтрока(МатчОбъект.Комментарий) Тогда
						МатчОбъект.Комментарий = Элемент.Комментарий;
					КонецЕсли;
					Если НЕ ЗначениеЗаполнено(МатчОбъект.Тур) И НЕ ПустаяСтрока(Элемент.Комментарий2) Тогда
						МатчОбъект.Тур = ТурПодобрать(Элемент);
					КонецЕсли;
				Иначе
...
Функция ДанныеРасшифровкиПолучитьЗначение(ДанныеРасшифровки, Расшифровка) Экспорт
	...
	Элемент	= Данные.Элементы.Получить(Расшифровка - 1);
		...
		Пока ПустаяСтрока(Ответ) И Выборка.Следующий() Цикл
			...
			ИначеЕсли СтрНайти(Ответ, "%") = 0 Тогда
				...
				ИначеЕсли СтрНайти(Ответ, Год) > 0 Тогда
					Ответ = СтрШаблон(СтрУдалить(Ответ, "games.htm") + ?(Год(Выборка.ДатаОкончания) > 2010, "%1/", "%1.htm"), Код);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ФоновоеЗаданиеВыполнить(ИмяМетода, Синоним)
	Ответ = Ложь;
	Если ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("ИмяМетода,Состояние", ИмяМетода, СостояниеФоновогоЗадания.Активно)).Количество() = 0 Тогда
		ФоновыеЗадания.Выполнить(ИмяМетода,,, СтрШаблон("Запуск вручную: %1", Синоним));
		Ответ = Истина;
	КонецЕсли;
	Возврат Ответ;
КонецФункции

Функция ЗапуститьРегламентноеЗадание(Ключ="") Экспорт
	Ответ = Ложь;
	Если ПустаяСтрока(Ключ) Тогда
		Для Каждого Регламент Из РегламентныеЗадания.ПолучитьРегламентныеЗадания() Цикл
			Если Регламент.Использование Тогда
				Ответ = Макс(Ответ, ФоновоеЗаданиеВыполнить(Регламент.Метаданные.ИмяМетода, Регламент.Метаданные.Синоним));
			КонецЕсли;
		КонецЦикла;
	Иначе
		Регламент	= Метаданные.РегламентныеЗадания.Найти(Ключ);
		Ответ		= ФоновоеЗаданиеВыполнить(Регламент.ИмяМетода, Регламент.Синоним);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Процедура ИмпортСтроки2(сИгрок, Слово, Параметры)
	...
	Тогда
		сИгрок.Амплуа = Амплуа(Слово, Параметры.ВидСпорта);
	КонецЕсли;
КонецПроцедуры

Функция ИмпортЧемпионатПоСтроке2(Текст, Чемпионат) Экспорт
	Итого	= 0;
	Матч	= Новый Структура("Дата,Клубы,Команды,Тур", Дата(1, 1, 1), Новый Массив, Новый Массив);
	Исключения	= Разделить("матч,клуб,мячи,очки,метка,группа,счет,финал", ",");
	Итератор	= 0;
	Разрыв		= Справочники.Туры.КоличествоМакс(Чемпионат.Команды.Количество());
	Команды	= Новый Соответствие;
	
	Для Каждого Клуб Из Чемпионат.Команды Цикл
		Команды.Вставить(Клуб.Команда.Наименование, Клуб.Команда);
	КонецЦикла;
	Искать	= Истина;
	Для Каждого Слова Из Разделить(Текст, "¶") Цикл
		Если НЕ ЗначениеЗаполнено(Матч.Тур) Тогда
			Для Каждого Слово Из Разделить(Слова, Символ(9)) Цикл
				Если НЕ ЗначениеЗаполнено(Матч.Тур) Тогда
					Если Исключения.Найти(НРег(Слово)) = Неопределено Тогда
						Код = СтрокаКакЧисло(Слово);
						Если Код > 0 Тогда
							Матч.Тур = ТурНайти(Код, Чемпионат);
							Если НЕ ЗначениеЗаполнено(Матч.Тур) Тогда
								Прервать;
							КонецЕсли;
						Иначе
							Исключения.Добавить(НРег(Слово));
						КонецЕсли;
					КонецЕсли;
				ИначеЕсли НЕ ЗначениеЗаполнено(Матч.Дата) Тогда
					Матч.Дата = СтрокаКакДата(Лев(СокрЛ(Слово), 10));
					Если Матч.Дата > НачалоДня(ТекущаяДатаСеанса()) Тогда
						Матч.Дата = Дата(1, 1, 1);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли Матч.Клубы.Количество() = 0 Тогда
			Разрыв	= СтрНайти(Слова, " – ");
			Если Разрыв = 0 Тогда
				Матч	= Новый Структура("Дата,Клубы,Команды,Тур", Дата(1, 1, 1), Новый Массив, Новый Массив);
			Иначе
				Для Итератор = 0 По 1 Цикл
					Слово = ?(Итератор = 0, Лев(Слова, Разрыв - 1), Сред(Слова, Разрыв + 3));
					Клуб = Команды.Получить(СокрЛП(Слово));
					Если НЕ ЗначениеЗаполнено(Клуб) Тогда
						Клуб = КомандаНайти(Слово, Чемпионат);
						Если ЗначениеЗаполнено(Клуб) Тогда
							Команды.Вставить(СокрЛП(Слово), Клуб);
						Иначе
							Сообщить(Слово);
						КонецЕсли;
					КонецЕсли;
					...
					КонецЕсли;
				КонецЦикла;
				Если Матч.Клубы.Количество() = 1 Тогда Прервать; КонецЕсли;
			КонецЕсли;
		ИначеЕсли Матч.Клубы.Количество() = 2 Тогда
			Если СтрНайти(Слова, "– : –") = 0 Тогда
				Цифры		= Разделить(СтрУдалить(Слова, Разделить("ДВ Т")), ":");
				Итератор	= 0;
				Для Каждого Клуб Из Матч.Клубы Цикл
					Матч.Команды.Добавить(Новый Структура("Команда,КоличествоГолов,Пенальти", Клуб, СтрокаКакЧисло(Цифры.Получить(Итератор)), 0));
					Итератор = Итератор + 1;
				КонецЦикла;
				Если Матч.Команды.Количество() = 2 Тогда
					Ссылка = МатчНайти(Матч.Клубы, Матч.Дата, Матч.Тур);
					Если ЗначениеЗаполнено(Ссылка) Тогда
						Док	= Ссылка.ПолучитьОбъект();
						Док.УстановитьПометкуУдаления(Ложь);
						ОбщегоНазначения.УстановитьЗначение(Док.Дата, Матч.Дата);
						Если Док.Команды.Количество() = 2 Тогда
							Итератор	= 0;
							Для Каждого Элемент Из Док.Команды Цикл
								Команда = Матч.Команды.Получить(Итератор);
								ОбщегоНазначения.УстановитьЗначение(Элемент.Команда, Команда.Команда);
								ОбщегоНазначения.УстановитьЗначение(Элемент.КоличествоГолов, Команда.КоличествоГолов);
								ОбщегоНазначения.УстановитьЗначение(Элемент.Пенальти, Команда.Пенальти);
								Итератор = Итератор + 1;
					...
					Иначе
						...
						КонецЦикла;
					КонецЕсли;
					ОбщегоНазначения.УстановитьЗначение(Док.Тур, Матч.Тур);
					Если Док.Модифицированность() Или НЕ Док.Проведен Тогда
						Если Док.ПроверитьЗаполнение() Тогда
							Попытка
								Док.ДополнительныеСвойства.Вставить("Импорт", Истина);
...
Функция НовоеСтарье(Новое, Старье, Параметры)
	...
	Для Каждого ТекСтрока Из Старье Цикл
		...
		ИначеЕсли Номер > 0 И Номер <> ТекСтрока.Номер Тогда
			Коэф	= 0.001;
			Запись = РегистрыСведений.ПеремещенияИгроков.СоздатьМенеджерЗаписи();
		...
		КонецЕсли;
		Если Коэф > 0 И ОбъектЗаписать(Запись) Тогда
			Итого = Итого + Коэф;
		КонецЕсли;
	КонецЦикла;
	Возврат Итого;
...
Функция ГодПолучить(Гиперссылка, Лимит=2000) Экспорт
	Ответ = 0;
	Для Каждого Слово Из Разделить(Гиперссылка, "/") Цикл
		Ответ = СтрокаКакЧисло(Слово);
		Если Ответ >= Лимит Тогда Прервать; КонецЕсли;
	КонецЦикла;
	Возврат Ответ;
КонецФункции

Функция ДатаНачалаСезона(Гиперссылка="", Команда)
	Год = ГодПолучить(Гиперссылка, 2007);
	Если Год > 0 Тогда 
		Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
		                      |	НАЧАЛОПЕРИОДА(Чемпионаты.ДатаНачала, МЕСЯЦ) КАК ДатаНачала
		                      |ИЗ
		                      |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
		                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Чемпионаты КАК Чемпионаты
		                      |		ПО ЧемпионатыКоманды.Ссылка = Чемпионаты.Ссылка
		                      ...
		                      |	И Чемпионаты.ПометкаУдаления = ЛОЖЬ
		                      |	И (Чемпионаты.Владелец.Страна.Ссылка ЕСТЬ NULL) = ЛОЖЬ
		                      |
		                      |УПОРЯДОЧИТЬ ПО
	...
	Если ЗначениеЗаполнено(Чемпионат) Тогда
		Возврат Дата(Год(ТекущаяДатаСеанса()), Месяц(Чемпионат.ДатаНачала), 1);
	КонецЕсли;
	
	Возврат Дата(Год(ТекущаяДатаСеанса()), Месяц(ТекущаяДатаСеанса()), 1);
КонецФункции
...
Функция ИмпортИгроков(Гиперссылка, Команда, КомОбъект=Неопределено) Экспорт
	Если ГодПолучить(Гиперссылка) < 2022 Тогда
		Возврат ИмпортИгроковНаСервере2(Гиперссылка, Команда);
	КонецЕсли;
	Итого		= 0;
	...
	Старье		= ИгрокиКоманды(Команда, Параметры.ДатаНачала);
	СтарьеМасса	= Старье.ВыгрузитьКолонку("Игрок");
	Новое		= Новый Соответствие;
	ВидСпорта	= Команда.Владелец;

	//Если ГодПолучить(Гиперссылка) < 2022 Тогда
	//	Слова	= ПолучитьЭлементыПоИмени(Гиперссылка,, КомОбъект);
	//Иначе
		Слова	= ПолучитьЭлементыПоИмени(Гиперссылка, "font", КомОбъект);
	//КонецЕсли;
	сИгрок	= ИмпортНаполнитель2(Команда, Параметры.ДатаНачала);
	Для Каждого Слово Из Слова Цикл
		Если СтрСравнить(Слово, "Соперник") = 0 Тогда
			Прервать;
		ИначеЕсли СтрНайти2(Слово, "покинули клуб") > 0 Тогда
			Прервать;
		...
		ИначеЕсли СтрСравнить(Слово, "Вратари") = 0 Тогда
			сИгрок.Амплуа = Амплуа("Вратарь", ВидСпорта);
		ИначеЕсли СтрСравнить(Слово, "Защитники") = 0 Тогда
			сИгрок.Амплуа = Амплуа("Защитник", ВидСпорта);
		ИначеЕсли СтрСравнить(Слово, "Полузащитники") = 0 Тогда
			сИгрок.Амплуа = Амплуа("Полузащитник", ВидСпорта);
		ИначеЕсли СтрСравнить(Слово, "Нападающие") = 0 Тогда
			сИгрок.Амплуа = Амплуа("Нападающий", ВидСпорта);
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгрок.Страна) Тогда
			...
			Если ЗначениеЗаполнено(сИгрок.Игрок) Тогда
				...
				Если Год(Параметры.ДатаНачала) >= Год(ТекущаяДатаСеанса()) Или СтарьеМасса.Найти(сИгрок.Игрок) = Неопределено Тогда
			...
			Если ?(ЗначениеЗаполнено(сИгрок.Команда), сИгрок.Команда <> КомандаИгрока(сИгрок.Игрок, сИгрок.Период), Команда <> КомандаИгрока(сИгрок.Игрок, сИгрок.Период)) Тогда
				Запись = РегистрыСведений.ПеремещенияИгроков.СоздатьМенеджерЗаписи();
...
Функция ИмпортИгроковПоСтроке(Гиперссылка, Команда) Экспорт
	...
	Параметры.Вставить("ВидСпорта",		ВидСпорта(Команда));
	Старье		= ИгрокиКоманды(Команда, Параметры.ДатаНачала);
	СтарьеМасса	= Старье.ВыгрузитьКолонку("Игрок");
	Новое		= Новый Соответствие;
	сИгрок		= ИмпортНаполнитель2(Команда, Параметры.ДатаНачала);
	Для Каждого Слово Из Разделить(Гиперссылка, "¶") Цикл
		...
		Тогда
			...
			Если ЗначениеЗаполнено(сИгрок.Игрок) Тогда
				НовыйИгрок = сИгрок.Игрок.ПолучитьОбъект();
				Если Год(Параметры.ДатаНачала) >= Год(ТекущаяДатаСеанса()) Или СтарьеМасса.Найти(сИгрок.Игрок) = Неопределено Тогда
					ОбщегоНазначения.УстановитьЗначение(НовыйИгрок.Амплуа, сИгрок.Амплуа, Истина);
					ОбщегоНазначения.УстановитьЗначение(НовыйИгрок.Страна, сИгрок.Страна, Истина);
				КонецЕсли;
			Иначе
				НовыйИгрок = Справочники.Игроки.СоздатьЭлемент();
				ЗаполнитьЗначенияСвойств(НовыйИгрок, сИгрок);
...
Процедура КартинкаУстановить(ОбластьМакета, Команда, Принудительно=Ложь) Экспорт
	Если Принудительно Или НаСервереПовторно.ВыводитьЛоготипыКоманд() Тогда
		Картинка = ИзображениеПолучить(Команда, Истина);
		Если НЕ Картинка.Вид = ВидКартинки.Пустая Тогда
			ОбластьМакета.ТекущаяОбласть.Картинка = Картинка;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Функция ИмпортТрансферов(Гиперссылка, Ссылка) Экспорт
	Слова	= ПолучитьЭлементыПоИмени(Гиперссылка);
	Возврат ИмпортТрансферовПоСтроке(Слова, Ссылка);
КонецФункции

Функция ИмпортТрансферовПоСтроке(Слова, Игрок) Экспорт
	Отказ	= Ложь;
	Ответ	= 0;
	Пред	= "";
	Данные	= Новый Массив;
	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли СтрЧислоСтрок(Слово) > 1 Тогда
			Продолжить;
		ИначеЕсли СтрСравнить(Пред, "Полное имя") = 0 Тогда
			ИмпортИгрокПолноеИмяОбновить(Игрок, Слово);
		ИначеЕсли СтрНайти(Слово, "Полное имя") = 1 Тогда
			ИмпортИгрокПолноеИмяОбновить(Игрок, СокрЛП(СтрУдалить(Слово, "Полное имя")));
		ИначеЕсли СтрЧислоВхождений(Слово, """") > 1 Тогда
			Запись	= Новый Структура("Игрок,Амплуа,Период,Команда", Игрок, Игрок.Амплуа, Дата(1,1,1), Справочники.Команды.ПустаяСсылка());
			Если ДанныеПолучить(Запись, Слово) Тогда
				Если Данные.Количество() = 0 Тогда
					Данные.Вставить(0, Запись);
					КлубП	= Запись.Команда;
				Иначе
					Если КлубП = Запись.Команда Тогда
						Данные.Получить(0).Период = Запись.Период;
					Иначе
						Данные.Вставить(0, Запись);
						КлубП = Запись.Команда;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли СтрСравнить(Лев(Слово, 14), "Предупреждения") = 0 Тогда
			Ответ	= Ответ + 1;
		ИначеЕсли СтрСравнить(Лев(Слово, 8), "Удаления") = 0 Тогда
			Ответ	= Ответ + 2;
		ИначеЕсли Данные.Количество() = 0 И Ответ > 3 Тогда
			Отказ	= Истина;
			Прервать;
		КонецЕсли;
		Пред	= Слово;
	КонецЦикла;
	Возврат ?(Отказ, 0, ПеремещенияЗаписать(Данные) > 0);
КонецФункции

Процедура ИмпортИгрокПолноеИмяОбновить(Ссылка, ПолноеИмя)
	Если СтрСравнить(Ссылка.Наименование, ПолноеИмя) <> 0
	И СтрНайти(Ссылка.Комментарий, ПолноеИмя) = 0
	Тогда
		Игрок	= Ссылка.ПолучитьОбъект();
		Игрок.Комментарий	= ПолноеИмя + Разрыв(Истина) + Игрок.Комментарий;
		Игрок.Записать();
	КонецЕсли;
КонецПроцедуры

Функция ДанныеПолучить(Данные, Слово)
	Слова	= Лев(Слово, СтрНайти(Слово, """") - 1);
	Клуб	= СтрУдалить(Сред(Слово, СтрНайти(Слово, """")), Разделить(""" :"));
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 2
	      	               |	НАЧАЛОПЕРИОДА(Чемпионаты.ДатаНачала, МЕСЯЦ) КАК Период,
	      	               |	ЧемпионатыКоманды.Команда КАК Команда
	      	               |ИЗ
	      	               |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
	      	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Чемпионаты КАК Чемпионаты
	      	               ...
	      	               |			ПО Чемпионаты.Владелец = Лига.Ссылка
	      	               |		ПО ЧемпионатыКоманды.Ссылка = Чемпионаты.Ссылка
	      	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Команды КАК Клубы
	      	               |		ПО ЧемпионатыКоманды.Команда = Клубы.Ссылка
	      	               |ГДЕ
	      	               |	Чемпионаты.ПометкаУдаления = ЛОЖЬ
	      	               |	И (Чемпионаты.Наименование ПОДОБНО &Комментарий
	      	               |			ИЛИ Чемпионаты.Комментарий ПОДОБНО &Комментарий
	      	               |			ИЛИ Лига.Наименование ПОДОБНО &Комментарий
	      	               |			ИЛИ Лига.Комментарий ПОДОБНО &Комментарий)
	      	               |	И Чемпионаты.Владелец.ОлимпийскаяСистема = ЛОЖЬ
	      	               |	И (Клубы.Наименование ПОДОБНО &Клуб
	      	               |			ИЛИ Клубы.Комментарий ПОДОБНО &Клуб)");
	мСлова	= Разделить(Слова);
	Год		= мСлова.Получить(мСлова.ВГраница());
	Если СтрокаКакЧисло(Год) > 2000 Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 2
		               ...
		               |		ПО ЧемпионатыКоманды.Команда = Клубы.Ссылка
		               |ГДЕ
		               |	ГОД(Чемпионаты.ДатаОкончания) = &Год
		               |	И Чемпионаты.ПометкаУдаления = ЛОЖЬ
		               |	И (Чемпионаты.Наименование ПОДОБНО &Комментарий
		               |			ИЛИ Чемпионаты.Комментарий ПОДОБНО &Комментарий
		               |			ИЛИ Лига.Наименование ПОДОБНО &Комментарий
		               ...
		               |	И (Клубы.Наименование ПОДОБНО &Клуб
		               |			ИЛИ Клубы.Комментарий ПОДОБНО &Клуб)";
		Запрос.УстановитьПараметр("Год",				СтрокаКакЧисло(Год));
		Запрос.УстановитьПараметр("Комментарий",		Обернуть(СтрУдалить(Слова, Год)));
	Иначе
		Запрос.УстановитьПараметр("Комментарий",		Обернуть(Слова));
	КонецЕсли;
	Запрос.УстановитьПараметр("Клуб",				Обернуть(Клуб));
	Выборка	= Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() И Выборка.Количество() = 1 Тогда
		ЗаполнитьЗначенияСвойств(Данные, Выборка);
	Иначе
		ЗаполнитьЗначенияСвойств(Данные, Новый Структура("Период,Команда", Дата(1,1,1), Справочники.Команды.ПустаяСсылка()));
	КонецЕсли;
	Возврат ЗначениеЗаполнено(Данные.Период);
КонецФункции

Функция ПеремещенияЗаписать(Данные)
	Ответ	= 0;
	Для Каждого Новое Из Данные Цикл
		Отбор	= Новый Структура("Игрок", Новое.Игрок);
		Последнее = РегистрыСведений.ПеремещенияИгроков.ПолучитьПоследнее(Новое.Период, Отбор);
		Если Последнее.Команда <> Новое.Команда Тогда
			Запись = РегистрыСведений.ПеремещенияИгроков.СоздатьМенеджерЗаписи();
			ЗаполнитьЗначенияСвойств(Запись, Новое);
			Если ОбъектЗаписать(Запись) Тогда
				Ответ = Ответ + 1;
			КонецЕсли;
		КонецЕсли;
