...
Функция ЧемпионатПустой(Чемпионат) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
...
Функция ТурнирнаяТаблица(Чемпионат, НомерСтроки=0) Экспорт
	...
	Если ЧемпионатПустой(Чемпионат) Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ЧемпионатыКоманды.Команда КАК Команда,
		               |	0 КАК КоличествоИгр,
...
Функция МатчСохранить(сМатч) Экспорт
	Ответ	= Ложь;
	...
	КонецЕсли;
	Если Док.Команды.НайтиСтроки(Новый Структура("Команда,КоличествоГолов", сМатч.Команда1, сМатч.Команды.Получить(0).КоличествоГолов)).Количество() = 0
		Или Док.Команды.НайтиСтроки(Новый Структура("Команда,КоличествоГолов", сМатч.Команда2, сМатч.Команды.Получить(1).КоличествоГолов)).Количество() = 0
		Тогда
		Док.Команды.Очистить();
		Для Каждого ТекЭлемент Из сМатч.Команды Цикл
...
Функция ТурИлиЧемпионат(Чемпионат)
	Если ТипЗнч(Чемпионат) = Тип("СправочникСсылка.Чемпионаты") Тогда
		Тур		= ?(НЕ Чемпионат.Владелец.ОлимпийскаяСистема И НаСервереПовторно.ВыводитьТурВместоЧемпионата(), ТурТекущий(Чемпионат), Чемпионат);
	Иначе
		Тур		= ?(НЕ Чемпионат.ОлимпийскаяСистема И НаСервереПовторно.ВыводитьТурВместоЧемпионата(), ТурТекущий(Чемпионат), Чемпионат);
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(Гиперссылка) Тогда
		...
		Для Каждого Слово Из Слова Цикл
			...
			Если Год >= 2007 И Год <= 1 + Год(ТекущаяДатаСеанса()) Тогда
				...
				Выборка = Запрос.Выполнить().Выбрать();
				Если Выборка.Следующий() Тогда
					Возврат НачалоМесяца(Выборка.ДатаНачала);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
