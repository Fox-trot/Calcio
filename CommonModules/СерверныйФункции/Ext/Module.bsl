...
Функция МатчНайти(Команды, Дата)
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      |	Матч.Ссылка КАК Ссылка,
	                      |	ВЫБОР
...
Функция СобытияНаСегодня(Команда=Неопределено) Экспорт
	Ответ	= Новый Структура("Оповещения,Старье", Новый Массив, НастройкиВосстановить());
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ?(Авто, Ответ.Старье.Найти(Выборка.Ссылка) = Неопределено, Истина) Тогда
			Ответ.Оповещения.Добавить(Новый Структура("Ссылка,Дата,Картинка,Ключ",
				Выборка.Ссылка,
			...
			Если Авто Тогда
				Ответ.Старье.Добавить(Выборка.Ссылка);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Если Авто Тогда
		НастройкиСохранить(Ответ.Старье);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
