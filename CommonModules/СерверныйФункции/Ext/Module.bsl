...
Функция ЧемпионатыТекущие(Все=Ложь) Экспорт
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               ...
	      	               |		ПО Чемпионаты.Ссылка = Итоги.Чемпионат
	      	               |			И (Итоги.Место = 1)
	      	               |ГДЕ
	      	               |	(Чемпионаты.Владелец, Чемпионаты.ДатаОкончания) В
	      	               |			(ВЫБРАТЬ
	      	               |				ТурнирнаяТаблица.Тур.Владелец.Владелец КАК Ссылка,
	      	               |				МАКСИМУМ(ТурнирнаяТаблица.Тур.Владелец.ДатаОкончания)
	      	               |			ИЗ
	      	               |				РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	      	               |			ГДЕ
	      	               ...
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	Итоги.Чемпионат ЕСТЬ NULL УБЫВ,
	      	               |	Ссылка
	      	               |АВТОУПОРЯДОЧИВАНИЕ");
	Если Все Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция МатчНайти(Команды, Дата, Тур=Неопределено) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Матч.Команды КАК Противник
	                      |		ПО Матч.Ссылка = Противник.Ссылка
	                      |ГДЕ
	                      ...
	                      |	Проведен УБЫВ,
	                      |	ПометкаУдаления,
	                      |	НАЧАЛОПЕРИОДА(Матч.Дата, ДЕНЬ) >= &Дата УБЫВ");
	Если Команды.Количество() = 2 Тогда
		...
		Если ЗначениеЗаполнено(Тур) Тогда
			Запрос.УстановитьПараметр("Тур",		Тур);
		Иначе
			Запрос.Текст = СтрУдалить(Запрос.Текст, "Матч.Тур = &Тур УБЫВ,");
		КонецЕсли;
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			Возврат Выборка.Ссылка;
...
Функция ИмпортИгрПоСтроке2(Текст, Тур=Неопределено) Экспорт
	Ответ	= 0;
	Матч	= Новый Структура("Дата,Клубы,Команды,Тур", Дата(1, 1, 1), Новый Массив, Новый Массив, Тур);
	Искать	= Истина;
	КлубИмя	= "";
	...
	Для Каждого Слова Из Разделить(СтрЗаменить(Текст, "¶", " "), "	") Цикл
		Если ЗначениеЗаполнено(Матч.Дата) Тогда
			...
			Если Матч.Команды.Количество() = 2 Тогда
				...
				КонецЕсли;
				Если ЗначениеЗаполнено(Тур) Тогда
					ОбщегоНазначения.УстановитьЗначение(Док.Тур, Тур);
				КонецЕсли;
				Если Док.Модифицированность() Или НЕ Док.Проведен Тогда
					Если Док.ПроверитьЗаполнение() Тогда
						Попытка
							Док.ДополнительныеСвойства.Вставить("Импорт", Истина);
							Док.Записать(?(Матч.Дата > НачалоДня(ТекущаяДатаСеанса()), РежимЗаписиДокумента.Запись, РежимЗаписиДокумента.Проведение));
							Ответ = Ответ + 1;
						Исключение
