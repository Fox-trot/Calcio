...
Функция ИмпортЧемпионата(Гиперссылка, Чемпионат) Экспорт
	...
	Если СтрНайти2(Гиперссылка, "games") = 0 Тогда
		...
		КонецЦикла;
	Иначе
		//https://footb all.kulichki. net/france/2024/games.htm
...
Процедура Зачистить(Виктим, ЭтоСобытие=Ложь, ОставитьДату=Ложь)
	ЗаполнитьЗначенияСвойств(Виктим, Новый Структура("ЭтоСобытие,Счет,Гол1,Гол2,КоличествоЗрителей,Проведен,Команды,Комментарий,Комментарий2,ТурКод,Стадион,Команда1,Команда2", ЭтоСобытие, "", -1, -1, 0, Истина, Новый Массив, "", "", 0));
