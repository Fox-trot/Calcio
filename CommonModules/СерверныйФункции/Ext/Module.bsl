...
Функция ИмпортНаполнитель(Тур)
	Возврат Новый Структура("Тур,Дата,Счет,Гол1,Гол2,Пенальти1,Пенальти2,КоличествоЗрителей,Проведен,Комментарий,Команда1,Команда2,Стадион", Тур, Дата(1,1,1), "", -1, -1, 0, 0, 0, Истина, "");
КонецФункции
...
Процедура ИмпортИгр(ОдинРаз=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ГОД(ТурнирнаяТаблица.Тур.Владелец.ДатаОкончания) КАК Год,
	                      |	МАКСИМУМ(ТурнирнаяТаблица.Тур) КАК Тур,
	                      |	МАКСИМУМ(ТурнирнаяТаблица.Тур.Код) КАК ТурКод,
	                      |	ЦЕЛ(КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Регистратор) / ЦЕЛ(КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЧемпионатыКоманды.НомерСтроки) / 2)) КАК Количество
	                      |ИЗ
	                      |	РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОдинРаз Тогда
			Если Туры.Следующий() Тогда
				Гиперссылка	= ИмпортУРЛ(Туры.Тур);
				ИмпортИгрПоГиперссылке(Гиперссылка, Туры.Тур);
			КонецЕсли;
			Прервать;
		Иначе
			Пока Туры.Следующий() Цикл
				Гиперссылка	= ИмпортУРЛ(Туры.Тур);
				Если НЕ ИмпортИгрПоГиперссылке(Гиперссылка, Туры.Тур) Тогда
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
