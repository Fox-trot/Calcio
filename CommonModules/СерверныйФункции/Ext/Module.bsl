...
Функция ТурнирнаяТаблица2(Чемпионат) Экспорт
	...
	Если НаСервереПовторно.ПоказыватьПредстоящиеМатчи() Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		МАКСИМУМ(МатчКоманды.Ссылка.Проведен) КАК Проведен2,
		               |		МИНИМУМ(МатчКоманды.Ссылка.Проведен) КАК Проведен
		               |	ИЗ
		               |		Документ.Матч.Команды КАК МатчКоманды
		               ...
		               |	СГРУППИРОВАТЬ ПО
		               |		Противник.Команда,
		               |		МатчКоманды.Ссылка.Тур,
		               |		МатчКоманды.Ссылка.Тур.Код,
...
Функция ИмпортИгрПоСтроке2(Текст, Тур=Неопределено) Экспорт
	...
	Искать	= Истина;
	КлубИмя	= "";
	Уникум = ЗначениеЗаполнено(Тур) И НЕ Тур.Владелец.Владелец.ОлимпийскаяСистема;
	Для Каждого Слова Из Разделить(СтрЗаменить(Текст, "¶", " "), "	") Цикл
		Если ЗначениеЗаполнено(Матч.Дата) Тогда
			...
			КонецЦикла;
			Если Матч.Команды.Количество() = 2 Тогда
				Ссылка = МатчНайти(Матч.Клубы, Матч.Дата, ?(Уникум, Тур, Неопределено));
...
Функция ИмпортТрансферовПоСтроке(Слова, Игрок) Экспорт
	...
	Для Каждого Слово Из Слова Цикл
		...
		Пред	= Слово;
	КонецЦикла;
	Ответ	= ?(Отказ, 0, ПеремещенияЗаписать(Данные));
	Возврат (Ответ > 0);
КонецФункции
