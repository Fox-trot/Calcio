...
Функция ИгрокНайти(Наименование, Параметр=Неопределено) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	Игроки.Комментарий ПОДОБНО &Комментарий УБЫВ,
...
Функция ИмпортИгрПоГиперссылке(Гиперссылка, Тур) Экспорт
	...
	Для Каждого Слово Из Слова Цикл
		Если Итого > 0 И СтрНайти2(Слово, "таблица") > 0 Тогда		// финито
			Прервать;
...
Функция ИмпортИгрПоГиперссылке2010(Гиперссылка, Тур) Экспорт
	Итого	= 0;

	Чемпионат	= Тур.Владелец;
	ВсегоИгр	= Цел(Чемпионат.Команды.Количество() / 2);
	Дата		= Дата(1,1,1);
	Исключения	= Разделить("онлайн трансляция.", ",");
	сИгра	= Новый Структура("Тур,Дата,Счет,Гол1,Гол2,КоличествоЗрителей,Проведен,Команда1,Команда2,Стадион", Тур);
	Зачистить(сИгра);
	Слова	= ПолучитьЭлементыПоИмени(Гиперссылка);
	Для Каждого Слово Из Слова Цикл
		Если Итого >= ВсегоИгр И СтрНайти2(Слово, "таблица") > 0 Тогда
			Прервать;
		ИначеЕсли Исключения.Найти(НРег(Слово)) <> Неопределено Тогда
			Продолжить;
		ИначеЕсли СтрДлина(Слово) < 3 Тогда
			Если Итого > 0 И (Итого - Цел(Итого)) * 100 = ВсегоИгр Тогда
				Прервать;
			Иначе
				Продолжить;
			КонецЕсли;
		...
		Если СтрокаПохожаНаДату(Слово) Тогда
			...
			Если Год(сИгра.Дата) >= 2000 Тогда
				Дата = сИгра.Дата;
			ИначеЕсли Год(Дата) >= 2000 Тогда
				сИгра.Дата = Дата;
			КонецЕсли;
		ИначеЕсли Год(Дата) < 2 Тогда
			//
...
Функция ИмпортИгрПоСтроке(Текст, Тур) Экспорт
	...
	Для Каждого Слово Из Слова Цикл
		...
		Если ЗначениеЗаполнено(сИгра.Дата)
		И (сИгра.Гол1 >= 0 И сИгра.Гол2 >= 0 Или сИгра.Гол1 < 0 И сИгра.Гол2 < 0)
		И ЗначениеЗаполнено(сИгра.Команда1) И ЗначениеЗаполнено(сИгра.Команда2)
		Тогда
			Итого = Итого + 0.01;
			Команды	= Новый Массив;
			...
			Если Док.Модифицированность() Тогда
				Если ?(сИгра.Проведен, Док.ПроверитьЗаполнение(), Истина) Тогда
					...
					Исключение
						Сообщить(ОписаниеОшибки());
						Логировать("СерверныйФункции.ИмпортИгрПоСтроке", ОписаниеОшибки(), Текст);
						Прервать;
					КонецПопытки;
				КонецЕсли;
			КонецЕсли;
...
Функция КомандыОпределить(Слова)
	...
	Если СтрЧислоСтрок(Слова) = 2 Тогда
		Для НомерСтроки = 1 По СтрЧислоСтрок(Слова) Цикл
			...
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли СтрНайти(Слова, "-") > 0 Тогда
		Для Каждого Слово Из Разделить(СтрЗаменить(СтрЗаменить(Слова, ".", "="), "-", "="), "=") Цикл
...
Функция ГолыОпределить(сИгра, Слова)
	Если СтрЧислоВхождений(Слова, ":") = 2 И СтрНайти(Слова, ")") > 0 Тогда
		...
		Если НЕ ПустаяСтрока(сИгра.Счет) Тогда
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли СтрЧислоВхождений(Слова, ":") = 1 И СтрНайти(Слова, ")") = 0 Тогда
		мСлова = Разделить(Слова, ":");
