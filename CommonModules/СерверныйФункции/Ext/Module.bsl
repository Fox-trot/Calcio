...
Функция ТурНеПоследний(Чемпионат)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |		ГДЕ
	                      |			ТурнирнаяТаблица.Тур.Владелец = &Чемпионат
	                      |			И ТурнирнаяТаблица.Регистратор ССЫЛКА Документ.Матч) КАК ТурнирнаяТаблица
...
Функция ТекстЗапросаТурнирнаяТаблица(НомерСтроки=0)
	Текст = "ВЫБРАТЬ
	        |	ЧемпионатыКоманды.Команда КАК Команда,
	        |	СУММА(ВЫБОР
	        ...
	        |	Команда
	        |АВТОУПОРЯДОЧИВАНИЕ";
	Возврат ?(НомерСтроки = 0, Текст, СтрУдалить(Текст, "И (ТурнирнаяТаблица.НомерСтроки = &НомерСтроки)"));
КонецФункции

Функция ТурнирнаяТаблица(Чемпионат, НомерСтроки=0) Экспорт
	Запрос = Новый Запрос(ТекстЗапросаТурнирнаяТаблица(НомерСтроки));
	Запрос.УстановитьПараметр("Чемпионат",		Чемпионат);
	Запрос.УстановитьПараметр("Команда",		МояКомандаПолучить());
	...
	Если НомерСтроки = 0 Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ТурнирнаяТаблица.Команда КАК Команда,
		               |	ТурнирнаяТаблица.Место КАК Место,
		               |	ТурнирнаяТаблица.КоличествоОчков КАК КоличествоОчков,
		               |	ТурнирнаяТаблица.КоличествоИгр КАК КоличествоИгр,
		               |	ТурнирнаяТаблица.КоличествоВыигрыш КАК Выигрыш,
		               |	ТурнирнаяТаблица.КоличествоНичья КАК Ничья,
		               |	ТурнирнаяТаблица.КоличествоПроигрыш КАК Проигрыш,
		               |	ТурнирнаяТаблица.КоличествоЗабито КАК КоличествоЗабито,
		               |	ТурнирнаяТаблица.КоличествоПропущено КАК КоличествоПропущено,
		               |	ТурнирнаяТаблица.КоличествоЗабито - ТурнирнаяТаблица.КоличествоПропущено КАК Разница
		               |ИЗ
		               |	РегистрСведений.ИтогиПромежуточные КАК ТурнирнаяТаблица
		               |ГДЕ
		               |	ТурнирнаяТаблица.Тур.Владелец = &Чемпионат
		               |	И ТурнирнаяТаблица.Тур.Код В
		               |			(ВЫБРАТЬ
		               |				МАКСИМУМ(ИтогиПромежуточные.Тур.Код)
		               |			ИЗ
		               |				РегистрСведений.ИтогиПромежуточные.СрезПоследних(, Тур.Владелец = &Чемпионат) КАК ИтогиПромежуточные)
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Место";
		Ризалт	= Запрос.Выполнить();
		Если НЕ Ризалт.Пустой() Тогда Возврат Ризалт.Выбрать(); КонецЕсли;
		Запрос.Текст = ТекстЗапросаТурнирнаяТаблица();
	КонецЕсли;
