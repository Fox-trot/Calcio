...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	...
	Если ТипЗнч(Параметр) = Тип("Структура") Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	Туры.Владелец.Владелец.ОлимпийскаяСистема = ИСТИНА
		               ...
		               |					ИЛИ Команды.Ссылка = &Команда)) КАК Тотал
		               |		ПО (ИСТИНА)
		               |ГДЕ
		               |	ВременнаяТаблица.ЭтоФинал = ЛОЖЬ
		               |	И (Стадионы.Владелец = &Команда
...
Функция ПоказыватьРазницуМячей() Экспорт
	Возврат Константы.ПоказыватьРазницуМячей.Получить() И НЕ ТурнирнаяТаблицаКакНаBBC();
КонецФункции
...
Процедура Зачистить(Виктим, ЭтоСобытие=Ложь)
	ЗаполнитьЗначенияСвойств(Виктим, Новый Структура("ЭтоСобытие,Дата,Счет,Гол1,Гол2,КоличествоЗрителей,Проведен,Команды,Комментарий,Комментарий2,ТурКод,Стадион,Команда1,Команда2", ЭтоСобытие, Дата(1,1,1), "", -1, -1, 0, Истина, Новый Массив, "", "", 0));
КонецПроцедуры

Функция ГолыОпределить(сИгра, Слова)
	Если СтрЧислоВхождений(Слова, ":") = 2 И СтрЗаканчиваетсяНа(Слова, ")") Тогда
...
Функция ОповещенияНаСегодня(ТолькоМатчи=Ложь) Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				Если Выборка.Дата = НачалоДня(Дата) Тогда
				...
				ИначеЕсли НачалоДня(Выборка.Сдвиг) = НачалоДня(Дата) Тогда
