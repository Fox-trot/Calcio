...
Функция ТурнирнаяТаблица(Чемпионат, НомерСтроки=0) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	                      |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	                      |				МИНИМУМ(Туры.ОлимпийскаяСистема) КАК ОлимпийскаяСистема
	                      |			ИЗ
	                      |				Справочник.Туры КАК Туры
	                      |			ГДЕ
	                      |				Туры.Владелец = &Чемпионат) КАК Туры
	                      |			ПО ТурнирнаяТаблица.Тур.ОлимпийскаяСистема = Туры.ОлимпийскаяСистема
