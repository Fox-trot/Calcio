...
Функция ТурнирнаяТаблица2(Чемпионат) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	ВЫБОР
	                      |			КОГДА ТурнирнаяТаблица.КоличествоМатчей = 1
	                      |				ТОГДА ТурнирнаяТаблица.КоличествоОчков >= ТурнирнаяТаблица.КоличествоОчков2
	...
	Если НаСервереПовторно.ПоказыватьПредстоящиеМатчи() Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		МатчКоманды1.Команда КАК Команда2,
		               |		ЕСТЬNULL(СУММА(Противник.КоличествоОчков), 0) КАК КоличествоОчков2,
		               |		СУММА(МатчКоманды1.КоличествоГолов) КАК КоличествоГолов2,
		               |		СУММА(МатчКоманды1.Пенальти) КАК Пенальти2,
		               ...
		               |		МАКСИМУМ(МатчКоманды.Ссылка.Проведен) КАК Проведен
		               |	ИЗ
		               |		Документ.Матч.Команды КАК МатчКоманды
		               |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
		               |			ПО МатчКоманды.Ссылка = ТурнирнаяТаблица.Регистратор
		               |				И МатчКоманды.Команда = ТурнирнаяТаблица.Команда
		               |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Матч.Команды КАК МатчКоманды1
		               |				ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК Противник
		               |				ПО МатчКоманды1.Ссылка = Противник.Регистратор
		               |					И МатчКоманды1.Команда = Противник.Команда
		               |			ПО МатчКоманды.Ссылка = МатчКоманды1.Ссылка
		               |				И МатчКоманды.НомерСтроки <> МатчКоманды1.НомерСтроки
		               |	ГДЕ
		               |		МатчКоманды.Ссылка.Тур.Владелец = &Чемпионат
		               |		И МатчКоманды.Ссылка.Тур.ОлимпийскаяСистема = ИСТИНА
		               |	
		               |	СГРУППИРОВАТЬ ПО
		               |		МатчКоманды1.Команда,
...
Функция МатчиПоследние5(Команда, Чемпионат, Дата=Неопределено, Сколько=5, НомерСтроки=0) Экспорт
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 5
	      	               |	ТурнирнаяТаблица.Период КАК Период,
	      	               |	ТурнирнаяТаблица.КоличествоОчков КАК КоличествоОчков,
	      	               |	ТурнирнаяТаблица.Регистратор КАК Регистратор
	      	               ...
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	Период");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "5", ФорматЧГ(Макс(1, Сколько)));
