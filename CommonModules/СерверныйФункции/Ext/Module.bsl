...
Функция ОповещенияНаСегодня() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Дата >= Дата Тогда
			Туча.Дата		= Формат(Выборка.Дата + Часов(6), "ДФ='d MMMM HH:mm'");
			Если Выборка.Минута < 60 Тогда
				Туча.Дата		= "Через " + СтрокаСЧислом(";%1 минуту;%1 минуты;%1 минут;%1 минут;", Выборка.Минута, ВидЧисловогоЗначения.Количественное);
			ИначеЕсли Выборка.Час <= 12 Тогда
				Туча.Дата		= "Через " + СтрокаСЧислом(";%1 час;%1 часа;%1 часов;%1 часов;", Выборка.Час, ВидЧисловогоЗначения.Количественное);
			КонецЕсли;
		Иначе
			Туча.Дата		= "Сегодня";
		КонецЕсли;
