...
Функция СтатистикаИгр(Знач Команда, Знач Чемпионат=Неопределено, Период=Неопределено) Экспорт
	...
	Ответ.Вставить("Побед",				0);
	Ответ.Вставить("Ничьих",			0);
	Ответ.Вставить("Проигрышей",		0);
	Ответ.Вставить("Результативность",	0);
	МатчИтого	= 0;
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	И ТурнирнаяТаблица.Период МЕЖДУ &ДатаН И &ДатаК
	...
	Запрос.УстановитьПараметр("Команда",	Команда);
	Запрос.УстановитьПараметр("Чемпионат",	Чемпионат);
	Запрос.УстановитьПараметр("ДатаН",		НачалоМесяца(ДобавитьМесяц(?(Период=Неопределено, ТекущаяДатаСеанса(), Период), -12)));
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КоличествоОчков = 0 Тогда
			Ответ.Вставить("Проигрышей", Выборка.Количество);
		ИначеЕсли Выборка.КоличествоОчков = 1 Тогда
			Ответ.Вставить("Ничьих", Выборка.Количество);
		Иначе
			Ответ.Вставить("Побед", Выборка.Количество);
		КонецЕсли;
		Ответ.Вставить("Результативность", Ответ.Получить("Результативность") + Выборка.КоличествоГолов);
		МатчИтого	= МатчИтого + Выборка.Количество;
	КонецЦикла;
	Если МатчИтого > 0 И Ответ.Получить("Результативность") > 0 Тогда
		Ответ.Вставить("Средняя результативность", Окр(Ответ.Получить("Результативность") / МатчИтого, 1));
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Функция Страна(Наименование) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
