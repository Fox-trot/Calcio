...
Функция ОповещенияНаСегодня(ТолькоМатчи=Ложь) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Туча	= Новый Структура("Повторить,Ссылка,Картинка,Ключ,КлючУникальности,Дата", Ложь, Выборка.Представление, Лого, ПолучитьНавигационнуюСсылку(Выборка.Ссылка), Строка(Выборка.Ссылка.УникальныйИдентификатор()));
