...
Функция КомандыЛиги(Лига, Топ6=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	ЧемпионатыКоманды.Ссылка В(&Ссылка)");
	Если Топ6 Тогда
...
Функция МояКомандаПолучить() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(МояКоманда) Тогда
		МояКоманда	= КомандаНайти("Арсенал", СтранаНайти("ENG"));
	КонецЕсли;
	Возврат МояКоманда;
КонецФункции
...
Функция ИмпортИгрПоГиперссылке(Гиперссылка, Тур) Экспорт
	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли СтрНайти("210", Лев(Слово, 1)) > 0 И СтрНайти(":00:30:15:45", Прав(Слово, 3)) > 0 Тогда
			сИгра.Проведен = Ложь;
