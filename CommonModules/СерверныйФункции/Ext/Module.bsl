...
Функция НайтиПоНаименованию(Наименование, СправочникИмя)
...
Функция ТурНеПоследний(Чемпионат)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	                      |			ЧемпионатыКоманды.Ссылка КАК Ссылка,
	                      |			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЧемпионатыКоманды.Команда) * 2 - ВЫБОР
	                      |				КОГДА 2 * ЦЕЛ(КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЧемпионатыКоманды.Команда) / 2) = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЧемпионатыКоманды.Команда)
	                      |					ТОГДА 2
	                      |				ИНАЧЕ 0
...
Функция ИмпортЧемпионата(Гиперссылка, Чемпионат) Экспорт
	...
	Предел	= Справочники.Туры.КоличествоМакс(Чемпионат.Команды.Количество());
