...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |				ТОГДА 0
	                      |			КОГДА Стадионы.Владелец = &Параметр
	                      |				ТОГДА 0
...
Функция ЭтоДерби(Матч) Экспорт
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ МатчКоманды.Команда) + КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Команды.Город) * 10 КАК Количество
	|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Возврат (Выборка.Следующий() И Выборка.Количество = 12);
КонецФункции
...
Функция СтранаНайти(Параметр) Экспорт
	...
	Если ТипЗнч(Параметр) = Тип("СправочникСсылка.Туры") Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |
		               |ИМЕЮЩИЕ
		               |	МИНИМУМ(Города.Владелец) = МАКСИМУМ(Города.Владелец)
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Порядок";
	КонецЕсли;
