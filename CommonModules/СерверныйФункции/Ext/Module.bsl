...
Функция ЧемпионатыТекущие(ОлимпийскаяСистема=Ложь) Экспорт
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	      	               ...
	      	               |	И Туры.Владелец.ДатаОкончания >= &ДатаК
	...
	Если ОлимпийскаяСистема Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |	И Туры.Владелец.ДатаНачала <= &ДатаН";
	КонецЕсли;
	Запрос.УстановитьПараметр("ДатаН",				НачалоДня(ТекущаяДатаСеанса()));
	Запрос.УстановитьПараметр("ДатаК",				ДобавитьМесяц(ТекущаяДатаСеанса(), -1));
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку(0);
КонецФункции
...
Функция ТурнирнаяТаблица(Чемпионат, НомерСтроки=0) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |			ПО (ТурнирнаяТаблица.Тур.Владелец = &Чемпионат)
	                      |				И ТурнирнаяТаблица.Тур.ОлимпийскаяСистема = Туры.ОлимпийскаяСистема
