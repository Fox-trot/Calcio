...
Функция ТурнирнаяТаблица2(Чемпионат) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	
	                      |	ИМЕЮЩИЕ
	                      |		СУММА(ТурнирнаяТаблица.КоличествоОчков) >= СУММА(Противник.КоличествоОчков)) КАК ТурнирнаяТаблица
	                      |ГДЕ
	                      |	ВЫБОР
	                      |			КОГДА ТурнирнаяТаблица.КоличествоОчков = ТурнирнаяТаблица.КоличествоОчков2
	                      |				ТОГДА ТурнирнаяТаблица.КоличествоГолов > ТурнирнаяТаблица.КоличествоГолов2
	                      |			КОГДА ТурнирнаяТаблица.КоличествоОчков = ТурнирнаяТаблица.КоличествоОчков2
	                      |				ТОГДА ТурнирнаяТаблица.Пенальти > ТурнирнаяТаблица.Пенальти2
	                      |			ИНАЧЕ ТурнирнаяТаблица.КоличествоОчков > ТурнирнаяТаблица.КоличествоОчков2
	                      |		КОНЕЦ
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	ТурнирнаяТаблица.ТурКод УБЫВ,
	                      |	Первая
	                      |ИТОГИ
	                      |	СУММА(КоличествоМатчей)
	                      |ПО
	                      |	Тур");
	Если ПоказыватьПредстоящиеМатчи() Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ТурнирнаяТаблица.Тур КАК Тур,
		               ...
		               |	ИЗ
		               |		Документ.Матч.Команды КАК МатчКоманды
		               |			ЛЕВОЕ СОЕДИНЕНИЕ Документ.Матч.Команды КАК Второй
		               |			ПО МатчКоманды.Команда = Второй.Команда
		               |				И МатчКоманды.Ссылка.Тур = Второй.Ссылка.Тур
		               |				И МатчКоманды.Ссылка.Дата < Второй.Ссылка.Дата
		               ...
		               |		И ЕСТЬNULL(Второй.НомерСтроки, 0) = 2) КАК МатчКоманды
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	ТурКод УБЫВ,
		               |	Проведен УБЫВ,
...
Функция ТурИлиЧемпионат(Чемпионат)
	Если ТипЗнч(Чемпионат) = Тип("СправочникСсылка.Чемпионаты") Тогда
		Тур		= ?(НЕ Чемпионат.Владелец.ОлимпийскаяСистема И ВыводитьТурВместоЧемпионата(), ТурТекущий(Чемпионат), Чемпионат);
	Иначе
		Тур		= ?(НЕ Чемпионат.ОлимпийскаяСистема И ВыводитьТурВместоЧемпионата(), ТурТекущий(Чемпионат), Чемпионат);
	КонецЕсли;
