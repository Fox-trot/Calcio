...
Функция ТурНайти(КодИлиНаименование, Чемпионат) Экспорт
	...
	Запрос.УстановитьПараметр("Код",			СтрокаКакЧисло(КодИлиНаименование));
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Ссылка;
	КонецЕсли;
...
Функция МатчиПоследние5(Команда, Чемпионат, Дата=Неопределено, Сколько=5) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаН",			НачалоДня(ДобавитьМесяц(?(Дата=Неопределено, ТекущаяДатаСеанса(), Дата), -6)));
	Запрос.УстановитьПараметр("ДатаК",			НачалоДня(?(Дата=Неопределено, ТекущаяДатаСеанса(), Дата)) - 1);
	Запрос.УстановитьПараметр("Чемпионат",		Чемпионат);
	Запрос.УстановитьПараметр("Команда",		Команда);
