...
Функция ТекстЗапросаТурнирнаяТаблица(НомерСтроки=0)
	Текст = "ВЫБРАТЬ
	        ...
	        |	ЛигаКоманды.Команда ЕСТЬ NULL УБЫВ,
	        |	Команда
	        |АВТОУПОРЯДОЧИВАНИЕ";
	Возврат ?(НомерСтроки = 0, СтрУдалить(Текст, "И (ТурнирнаяТаблица.НомерСтроки = &НомерСтроки)"), Текст);
КонецФункции
...
Функция ТурнирнаяТаблица(Чемпионат, НомерСтроки=0) Экспорт
	...
	Если НомерСтроки = 0 Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Итоги КАК Итоги
