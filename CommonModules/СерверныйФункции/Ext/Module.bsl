...
Функция СоставКоманды(Команда, Дата=Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Команда) Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |ИЗ
		                      |	РегистрСведений.ПеремещенияИгроков.СрезПоследних(&Дата, Команда = &Команда) КАК Перемещения");
...
Функция ИгрокНайти(Наименование, Команда=Неопределено) Экспорт
	Ответ	= Новый Структура("Ссылка,Команда", Справочники.Игроки.ПустаяСсылка(), Справочники.Команды.ПустаяСсылка());
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Если ЗначениеЗаполнено(Команда) Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ПеремещенияИгроков.Игрок КАК Ссылка,
...
Функция МатчНайти(Команды, Дата) Экспорт
	...
	Запрос.УстановитьПараметр("Команды",	Команды);
	Запрос.УстановитьПараметр("Дата",		НачалоДня(Дата));
	Запрос.УстановитьПараметр("Дата1",		НачалоНедели(Дата));
	Запрос.УстановитьПараметр("Дата2",		КонецНедели(Дата));
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Ссылка;
	КонецЕсли;
...
Процедура ОбъектПередЗаписью(Источник, Отказ) Экспорт
	...
	ИначеЕсли Источник.Модифицированность() Тогда
		...
		Исправитель.Комментарий		= СокрЛП(Лев(Исправитель.Комментарий, 1000));
...
Функция ОповещенияНаСегодня(Команда=Неопределено) Экспорт
	...
	Тим		= ?(Команда=Неопределено,	МояКомандаПолучить(), Команда);
	...
	Запрос.УстановитьПараметр("ДатаН",				Дата);
	Запрос.УстановитьПараметр("ДатаК",				КонецДня(Дата + Сутки(3)));
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ?(Авто, Старье.Найти(Выборка.Ссылка) = Неопределено, Истина) Тогда
			Ответ.Добавить(Новый Структура("Ссылка,Дата,Картинка,Ключ",
				Выборка.Ссылка,
				?(Выборка.ЭтоИгра, Формат(Выборка.Дата, ?(Дата=НачалоДня(Выборка.Дата), "ДФ=HH:mm", "ДЛФ=D")), СтрШаблон("%1 (%2)", Формат(Выборка.Дата, "ДЛФ=D"), Возраст(Выборка.Дата))),
				?(Выборка.ЭтоИгра, Лого, ИзображениеПолучить(Выборка.Ссылка, Истина)),
...
Процедура НастройкиСохранить(Значение=Неопределено)
	...
	Иначе	//Если АвтоудалениеНапоминаний() Тогда
...
Функция НастройкиВосстановить()
	...
	Если Настройки.Следующий() Тогда
		Если НачалоДня(Настройки.ДатаИзменения) = НачалоДня(ТекущаяУниверсальнаяДата()) Тогда
