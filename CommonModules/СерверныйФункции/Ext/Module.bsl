...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |					ИЛИ ЧемпионатыКоманды.Команда = &Параметр)
	                      ...
	                      |
	                      |ИМЕЮЩИЕ
	                      |	МИНИМУМ(Стадионы.Порядок) = Стадионы.Порядок
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Ссылка");
	Запрос.УстановитьПараметр("Параметр",			Параметр);
	Запрос.УстановитьПараметр("Наименование",		СокрЛП(Параметр));
	Запрос.УстановитьПараметр("Комментарий",		?(СтрДлина(СокрЛП(Параметр)) > 4, "%" + СокрЛП(Параметр) + "%", Строка(Новый УникальныйИдентификатор)));
		...
		Если Слова.Количество() > 1 Тогда
			...
			Иначе
				Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
				               ...
				               |	Игроки.ПометкаУдаления";
			КонецЕсли;
			Запрос.УстановитьПараметр("Слово1",		"%" + СокрЛП(Слова.Получить(0)) + "%");
			Запрос.УстановитьПараметр("Слово2",		"%" + СокрЛП(Слова.Получить(1)) + "%");
			Выборка	= Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				ЗаполнитьЗначенияСвойств(Ответ, Выборка);
			КонецЕсли;
...
Функция ИмпортИгрПоГиперссылке(Гиперссылка, Тур) Экспорт
	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли СтрНайти("210", Лев(Слово, 1)) > 0 И СтрНайти(":00:30", Прав(Слово, 3)) > 0 Тогда
			сИгра.Проведен = Ложь;
			Попытка
				сИгра.Дата	= Дата(Формат(сИгра.Дата, "ДФ=yyyyMMdd") + СтрЗаменить(Слово, ":", "") + "00") - ТриЧаса;
			Исключение
			КонецПопытки;
...
Функция ИмпортИгрПоСтроке(Гиперссылка, Тур) Экспорт
	Итого	= 0;
	ТриЧаса	= Часов(3);
	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли Год(сИгра.Дата) < 2 Тогда
			...
			Если мСлова.Количество() = 1 Тогда
				сИгра.Дата	= КакДата(Слово, сИгра.Тур);
			Иначе
				сИгра.Дата	= СтрокаКакДата(СокрЛП(мСлова.Получить(0)) + "." + Формат(Тур.ДатаНачала, "ДФ=yyyy"));
				Попытка
					сИгра.Дата	= Дата(Формат(сИгра.Дата, "ДФ=yyyyMMdd") + СтрЗаменить(СокрЛП(мСлова.Получить(1)), ":", "") + "00") - ТриЧаса;
				Исключение
				КонецПопытки;
			КонецЕсли;
		Иначе
...
Функция МатчСоздать() Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		КонецПопытки;
	КонецЕсли;
	Возврат Документы.Матч.СоздатьДокумент();
КонецФункции
...
Процедура Зачистить(Виктим, ЭтоСобытие=Ложь)
	ЗаполнитьЗначенияСвойств(Виктим, Новый Структура("ЭтоСобытие,Дата,Счет,Гол1,Гол2,КоличествоЗрителей,Проведен,Команды,Стадион,Комментарий,Команда1,Команда2", ЭтоСобытие, Дата(1,1,1), "", 0, 0, 0, Истина, Новый Массив));
КонецПроцедуры
...
Функция ОповещенияНаСегодня() Экспорт
	...
	Лого	= ИзображениеПолучить(Тим, Истина);
	ОдинЧас	= Часов();
	Если Лого.Вид = ВидКартинки.Пустая Тогда
		Лого	= БиблиотекаКартинок.Goal;
	КонецЕсли;
	Запрос	= Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	               |	Матч.Ссылка КАК Ссылка,
	      	               |	Матч.Дата КАК Дата,
	      	               |	РАЗНОСТЬДАТ(&Дата, Матч.Дата, ЧАС) КАК Количество,
	      	               |	ИСТИНА КАК ЭтоИгра
	      	               |ИЗ
	      	               |	Документ.Матч КАК Матч
	      	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Матч.Команды КАК МатчКоманды
	      	               ...
	      	               |	И ВЫБОР
	      	               |			КОГДА РАЗНОСТЬДАТ(&Дата, Матч.Дата, ЧАС) > 12
	      	               |					ИЛИ Матч.Дата = &ДатаН
	...
	Запрос.УстановитьПараметр("ДатаН",				НачалоДня(Дата));
	Запрос.УстановитьПараметр("ДатаК",				КонецДня(Дата + Сутки(3)));
	Запрос.УстановитьПараметр("Дата",				Дата);
	Запрос.УстановитьПараметр("Старье",				Старье);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Туча	= Новый Структура("Ссылка,Картинка,Ключ,Дата", Выборка.Ссылка, Лого, ПолучитьНавигационнуюСсылку(Выборка.Ссылка));
		Если НЕ Выборка.ЭтоИгра Тогда
			Туча.Дата		= СтрШаблон("%1 (%2)", Формат(Выборка.Дата, "ДЛФ=D"), Выборка.Количество);
			Туча.Картинка	= ИзображениеПолучить(Выборка.Ссылка, Истина);
			
		ИначеЕсли Выборка.Дата >= Дата Тогда
			Туча.Дата		= Формат(Выборка.Дата, "ДФ='d MMMM'");
			Если Выборка.Дата - Дата <= ОдинЧас Тогда
				Туча.Дата		= "В течение часа";
			ИначеЕсли Выборка.Количество = 1 Тогда
				Туча.Дата		= "Через час";
			ИначеЕсли Выборка.Количество <= 4 Тогда
				Туча.Дата		= СтрШаблон("Через %1 часа", Выборка.Количество);
			ИначеЕсли Выборка.Количество <= 12 Тогда
				Туча.Дата		= СтрШаблон("Через %1 часов", Выборка.Количество);
			КонецЕсли;
		Иначе
			Туча.Дата		= "Сегодня";
		КонецЕсли;
		...
		Ответ.Добавить(Туча);
	КонецЦикла;
	Если Авто И Выборка.Количество() > 0 Тогда
...
Функция НастройкиВосстановить()
	...
	Попытка
		...
		И НачалоДня(Настройки.ДатаИзменения + (ТекущаяДатаСеанса() - ТекущаяУниверсальнаяДата())) = НачалоДня(ТекущаяДатаСеанса())
		Тогда
