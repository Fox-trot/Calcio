...
Функция ТурнирнаяТаблица(Чемпионат, НомерСтроки=0) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ИСТИНА КАК Проведен,
	                      |	МАКСИМУМ(ВЫБОР
	                      |			КОГДА ЕСТЬNULL(ТурнирнаяТаблица.Тур.ОлимпийскаяСистема, ЛОЖЬ) = ИСТИНА
	                      |					И ЕСТЬNULL(ТурнирнаяТаблица.Тур.Владелец.Владелец.ОлимпийскаяСистема, ЛОЖЬ) = ИСТИНА
	                      |				ТОГДА ЧемпионатыКоманды.Команда
	                      |		КОНЕЦ) КАК Порядок
	                      |ИЗ
	                      |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
