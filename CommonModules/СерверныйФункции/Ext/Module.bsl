...
Функция ТурНеПоследний(Чемпионат) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	МАКСИМУМ(Туры.Ссылка) КАК Ссылка
	                      |ИЗ
	                      |	РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Туры КАК Туры
	                      |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
	                      ...
	                      |
	                      |ИМЕЮЩИЕ
	                      |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Туры.Ссылка) < КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЧемпионатыКоманды.Команда) * 2 - 2");
	Запрос.УстановитьПараметр("Чемпионат",	Чемпионат);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Ссылка;
...
Функция ИмпортИгрПоГиперссылке(Гиперссылка, Тур) Экспорт
	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли СтрНайти("210", Лев(Слово, 1)) > 0 И СтрНайти(":00:30:15", Прав(Слово, 3)) > 0 Тогда
			сИгра.Проведен = Ложь;
...
Функция ОповещенияНаСегодня(ТолькоМатчи=Ложь) Экспорт
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	               ...
	      	               |	ВТ.Дата КАК Дата,
	      	               |	РАЗНОСТЬДАТ(&Дата, ВТ.Дата, ЧАС) КАК Час,
	      	               |	РАЗНОСТЬДАТ(&Дата, ВТ.Дата, МИНУТА) КАК Минута,
	      	               |	ЛОЖЬ КАК ЭтоИгрок
	      	               |ИЗ
	      	               |	ВТ КАК ВТ
	      	               |
	...
	Если ТолькоМатчи Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	И Матч.Проведен = ЛОЖЬ
		               |	И Матч.ПометкаУдаления = ЛОЖЬ
		               |	И РАЗНОСТЬДАТ(&Дата, Матч.Дата, ЧАС) МЕЖДУ 0 И 12
		               |	И МатчКоманды.Команда = &Команда
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Дата";
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Туча	= Новый Структура("Ссылка,Картинка,Ключ,КлючУникальности,Дата", Выборка.Представление, Лого, ПолучитьНавигационнуюСсылку(Выборка.Ссылка), Строка(Выборка.Ссылка.УникальныйИдентификатор()));
