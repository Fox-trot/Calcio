...
Функция СобытияНаСегодня(Команда=Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаК",				КонецДня(ТекущаяДатаСеанса()));
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Ответ.Добавить(Новый Структура("Ссылка,Дата,Картинка",
			Выборка.Ссылка,
			Формат(Выборка.Дата, ?(Выборка.ЭтоИгра, "ДФ=HH:mm", "ДЛФ=D")),
...
Процедура НастройкиСохранить(Значение=Неопределено) Экспорт
	ХранилищеОбщихНастроек.Сохранить(ОповещенияПользователяИмя(),, ?(Значение=Неопределено, Новый Массив, Значение),, ИмяПользователя());
КонецПроцедуры

Функция НастройкиВосстановить() Экспорт
	...
	Возврат ?(Настройки.Следующий() И НачалоДня(Настройки.ДатаИзменения) = НачалоДня(ТекущаяДатаСеанса()), Настройки.Настройки, Новый Массив);
КонецФункции
