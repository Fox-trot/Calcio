...
Функция ИмпортИгрПоГиперссылке(Гиперссылка, Тур) Экспорт
	...
	Для Каждого Слово Из Слова Цикл
		Если Итого >= Параметры.ВсегоИгр Или СтрНайти2(Слово, "таблица") > 0 Тогда
			Прервать;
	...
	Для Каждого Слово Из Разделить(Слова, ".") Цикл
		...
		ИначеЕсли СтрНайти2(Слово, "стадион") > 0 Тогда
			...
			Если НЕ ЗначениеЗаполнено(Стадион) Тогда
				Если ЗначениеЗаполнено(Матч.Тур) И ЗначениеЗаполнено(Матч.Тур.Владелец.Владелец.Страна) Тогда
					Стадион	= СтадионНайти(Новый Структура("Наименование,Тур,Страна", СтадионИмя, Матч.Тур, Матч.Тур.Владелец.Владелец.Страна));
				ИначеЕсли ЗначениеЗаполнено(Матч.Тур) Тогда
					Стадион	= СтадионНайти(Новый Структура("Наименование,Тур", СтадионИмя, Матч.Тур));
				Иначе
