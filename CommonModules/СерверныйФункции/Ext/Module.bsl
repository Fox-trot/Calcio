...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |
	                      |ИМЕЮЩИЕ
	                      |	Стадионы.Порядок <= СРЕДНЕЕ(Стадионы.Порядок)
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Стадионы.Порядок");
...
Функция ТурнирнаяТаблица(Чемпионат) Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
...
Функция ОповещенияНаСегодня() Экспорт
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	               ...
	      	               |	ВТ.Дата КАК Дата,
	      	               |	ВЫБОР
	      	               |		КОГДА ВТ.ЭтоИгра = ИСТИНА
	      	               |			ТОГДА РАЗНОСТЬДАТ(&Дата, ВТ.Дата, ЧАС)
	      	               |		ИНАЧЕ РАЗНОСТЬДАТ(ВТ.Дата, &Дата, ГОД)
	      	               |	КОНЕЦ КАК Час,
	      	               |	ВЫБОР
	      	               |		КОГДА ВТ.ЭтоИгра = ИСТИНА
	      	               |			ТОГДА РАЗНОСТЬДАТ(&Дата, ВТ.Дата, МИНУТА)
	      	               |		ИНАЧЕ 0
	      	               |	КОНЕЦ КАК Минута,
	      	               |	ВТ.ЭтоИгра КАК ЭтоИгра
	      	               |ИЗ
	      	               |	ВТ КАК ВТ
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
