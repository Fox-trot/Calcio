...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	...
	Если ТипЗнч(Параметр) = Тип("Структура") Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		И Туры.Код = МАКСИМУМ(Финал.Код) КАК ЭтоФинал,
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				...
				ИначеЕсли НачалоДня(Выборка.Сдвиг) = НачалоДня(Дата) Тогда
					Туча.Дата		= "Сегодня " + Формат(Выборка.Сдвиг, "ДФ='HH:mm'");
				ИначеЕсли НачалоДня(Выборка.Сдвиг) = НачалоДня(Дата + Сутки()) Тогда
					Туча.Дата		= "Завтра " + Формат(Выборка.Сдвиг, "ДФ='HH:mm'");
				Иначе
					Туча.Дата		= Формат(Выборка.Сдвиг, "ДФ='d MMMM HH:mm'");
				КонецЕсли;
				Туча.Картинка	= ИзображениеПолучить(Тим, Истина);
			КонецЕсли;
			Если Туча.Картинка.Вид = ВидКартинки.Пустая Тогда
				Туча.Картинка	= ?(Выборка.ЭтоИгрок, БиблиотекаКартинок.Пользователь, БиблиотекаКартинок.Goal);
			КонецЕсли;
...
Процедура ГрафикОбновить(График, Данные, ЗабитоПропущено=Ложь) Экспорт
	ШалостьУдалась	= Ложь;
	Если График.Серии.Количество() <> ?(ЗабитоПропущено, 2, 3) Тогда
		График.Очистить();
