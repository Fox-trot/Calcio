...
Функция КомандыЛиги(Лига) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      ...
	                      |	Команда
	                      |АВТОУПОРЯДОЧИВАНИЕ");
	Если НЕ ЗначениеЗаполнено(Лига.Страна) Тогда
...
Функция КомандыСтраны(Параметр) Экспорт
	...
	Если ТипЗнч(Параметр) = Тип("СправочникСсылка.Лига") Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	Команды.ПометкаУдаления = ЛОЖЬ
		               |	И Команды.Владелец = Лига.Владелец
...
Функция КомандаНайти(Наименование, ВидСпорта=Неопределено) Экспорт
...
Функция СтадионНайти(Параметр, Все=Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |				И Чемпионаты.ОлимпийскаяСистема = ИСТИНА)
	                      |;
	                      |
	                      |////////////////////////////////////////////////////////////////////////////////
	                      |ВЫБРАТЬ
...
Функция ЧемпионатПолучитьПоследнее(Команды=Неопределено, ОлимпийскаяСистема=Ложь) Экспорт
	...
	Если ТипЗнч(Команды) = Тип("СправочникСсылка.Стадионы") Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	И Чемпионаты.ОлимпийскаяСистема = ЛОЖЬ
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Матч.Дата";
...
Функция ЛигаОлимпийскаяСистема(Лига) Экспорт
...
Функция МатчНайти(Команда, Тур) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |			ТОГДА МатчКоманды.НомерСтроки
	                      |		КОГДА МатчКоманды.Ссылка.ПометкаУдаления = ЛОЖЬ
	                      |			ТОГДА 10 + МатчКоманды.НомерСтроки
	                      |		ИНАЧЕ 100
	                      |	КОНЕЦ КАК Порядок
	                      |ИЗ
...
Функция ТурНайти(КодИлиНаименование, Чемпионат) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |	Туры.Код = &Код УБЫВ";
	Запрос.УстановитьПараметр("Чемпионат",		Чемпионат);
	Запрос.УстановитьПараметр("Наименование",	?(ТипЗнч(КодИлиНаименование)=Тип("Строка"), КодИлиНаименование, Формат(КодИлиНаименование, "ЧГ=0")) + "-%");
...
Функция СтатистикаИгр(Знач Команда, Знач Чемпионат=Неопределено, Период=Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("Команда",	Команда);
	Запрос.УстановитьПараметр("Чемпионат",	Чемпионат);
	Запрос.УстановитьПараметр("ДатаК",		НачалоДня(?(Период=Неопределено, ТекущаяДатаСеанса(), Период)) - 1);
	...
	ИначеЕсли Период = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТурнирнаяТаблица.Период МЕЖДУ &ДатаН И &ДатаК", "Туры.Владелец = &Чемпионат");
	КонецЕсли;
	Выборка = Запрос.Выполнить().Выбрать();
...
Функция Игрок(Наименование) Экспорт
	...
	Если Ризалт.Пустой() Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	Справочник.Игроки КАК Игроки
		               |ГДЕ
		               |	Игроки.Комментарий ПОДОБНО &Комментарий
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Игроки.ПометкаУдаления";
...
Функция МатчиПоследние5(Команда, Чемпионат, Дата=Неопределено, Сколько=5) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаН",			НачалоДня(ДобавитьМесяц(?(Дата=Неопределено, Чемпионат.ДатаОкончания, Дата), -6)));
...
Функция ИмпортЧемпионата(Гиперссылка, Чемпионат) Экспорт
	...
	Для Итератор = 1 По Предел Цикл
		...
		Если НЕ ЗначениеЗаполнено(Тур) Тогда
			...
			//оТур.Заполнить(Чемпионат);
			оТур.Владелец		= Чемпионат;
			оТур.Код			= Итератор;
			оТур.Наименование	= оТур.НаименованиеПолучить();
			оТур.ДатаНачала		= Чемпионат.ДатаНачала;
			оТур.ДатаОкончания	= Чемпионат.ДатаОкончания;
			оТур.Записать();
	...
	Для Итератор = 1 По Предел Цикл
		...
		Если ЗначениеЗаполнено(Тур) Тогда
			Если МатчиТура(Тур).Количество() = Игры Тогда
				Продолжить;
			ИначеЕсли ИмпортИгр(СтрШаблон(Гиперссылка + "%1/", Формат(Итератор, "ЧГ=0")), Тур) Тогда
				Итого	= Итого + 1;
...
Функция ИмпортИгр(Гиперссылка, Тур) Экспорт
	...
	//Предел		= Чемпионат.Команды.Количество();
	ВидСпорта	= Футбол();
	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгра.Команда1) И НЕ ЗначениеЗаполнено(сИгра.Команда2) Тогда
			Если СтрЧислоВхождений(Слово, "-") = 1 Тогда
				...
				сИгра.Команда1	= КомандаНайти(мСлова.Получить(0), ВидСпорта);
				сИгра.Команда2	= КомандаНайти(мСлова.Получить(1), ВидСпорта);
		...
		ИначеЕсли ГолыОпределить(сИгра, Слово) Тогда
			Игры = Игры + 1;
			Матч = МатчНайти(сИгра.Команда1, сИгра.Тур);
