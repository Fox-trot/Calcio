...
Процедура ШапкаЗаполнить(Матч, Слова)
	...
	Для Каждого Слово Из Разделить(Слова, ".") Цикл
		Если Год(Матч.Дата) < 2 И СтрокаПохожаНаДату(Слово) Тогда
			Матч.Дата = КакДата(Слово, ?(ЗначениеЗаполнено(Матч.Тур), Год(Матч.Тур.ДатаОкончания), 0));
			Если ЗначениеЗаполнено(Матч.Тур) Тогда
				Если НЕ (Матч.Дата >= Матч.Тур.ДатаНачала И Матч.Дата <= Матч.Тур.ДатаОкончания) Тогда
					Если ДеньГода(Матч.Дата) <= ДеньГода(Матч.Тур.ДатаОкончания) Тогда
						Матч.Дата = Дата(Год(Матч.Тур.ДатаОкончания), Месяц(Матч.Дата), День(Матч.Дата));
					Иначе
						Матч.Дата = Дата(Год(Матч.Тур.ДатаНачала), Месяц(Матч.Дата), День(Матч.Дата));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Если НЕ ПустаяСтрока(Гиперссылка) Тогда
		...
		Для Каждого Слово Из Слова Цикл
			...
			Если Год >= 2007 И Год <= 1 + Год(ТекущаяДатаСеанса()) Тогда
				...
				Выборка = Запрос.Выполнить().Выбрать();
				Если Выборка.Следующий() Тогда
					Возврат ДобавитьМесяц(Выборка.ДатаНачала, -1);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Если ЗначениеЗаполнено(Чемпионат) Тогда
			Возврат ДобавитьМесяц(НачалоМесяца(Чемпионат.ДатаНачала), -1);
		КонецЕсли;
	КонецЕсли;
...
Функция ИмпортИгроковНаСервере(Гиперссылка, Команда) Экспорт
	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгрок.Страна) Тогда
			...
			Если ?(ЗначениеЗаполнено(сИгрок.Команда), сИгрок.Команда <> КомандаИгрока(сИгрок.Игрок), Команда <> КомандаИгрока(сИгрок.Игрок)) Тогда
				Запись = РегистрыСведений.ПеремещенияИгроков.СоздатьМенеджерЗаписи();
