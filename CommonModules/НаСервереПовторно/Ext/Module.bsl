
Функция ФазаЦикличность(Фаза) Экспорт
	Возврат ?(Фаза=3, 33, ?(Фаза=2, 28, 23));
КонецФункции

Функция ФЭИ(Фаза=1) Экспорт
	Возврат ?(Фаза=3, "Интеллектуальный", ?(Фаза=2, "Эмоциональный", "Физический"));
КонецФункции

Функция Футбол() Экспорт
	Возврат Справочники.ВидыСпорта.Футбол;
КонецФункции

Функция МояКомандаПолучить() Экспорт
	Возврат Константы.МояКоманда.Получить();
КонецФункции

Функция ТурнирнаяТаблицаКакНаBBC() Экспорт
	Возврат Константы.ТурнирнаяТаблицаКакНаBBC.Получить();
КонецФункции

Функция ПоказыватьРазницуМячей() Экспорт
	Возврат ?(ТурнирнаяТаблицаКакНаBBC(), Истина, Константы.ПоказыватьРазницуМячей.Получить());
КонецФункции

Функция СтарыйВидАнонсов() Экспорт
	Возврат Константы.СтарыйВидАнонсов.Получить();
КонецФункции

Функция КоличествоМатчейВАнонсе() Экспорт
	Возврат Макс(5, Константы.КоличествоМатчейВАнонсе.Получить());
КонецФункции

Функция ВыводитьТурВместоЧемпионата() Экспорт
	Возврат Константы.ВыводитьТурВместоЧемпионата.Получить();
КонецФункции

Функция Цвет(Фаза, Дефолт=Ложь) Экспорт
	Ответ	= Неопределено;
	Цвета = ?(Дефолт, Неопределено, Константы.Цвета.Получить().Получить());
	Если ТипЗнч(Цвета) = Тип("Соответствие") Тогда
		Если Фаза = 3 Тогда
			Ответ = Цвета.Получить(3);
		ИначеЕсли Фаза = 2 Тогда
			Ответ = Цвета.Получить(2);
		Иначе
			Ответ = Цвета.Получить(1);
		КонецЕсли;
	КонецЕсли;
	Если Ответ = Неопределено Тогда
		Если Фаза = 3 Тогда
			Возврат ЦветаСтиля.ЦветФонаВыделенияПоля;
		ИначеЕсли Фаза = 2 Тогда
			Возврат ЦветаСтиля.ЦветОтрицательногоЧисла;
		КонецЕсли;
		Возврат ЦветаСтиля.ЦветАкцента;
	КонецЕсли;
	Возврат Ответ;
КонецФункции

Функция ПоказыватьПредстоящиеМатчи() Экспорт
	Возврат Константы.ПоказыватьПредстоящиеМатчи.Получить();
КонецФункции

Функция Сессия() Экспорт
	Возврат 1000 * НомерСоединенияИнформационнойБазы() + НомерСеансаИнформационнойБазы();
КонецФункции

Функция ВыводитьЛоготипыКоманд() Экспорт
	Возврат Константы.ВыводитьЛоготипыКоманд.Получить();
КонецФункции

Функция ИзображениеДефолтПолучить() Экспорт
	Возврат "";
КонецФункции

Функция ЧемпионатПустой(Чемпионат) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      |	ТурнирнаяТаблица.Период КАК Период
	                      |ИЗ
	                      |	РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	                      |ГДЕ
	                      |	ТурнирнаяТаблица.Тур.Владелец = &Чемпионат
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Период");
	Запрос.УстановитьПараметр("Чемпионат",		Чемпионат);
	Возврат Запрос.Выполнить().Пустой();
КонецФункции

Функция ЧтениеНовостей() Экспорт
	Возврат Константы.ЧтениеНовостей.Получить();
КонецФункции

Функция Ничья(ВидСпорта) Экспорт
	Возврат Справочники.ВидыСпорта.Ничья(ВидСпорта);
КонецФункции

Функция Победа(ВидСпорта) Экспорт
	Возврат Справочники.ВидыСпорта.Победа(ВидСпорта);
КонецФункции

Функция РазмерКартинкиМинимальный() Экспорт
	Возврат 128;
КонецФункции

Функция ЛогированияУровеньДоступен(Уровень) Экспорт
	Тест = Новый Массив;
	Если Уровень = Неопределено Тогда
		Тест.Добавить(УровеньЖурналаРегистрации.Ошибка);
	Иначе
		Тест.Добавить(Уровень);
	КонецЕсли;
	Возврат (ПолучитьИспользованиеЖурналаРегистрации().Количество() > 0);
КонецФункции
