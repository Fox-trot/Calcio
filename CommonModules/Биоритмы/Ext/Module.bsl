...
Функция ФазыРасчитать(Знач ДатаРождения, Знач ТекущаяДата=Неопределено) Экспорт
	...
	Ответ.Физический		= ФазаРасчитать2((ТекущаяДата - ДатаРождения) / 86400, 1);
	Ответ.Эмоциональный		= ФазаРасчитать2((ТекущаяДата - ДатаРождения) / 86400, 2);
	Ответ.Интеллектуальный	= ФазаРасчитать2((ТекущаяДата - ДатаРождения) / 86400, 3);
	Возврат Ответ;
КонецФункции

Функция ФазаРасчитать1(Знач ДатаРождения, Знач Фаза=1, Знач ТекущаяДата=Неопределено) Экспорт
	Возврат ФазаРасчитать2(Цел((НачалоДня(?(ТекущаяДата=Неопределено, ТекущаяДата(), ТекущаяДата)) - ДатаРождения) / 86400), Фаза);
КонецФункции
