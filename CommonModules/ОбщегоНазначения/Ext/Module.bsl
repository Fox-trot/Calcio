...
Функция УстановитьЗначение(Реквизит, Значение, ЕслиЗначениеЗаполнено=Ложь) Экспорт
	Если ?(ЕслиЗначениеЗаполнено, ЗначениеЗаполнено(Значение) И Реквизит <> Значение, Реквизит <> Значение) Тогда
		Реквизит = Значение;
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
...
Процедура БезАвтоНастроек(ЭтаФорма, Элемент) Экспорт
	ЭтаФорма.АвтоматическоеСохранениеДанныхВНастройках = АвтоматическоеСохранениеДанныхФормыВНастройках.НеИспользовать;
	ЭтаФорма.ЗакрыватьПриЗакрытииВладельца	= Истина;
	Если ТипЗнч(Элемент) = Тип("ДинамическийСписок") Тогда
		Элемент.АвтоматическоеСохранениеПользовательскихНастроек = Ложь;
...
Процедура СостояниеОтобразить(Отображение) Экспорт
	Отображение.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
	Отображение.Текст	= "Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.";
КонецПроцедуры
