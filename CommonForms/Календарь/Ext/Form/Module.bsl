...
&НаСервере
Функция Причесать(Слово)
	Если ПустаяСтрока(Слово) Тогда Возврат ""; КонецЕсли;
	Ответ	= СтрЗаменить(СтрЗаменить(Слово, "\n\n", Символы.ПС), "\n", " ");
	Возврат Ответ;
КонецФункции
	...
	Для НомерСтроки = 1 По СтрЧислоСтрок(Текст) Цикл
		...
		ИначеЕсли СтрСравнить(Первое, "DTSTART") = 0 Тогда
			...
			Если Элемент.Дата > ТекущаяУниверсальнаяДата() Тогда
				Элемент.Дата = УниверсальноеВремя(Элемент.Дата, ЧасовойПояс());
			Иначе
				Зачистить(Элемент);
			КонецЕсли;
		...
		ИначеЕсли СтрСравнить(Первое, "LOCATION") = 0 Тогда
			...
			Элемент.Стадион = СтадионНайти(Слово, Стадионы);
...
&НаСервереБезКонтекста
Функция СтадионНайти(Слово, Стадионы)
