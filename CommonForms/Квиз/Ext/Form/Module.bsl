...
Функция ВопросыОтветыПолучить()
	...
	Запрос.УстановитьПараметр("Дата",				НачалоГода(ДобавитьМесяц(ТекущаяДатаСеанса(), -11)));
	...
	Если Уровень = 10 И ЗначениеЗаполнено(МояКоманда) Тогда
	...
	ИначеЕсли Уровень = 2 Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 77
		               ...
		               |			ТурнирнаяТаблица.Команда.Город.Владелец КАК Страна,
		               |			МАКСИМУМ(ГОД(ТурнирнаяТаблица.Период)) - 2 КАК Год,
		               |			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Регистратор) КАК Количество
		               |		ИЗ
		               |			РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
		               |		ГДЕ
		               |			ТурнирнаяТаблица.Команда.ПометкаУдаления = ЛОЖЬ
		               |			И РАЗМЕРХРАНИМЫХДАННЫХ(ТурнирнаяТаблица.Команда.Изображение) >= 64
		               |		
		               |		СГРУППИРОВАТЬ ПО
		               |			ТурнирнаяТаблица.Команда.Город.Владелец) КАК Итоги
		               |		ПО Команды.Город.Владелец = Итоги.Страна
		               |ГДЕ
		               |	Команды.ПометкаУдаления = ЛОЖЬ
		               |	И РАЗМЕРХРАНИМЫХДАННЫХ(Команды.Изображение) >= 64
		               |	И (Команды.Ссылка В (&Исключения)) = ЛОЖЬ
		               ...
		               |УПОРЯДОЧИТЬ ПО
		               |	Команды.Ссылка В (&Было),
		               |	ЕСТЬNULL(МАКСИМУМ(ГОД(ТурнирнаяТаблица.Период)), 0) >= Итоги.Год УБЫВ,
		               |	Итоги.Количество";
	КонецЕсли;
...
Процедура ОчиститьКеш()
	Было.Очистить();
	Исключения.Очистить();
	Если Уровень = 2 И ЗначениеЗаполнено(МояКоманда) Тогда
