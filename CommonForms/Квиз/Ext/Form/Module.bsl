...
Функция ВопросыОтветыПолучить()
	...
	Если Уровень = 1 Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 99
		               ...
		               |		ГДЕ
		               |			КвизРезультаты.Период МЕЖДУ &Дата И &ДатаК
		               ...
		               |		
		               |		ИМЕЮЩИЕ
		               |			(КвизРезультаты.Вопрос В (&Исключения)) = ЛОЖЬ
		               |		
		               |		УПОРЯДОЧИТЬ ПО
		               ...
		               |УПОРЯДОЧИТЬ ПО
		               |	Пометка,
		               |	ЕСТЬNULL(МАКСИМУМ(КвизРезультаты.Количество), 1000) УБЫВ";
	ИначеЕсли Уровень = 2 Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 99
		               ...
		               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ ПЕРВЫЕ 99
		               |			КвизРезультаты.Вопрос КАК Команда,
		               |			ЕСТЬNULL(СРЕДНЕЕ(Итоги.Место), 99) <= 6 КАК Порядок,
		               ...
		               |УПОРЯДОЧИТЬ ПО
		               |	Пометка,
		               |	ЕСТЬNULL(КвизРезультаты.Количество, 0),
		               |	Ссылка
		               |АВТОУПОРЯДОЧИВАНИЕ";
	КонецЕсли;
