...
Процедура СтопНаСервере()
	Если ПравильныхОтветов = КоличествоВопросов Тогда
		...
		Если Затрачено <= Макс(30, 2.5 * КоличествоВопросов) Тогда
...
Функция ВопросыОтветыПолучить()
	...
	Запрос.УстановитьПараметр("Дата",				НачалоГода(ДобавитьМесяц(ТекущаяДатаСеанса(), -22)));
...
Процедура ОчиститьКеш()
	...
	Если Уровень = 2 Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |		РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
		                      |	ГДЕ
		                      |		ТурнирнаяТаблица.Тур.Владелец.Владелец.Страна.Ссылка ЕСТЬ NULL
		                      |		И ТурнирнаяТаблица.Тур.Владелец.Владелец.ОлимпийскаяСистема = ИСТИНА
		                      |	
		                      |	СГРУППИРОВАТЬ ПО
		                      |		ТурнирнаяТаблица.Команда
		                      |	
