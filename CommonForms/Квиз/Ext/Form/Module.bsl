...
&НаСервереБезКонтекста
Функция ВремяСтрокой(Знач Количество, Знач Степ=2)
	Степ = Степ - 1;
	Ответ = "";
	...
	ИначеЕсли Количество < 3600 Тогда
		Ответ = Строка(Цел(Количество/60)) + " мин " + ВремяСтрокой(Количество-Цел(Количество/60)*60, Степ);
	Иначе
		Ответ = Строка(Цел(Количество/3600)) + " час " + ВремяСтрокой(Количество-Цел(Количество/3600)*3600, Степ);
	КонецЕсли;
	Возврат СокрП(Ответ);
КонецФункции
...
Процедура СтопНаСервере(ШагСледующий)
	...
	Если ПравильныхОтветов = КоличествоВопросов И Затрачено > 0 Тогда
		...
		Если ?(РезультатЛучший = 0, Истина, Затрачено < РезультатЛучший) Тогда
			КвизРезультатЛучший = ВремяСтрокой(Затрачено);
		КонецЕсли;
...
Функция ЗапросИнит(ЧислоСлевина=0)
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 199
	                      ...
	                      |		СРЕДНЕЕ(&ЧислоСлевина + КвизРезультаты.Время)
	                      |	ИЗ
	                      |		РегистрСведений.КвизРезультаты КАК КвизРезультаты
	                      |	ГДЕ
	                      |		КвизРезультаты.Период >= &ДатаН
	                      |		И ВЫБОР
	                      |				КОГДА (ВЫРАЗИТЬ(ПРАВ(КвизРезультаты.Сессия, 1) КАК СТРОКА(1))) = ""0""
	                      |					ТОГДА КвизРезультаты.Время МЕЖДУ 0 И 2
	                      |				КОГДА (ВЫРАЗИТЬ(ПРАВ(КвизРезультаты.Сессия, 1) КАК СТРОКА(1))) = ""1""
	                      |					ТОГДА КвизРезультаты.Время МЕЖДУ 0 И 1
	                      |				ИНАЧЕ ЛОЖЬ
	                      |			КОНЕЦ
	                      |	
	                      |	СГРУППИРОВАТЬ ПО
	                      |		КвизРезультаты.Вопрос) КАК ВТ
	                      |ГДЕ
...
Функция ВопросыОтветыПолучить()
	...
	Если Уровень = 1 Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 199
		               ...
		               |			СРЕДНЕЕ(ОКР(&ЧислоСлевина + ВЫБОР
		               ...
		               |ГДЕ
		               |	(Команды.Ссылка В (&Исключения)) = ЛОЖЬ
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Пометка,
		               |	ОКР(ЕСТЬNULL(КвизРезультаты.Количество, 9)) УБЫВ";
	ИначеЕсли Уровень = 2 Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 199
		               ...
		               |			ЦЕЛ(СРЕДНЕЕ(&ЧислоСлевина - КвизРезультаты.Время)) КАК Количество
		               |		ИЗ
		               |			РегистрСведений.КвизРезультаты КАК КвизРезультаты
		               ...
		               |УПОРЯДОЧИТЬ ПО
		               |	Пометка,
		               |	ЕСТЬNULL(КвизРезультаты.Количество, 9) УБЫВ";
	КонецЕсли;
...
&НаСервере
Функция Сессия(Частично=Ложь)
	Возврат ?(Частично, СтрШаблон("%1%2", Формат(КоличествоВопросов, "ЧЦ=2; ЧН=00; ЧВН="), Уровень), СтрШаблон("%1%2%3", Формат(ТекущаяДатаСеанса(), "ДФ=HHmmss"), Формат(КоличествоВопросов, "ЧЦ=2; ЧН=00; ЧВН="), Уровень));
КонецФункции
...
Функция ХроносЗаписать()
	...
	Пока Выборка.Следующий() Цикл
		...
		НЗ.Отбор.Сессия.Установить(Выборка.Сессия);
...
Процедура КвизРезультатЛучшийУстановить()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      |	КвизРезультаты.Период КАК Период,
	                      |	КвизРезультаты.Сессия КАК Сессия,
	                      |	СУММА(КвизРезультаты.Время) КАК Время
	                      |ИЗ
	                      |	РегистрСведений.КвизРезультаты КАК КвизРезультаты
	                      |ГДЕ
	                      |	КвизРезультаты.Время > 0
	                      |	И КвизРезультаты.Сессия ПОДОБНО &Сессия
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	КвизРезультаты.Период,
	                      |	КвизРезультаты.Сессия
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Время,
	                      |	Период УБЫВ");
	Запрос.УстановитьПараметр("Сессия",				"%" + Сессия(Истина));
	Выборка = Запрос.Выполнить().Выбрать();
