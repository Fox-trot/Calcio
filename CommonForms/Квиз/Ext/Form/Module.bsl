...
Функция ВопросУстановить()
	Если НЕ ВопросУстановитьНаСервере() Тогда
		ХроносДобавить();
	КонецЕсли;
	Возврат ЗначениеЗаполнено(ТекущийВопрос);
КонецФункции
...
Функция ЗапросИнит(ЧислоСлевина=0)
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 199
	                      ...
	                      |	СРЕДНЕЕ(ВТ.Порядок) КАК Порядок
	                      |ИЗ
	                      |	(ВЫБРАТЬ
	                      |		Итоги.Команда КАК Ссылка,
	                      |		СУММА(&ЧислоСлевина + Итоги.КоличествоЗабито) КАК Порядок
	                      |	ИЗ
	                      ...
	                      |	ВЫБРАТЬ
	                      |		Противник.Команда,
	                      |		СУММА(&ЧислоСлевина + Противник.КоличествоГолов)
	                      |	ИЗ
	                      ...
	                      |		СУММА(&ЧислоСлевина + ЦЕЛ(КвизРезультаты.Время * 0.001))
	                      |	ИЗ
	                      |		РегистрСведений.КвизРезультаты КАК КвизРезультаты
	                      |	ГДЕ
	                      |		КвизРезультаты.Период >= &ДатаК
	                      |		И КвизРезультаты.Время МЕЖДУ -4 И 0
	                      |	
	                      |	СГРУППИРОВАТЬ ПО
