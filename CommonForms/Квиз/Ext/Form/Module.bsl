...
Процедура СтопНаСервере()
	...
	Если ПравильныхОтветов = КоличествоВопросов Тогда
		...
		Если Затрачено <= Макс(30, 2.8 * КоличествоВопросов) Тогда
...
Функция ЗапросИнит()
	...
	Если ЗначениеЗаполнено(МояКоманда) Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |			ИЛИ ТурнирнаяТаблица.Тур.Владелец.Владелец.Страна ЕСТЬ NULL)
	...
	КонецЕсли;
	Запрос.УстановитьПараметр("Дата",				НачалоГода(ДобавитьМесяц(ТекущаяДатаСеанса(), -33)));
