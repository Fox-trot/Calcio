...
Функция Цвет(Фаза, Дефолт=Ложь)
	Возврат НаСервереПовторно.Цвет(Фаза, Дефолт);
КонецФункции
...
Процедура ПриОткрытииНаСервере()
	...
	Для Каждого Регламент Из РегламентныеЗадания.ПолучитьРегламентныеЗадания() Цикл
		Если СтрСравнить(Регламент.Ключ, "ИмпортИгр") = 0 Тогда
			ИмпортИгр		= Регламент.Использование;
			Элементы.ИмпортИгрРасширеннаяПодсказка.Заголовок = Регламент.Расписание;
		ИначеЕсли СтрСравнить(Регламент.Ключ, "ИмпортНовостей") = 0 Тогда
			ИмпортНовостей	= Регламент.Использование;
			Элементы.ИмпортНовостейРасширеннаяПодсказка.Заголовок = Регламент.Расписание;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	ВыводитьЛоготипыКоманд		= НаборКонстант.ВыводитьЛоготипыКоманд;
	ИспользоватьСтадионы		= НаборКонстант.ИспользоватьСтадионы;
	ЧтениеНовостей				= НаборКонстант.ЧтениеНовостей;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Или ВыводитьЛоготипыКоманд <> НаборКонстант.ВыводитьЛоготипыКоманд
	Тогда
		ТурнирнаяТаблицаКакНаBBC	= НаборКонстант.ТурнирнаяТаблицаКакНаBBC;
		ВыводитьТурВместоЧемпионата	= НаборКонстант.ВыводитьТурВместоЧемпионата;
		ВыводитьЛоготипыКоманд		= НаборКонстант.ВыводитьЛоготипыКоманд;
		НаКлиенте.Оповещение("ТурнирнаяТаблица");
	КонецЕсли;
	Если ИспользоватьСтадионы <> НаборКонстант.ИспользоватьСтадионы Тогда
		ИспользоватьСтадионы	= НаборКонстант.ИспользоватьСтадионы;
		НаКлиенте.Оповещение("ИспользоватьСтадионы", ИспользоватьСтадионы);
	КонецЕсли;
	Если ЧтениеНовостей <> НаборКонстант.ЧтениеНовостей Тогда
		ЧтениеНовостей	= НаборКонстант.ЧтениеНовостей;
		НаКлиенте.Оповещение("ЧтениеНовостей", ЧтениеНовостей);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Константы.БуферНовостей.Получить()) Тогда
		ИмпортНовостей = Ложь;
	КонецЕсли;
	Для Каждого Регламент Из РегламентныеЗадания.ПолучитьРегламентныеЗадания() Цикл
		Если СтрСравнить(Регламент.Ключ, "ИмпортИгр") = 0 Тогда
			Регламент.Использование	= ИмпортИгр;
			Регламент.Записать();
		ИначеЕсли СтрСравнить(Регламент.Ключ, "ИмпортНовостей") = 0 Тогда
			Регламент.Использование	= ИмпортНовостей;
			Регламент.Записать();
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
