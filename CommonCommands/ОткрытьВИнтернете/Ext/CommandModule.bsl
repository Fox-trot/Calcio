...
Функция СсылкаСформировать(Ссылка)
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.Туры") Тогда
		УРЛ = СерверныйФункции.КомментарийКакУРЛ(Ссылка.Комментарий);
		Возврат ?(ПустаяСтрока(УРЛ), СтрШаблон("%1%2", Я(), Кодировать(СтрШаблон("обзор %1 %2", Ссылка.Наименование, Ссылка.Владелец))), УРЛ);
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.Игроки") Тогда
		УРЛ = СерверныйФункции.КомментарийКакУРЛ(Ссылка.Комментарий);
		Возврат ?(ПустаяСтрока(УРЛ), Я() + Кодировать(СтрШаблон("%1 %2 др %3", ?(ПустаяСтрока(СтрПолучитьСтроку(Ссылка.Комментарий, 1)), Ссылка.Наименование, СтрПолучитьСтроку(Ссылка.Комментарий, 1)), Ссылка.Амплуа, Формат(Ссылка.ДатаРождения, "ДФ=dd.MM.yyyy"))), УРЛ);
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.Матч") Тогда
		Возврат СтрШаблон("%1%2 %3 obzor %4", Я(), Кодировать(Ссылка.Счет), Формат(Ссылка.Дата, "ДЛФ=DD"), ?(СерверныйФункции.ВидСпорта(Ссылка) = НаСервереПовторно.Футбол(), "kulichki", ""));
	КонецЕсли;
