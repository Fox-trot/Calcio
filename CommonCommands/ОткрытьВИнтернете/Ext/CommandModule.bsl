...
Функция СсылкаСформировать(Ссылка)
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.Матч") Тогда
		Возврат СтрШаблон("%1%2%3 обзор %4", Я(), Кодировать(Ссылка.Счет), Формат(Ссылка.Дата, "ДЛФ=DD"), ?(СерверныйФункции.ВидСпорта(Ссылка) = НаСервереПовторно.Футбол(), "kulichki", ""));
	КонецЕсли;
