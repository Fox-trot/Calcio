		...
		ИначеЕсли СтрЧислоСтрок(Слово) = 1 Тогда
			Если СтрНайти(Слово, ":") > 0 Тогда
				...
				Тогда // это Комментарий
					сМатч.Вставить("Комментарий", КомментарийПричесать(?(ПустаяСтрока(КомментарийКакУРЛ(Слово)), Слово + Разрыв() + Гиперссылка, Слово)));
	...
	Тогда
		...
		Если Док.ПроверитьЗаполнение() Тогда
			...
			ИначеЕсли ПустаяСтрока(КомментарийКакУРЛ(Док.Комментарий)) Тогда
				Док.Комментарий = Док.Комментарий + Разрыв() + Гиперссылка;
			КонецЕсли;
		...
		ИначеЕсли СтрНайти2(Слово, "стадион") > 0 Тогда
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(Матч.Стадион) Тогда
					Матч.Стадион	= СтадионНайти(СтрУдалить(СтрУдалить(НРег(Слово), "стадион"), """"));
				КонецЕсли;
			КонецЕсли;
...
Функция КомментарийПричесать(Знач Комментарий)
	Если СтрНайти(Комментарий, "Фото - Getty Images.") > 0 Тогда
		Комментарий	= СтрЗаменить(Комментарий, "Фото - Getty Images.", Символы.ПС);
	Иначе
		Комментарий	= СтрЗаменить(Комментарий, "Фото - Getty Images", Символы.ПС);
	КонецЕсли;
	Если СтрНайти(Комментарий, "Предыстория.") > 0 Тогда
		Комментарий	= СтрЗаменить(Комментарий, "Предыстория.", Разрыв() + "Предыстория" + Разрыв());
	Иначе
		Комментарий	= СтрЗаменить(Комментарий, "Предыстория", Разрыв() + "Предыстория" + Разрыв());
	КонецЕсли;
	Если СтрНайти(Комментарий, "Ход матча.") > 0 Тогда
		Комментарий	= СтрЗаменить(Комментарий, "Ход матча.", Разрыв() + "Ход матча" + Разрыв());
	ИначеЕсли СтрНайти(Комментарий, "Ход матча") > 0 Тогда
		Комментарий	= СтрЗаменить(Комментарий, "Ход матча", Разрыв() + "Ход матча" + Разрыв());
	Иначе
		Комментарий	= СтрЗаменить(Комментарий, "Ход игры", Разрыв() + "Ход игры" + Разрыв());
	КонецЕсли;
