...
Функция ИмпортНаСервере(Матч, Гиперссылка)
	...
	Тогда
		...
		Если ДокМатч.Команды.НайтиСтроки(Новый Структура("Команда", Команда1)).Количество() = 0
		Или ДокМатч.Команды.НайтиСтроки(Новый Структура("Команда", Команда2)).Количество() = 0
		Тогда
		...
		КонецЕсли;
		Виктимз = Ложь;
		Для Каждого ТекЭлемент Из ДокМатч.СоставыКоманд Цикл
			Если ДокМатч.Команды.НайтиСтроки(Новый Структура("Команда", ТекЭлемент.Команда)).Количество() = 0 Тогда
				Виктимз = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если Виктимз Тогда ДокМатч.СоставыКоманд.Очистить(); КонецЕсли;
