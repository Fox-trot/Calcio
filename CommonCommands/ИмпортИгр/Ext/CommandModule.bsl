...
Функция ИмпортНаСервере(Гиперссылка, Тур)
	...
	Иначе
		Слово = СтрПолучитьСтроку(СтрЗаменить(Гиперссылка, "¶", Символ(13)), 1);
		Если СтрНайти(Слово, "Счет") > 0 И СтрНайти2(Слово, "дата, время") > 0 Тогда
			Ответ	= СерверныйФункции.ИмпортИгрПоСтроке2(Гиперссылка, Тур);
		ИначеЕсли ЗначениеЗаполнено(СтрокаКакДата(Разделить(СтрУдалить(Слово, ",")).Получить(0))) Тогда
			Ответ	= СерверныйФункции.ИмпортИгрПоСтроке3(Гиперссылка, Тур);
		Иначе
