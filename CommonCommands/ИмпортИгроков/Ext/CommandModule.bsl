...
Функция ИмпортИгроковНаСервере2(Гиперссылка, Команда)
	...
	//	Тогда
		ИначеЕсли ТипЗнч(сИгрок.Страна) = Тип("СправочникСсылка.Страны") Тогда
 			сИгрок.Игрок = ИгрокНайти(сИгрок.Наименование);
...
Функция ИгрокиКоманды(Команда, Дата)
	...
	Пока Выборка.Следующий() Цикл
		Ответ.Вставить(Выборка.Игрок, Выборка.Номер);
	КонецЦикла;
	Возврат Ответ;
КонецФункции
...
Функция ИмпортИгроковНаСервере(Гиперссылка, Команда)
	...
	Старое		= ИгрокиКоманды(Команда, ДатаНачала + Сутки());
	Новое		= Новый Массив;
	...
	//КонецЕсли;
	Для Каждого Игрок Из Старое Цикл
		Если Новое.Найти(Игрок.Ключ) = Неопределено Тогда
			Запись = РегистрыСведений.ПеремещенияИгроков.СоздатьМенеджерЗаписи();
			ЗаполнитьЗначенияСвойств(Запись, Новый Структура("Период,Игрок", ДатаНачала, Игрок.Ключ));
			Если Запись.ПроверитьЗаполнение() Тогда
				Попытка
					Запись.Записать();
				Исключение
...
Функция ДатаНачалаСезона(Гиперссылка="")
	...
	Для Каждого Слово Из Слова Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Возврат Дата(Год, ?(Год = Год(ТекущаяДатаСеанса()) И Месяц(ТекущаяДатаСеанса()) < Месяц, 1, Месяц), 1);
КонецФункции
...
&НаСервере
Функция ИгрокНайти(Наименование)
	Игрок	= СерверныйФункции.ИгрокНайти(Наименование);
	Возврат Игрок.Игрок;
КонецФункции
