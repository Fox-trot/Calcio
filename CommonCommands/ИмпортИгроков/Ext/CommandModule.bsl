...
Функция ИмпортИгроковНаСервере(Гиперссылка, Команда)
	ДатаНачала	= ДатаНачалаСезона(Команда);
	Если НЕ ЗначениеЗаполнено(ДатаНачала) Тогда Возврат Ложь; КонецЕсли;
	
	Итого		= 0;
	ВсеПлохо	= НРег(Команда.Наименование);
	Старье		= ИгрокиКоманды(Команда, ДатаНачала + Сутки());
	Новое		= Новый Соответствие;
	Исключения	= Разделить("- м г ж к гражданство");
...
&НаСервере
Функция ДатаНачалаСезона(Команда)
	Чемпионат	= СерверныйФункции.ЧемпионатЛигиПоследний(СерверныйФункции.ЛигаКоманды(Команда));
