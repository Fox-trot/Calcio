	...
	Для Каждого Слово Из Слова Цикл
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(сИгрок.Страна) Тогда
			...
			Если НовыйИгрок.Модифицированность() Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Новое.Вставить(сИгрок.Игрок, сИгрок.Номер);
...
Функция УРЛ(Команда)
	...
	Если СтрокаСодержитЧисло(УРЛ) И СтрНайти(УРЛ, ФорматГод(ТекущаяДатаСеанса())) = 0 Тогда
		Для Год = 2010 По 1 + Год(ТекущаяДатаСеанса()) Цикл
			Если СтрНайти(УРЛ, ФорматГод(Год)) > 0 Тогда
				УРЛ	= СтрЗаменить(УРЛ, ФорматГод(Год), ФорматГод(ТекущаяДатаСеанса()));
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
