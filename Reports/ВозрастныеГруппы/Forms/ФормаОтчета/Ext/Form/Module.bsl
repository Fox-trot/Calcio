...
Процедура КартинкиЗагрузить() Экспорт
	ОбщегоНазначения.КартинкиЗагрузить(_Команды, Кеш);
КонецПроцедуры
...
Процедура КомандыЗагрузить(Топ6=Ложь)
	...
	Иначе
		...
		Иначе
			Чемпионат	= СерверныйФункции.ЧемпионатЛигиПоследний(МояКоманда);
...
Процедура ГрафикВыбор(Элемент, ЗначениеДиаграммы, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ЗначениеДиаграммы) = Тип("ТочкаДиаграммы") Тогда
		...
		Если ТипЗнч(ЗначениеДиаграммы.Значение) = Тип("СправочникСсылка.Команды") Тогда
			Если НЕ (_Команды.Количество() = 1 И _Команды.Получить(0).Значение = ЗначениеДиаграммы.Значение) Тогда
				_Команды.Очистить();
				_Команды.Добавить(ЗначениеДиаграммы.Значение);
				ГрафикОбновить();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("Команда") Тогда
		...
		График.ВидПодписей	= ВидПодписейКДиаграмме.ЗначениеПроцент;
	...
	ИначеЕсли Параметры.Свойство("Чемпионат") Тогда
		Если Параметры.Чемпионат.Владелец.Страна.Пустая() Тогда
			Отказ = Истина;
		Иначе
