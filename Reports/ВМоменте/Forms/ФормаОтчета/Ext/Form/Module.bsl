...
Процедура ДанныеПолучить()
	...
	ИначеЕсли ТипГрафика = 3 Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ТурнирнаяТаблица.Команда КАК Команда,
		               |	Противники.КоличествоГолов = 0
		               |		И ТурнирнаяТаблица.КоличествоОчков >= 2 КАК Победа,
		               |	МАКСИМУМ(ДОБАВИТЬКДАТЕ(ТурнирнаяТаблица.Период, ДЕНЬ, 1)) КАК Период
		               |ПОМЕСТИТЬ Последнее
		               |ИЗ
		               |	РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
	...
	ИначеЕсли ТипГрафика = 5 Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ТурнирнаяТаблица.Команда КАК Команда,
		               |	ТурнирнаяТаблица.КоличествоОчков = 0 КАК Победа,
		               |	МАКСИМУМ(ДОБАВИТЬКДАТЕ(ТурнирнаяТаблица.Период, ДЕНЬ, 1)) КАК Период
		               |ПОМЕСТИТЬ Последнее
		               |ИЗ
		               |	РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
...
Функция МатчиПолучить(Расшифровка)
	Запрос = Новый Запрос(СтрЗаменить("ВЫБРАТЬ ПЕРВЫЕ 11
	                                  ...
	                                  |
	                                  |УПОРЯДОЧИТЬ ПО
	                                  |	ТурнирнаяТаблица.Период УБЫВ", "11", Мин(11, Расшифровка.Количество)));
	Запрос.УстановитьПараметр("Чемпионат",		Отчет.Чемпионат);
