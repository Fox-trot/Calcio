...
Процедура ГрафикОбновить()
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               ...
	      	               |ИЗ
	      	               |	РегистрСведений.ПеремещенияИгроков.СрезПоследних(, Игрок = &КомандаИлиИгрок) КАК Перемещения
	      	               |ГДЕ
	      	               |	ГОД(Перемещения.Игрок.ДатаРождения) > 1900");
	...
	Для тЭлемент=1 По 3 Цикл
		...
		Пока Шагомер < Отчет.КоличествоНедель * 7 Цикл
			Итера	= Итера + 1;
			Если Отчет.ТолькоВыходныеДни И ДеньНедели(Отчет.НачальнаяДата + Итера * 86400) < 6 Тогда
				Продолжить;
			КонецЕсли;
			Выборка.Сбросить();
			...
			КонецЦикла;
			График.УстановитьЗначение(График.УстановитьТочку(Формат(Отчет.НачальнаяДата + Итера * 86400, "ДФ=dd.MM")), Серия, Окр(ВесИтого / КолИтого, 1));
...
Процедура КомандаИлиИгрокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Игроки") Тогда
		...
		ОткрытьФорму("Справочник.Игроки.ФормаВыбора", Новый Структура("Отбор", Новый Структура("ТекущаяКоманда", СерверныйФункции.Команда(Отчет.КомандаИлиИгрок, Отчет.НачальнаяДата))), Элемент);
	КонецЕсли;
КонецПроцедуры
