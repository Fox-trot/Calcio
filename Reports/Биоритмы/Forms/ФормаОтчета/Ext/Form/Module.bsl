...
Процедура КомандаИлиИгрокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Игроки") Тогда
		...
		ДанныеВыбора.Вставить("Отбор", Новый Структура("ТекущаяКоманда", Команда(Отчет.КомандаИлиИгрок)));
...
&НаСервере
Функция Команда(Игрок)
	Возврат  СерверныйФункции.Команда(Игрок, Отчет.НачальнаяДата);
КонецФункции
