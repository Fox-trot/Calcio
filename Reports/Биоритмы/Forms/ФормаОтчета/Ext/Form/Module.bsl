
&НаСервере
Функция МатчиПолучить()
	Ответ	= Новый Соответствие;
	Запрос	= Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 11
	      	               |	НАЧАЛОПЕРИОДА(МатчКоманды.Ссылка.Дата, ДЕНЬ) КАК Дата,
	      	               |	МатчКоманды.Ссылка КАК Матч
	      	               |ИЗ
	      	               |	Документ.Матч.Команды КАК МатчКоманды
	      	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПеремещенияИгроков.СрезПоследних(, Игрок = &КомандаИлиИгрок) КАК Перемещения
	      	               |		ПО МатчКоманды.Команда = Перемещения.Команда
	      	               |ГДЕ
	      	               |	МатчКоманды.Ссылка.Дата >= &Дата
	      	               |	И МатчКоманды.Ссылка.ПометкаУдаления = ЛОЖЬ
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	Дата");
	Если ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Команды") Тогда
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 11
		               |	НАЧАЛОПЕРИОДА(МатчКоманды.Ссылка.Дата, ДЕНЬ) КАК Дата,
		               |	МатчКоманды.Ссылка КАК Матч
		               |ИЗ
		               |	Документ.Матч.Команды КАК МатчКоманды
		               |ГДЕ
		               |	МатчКоманды.Команда = &КомандаИлиИгрок
		               |	И МатчКоманды.Ссылка.Дата >= &Дата
		               |	И МатчКоманды.Ссылка.ПометкаУдаления = ЛОЖЬ
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Дата";
	КонецЕсли;
	Запрос.УстановитьПараметр("КомандаИлиИгрок",	Отчет.КомандаИлиИгрок);
	Запрос.УстановитьПараметр("Дата",				НачальнаяДата(Отчет.НачальнаяДата));
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Ответ.Вставить(Выборка.Дата, Выборка.Матч);
	КонецЦикла;
	Возврат Ответ;
КонецФункции

&НаСервереБезКонтекста
Функция НачальнаяДата(НачальнаяДата)
	Возврат ?(ЗначениеЗаполнено(НачальнаяДата), НачальнаяДата, ТекущаяДатаСеанса());
КонецФункции

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      |	Перемещения.Период КАК Дата
	                      |ИЗ
	                      |	РегистрСведений.ПеремещенияИгроков КАК Перемещения
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Дата");
	Если Параметры.Свойство("Игрок") Тогда
		Отчет.КомандаИлиИгрок	= Параметры.Игрок;
		Отчет.НачальнаяДата		= НачальнаяДата(Отчет.НачальнаяДата);
		
		ОбщегоНазначения.ЗаголовокСкрыть(ЭтаФорма, Элементы.КомандаИлиИгрок);

	ИначеЕсли Запрос.Выполнить().Пустой() Тогда
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ГрафикОбновитьНаСервере()
	Если НЕ ПроверитьЗаполнение() Тогда Возврат; КонецЕсли;
	
	Если Отчет.КоличествоНедель = 0 Тогда
		Отчет.КоличествоНедель = 5;
	КонецЕсли;
	График.Очистить();
	
	Выборка = ГрафикПолучитьНаСервере();
	Если Выборка.Количество() = 0 Тогда Возврат; КонецЕсли;
	
	Количество	= ?(ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Команды"), 0, Выборка.Получить(0));
	Матчи		= МатчиПолучить();
	Для Фаза = 1 По 3 Цикл
		Серия	= График.УстановитьСерию(ФЭИ(Фаза));
		Серия.Цвет	= Цвет(Фаза);
		Итера		= -1;
		Месяц		= 0;
		Шагомер		= 0;
		Пока Шагомер < Отчет.КоличествоНедель * 7 Цикл
			Итера	= Итера + 1;
			Дата	= НачальнаяДата(Отчет.НачальнаяДата) + Сутки(Итера);
			Если Отчет.ТолькоВыходныеДни И ДеньНедели(Дата) < 6 Тогда
				Продолжить;
			КонецЕсли;
			Матч	= Матчи.Получить(Дата);
			Если ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Команды") Тогда
				ВесИтого	= 0;
				Для Каждого Количество Из Выборка Цикл
					ВесИтого	= ВесИтого + ФазаРасчитать2(Количество + Итера, Фаза);
				КонецЦикла;
				Точка				= График.УстановитьТочку(Дата);
				График.УстановитьЗначение(Точка, Серия, ВесИтого);
			Иначе
				Точка				= График.УстановитьТочку(Дата);
				График.УстановитьЗначение(Точка, Серия, ФазаРасчитать2(Количество + Итера, Фаза));
			КонецЕсли;
			Если Месяц = Месяц(Точка.Значение) Тогда
				Точка.Текст = Формат(Точка.Значение, "ДФ=d");
			Иначе
				Точка.Текст	= Формат(Точка.Значение, "ДФ=d.MMM");
				Месяц = Месяц(Точка.Значение);
			КонецЕсли;
			Если ЗначениеЗаполнено(Матч) Тогда
				Точка.Расшифровка	= Матч;
				Точка.Цвет			= ЦветаСтиля.ЦветОсобогоТекста;
			КонецЕсли;
			
			Шагомер	= Шагомер + 1;
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры

&НаСервереБезКонтекста
Функция Цвет(Фаза)
	Возврат НаСервереПовторно.Цвет(Фаза);
КонецФункции

&НаСервереБезКонтекста
Функция ФЭИ(Элемент)
	Возврат НаСервереПовторно.ФЭИ(Элемент);
КонецФункции

&НаСервере
Функция ФазаРасчитать2(Количество, Фаза)
	Возврат Биоритмы.ФазаРасчитать2(Количество, Фаза);
КонецФункции

&НаСервере
Функция ГрафикПолучитьНаСервере()
	Если НЕ ЗначениеЗаполнено(Отчет.КомандаИлиИгрок) Тогда Возврат Новый Массив; КонецЕсли;
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               |	РАЗНОСТЬДАТ(Игроки.ДатаРождения, &Дата, ДЕНЬ) КАК Количество
	      	               |ИЗ
	      	               |	Справочник.Игроки КАК Игроки
	      	               |ГДЕ
	      	               |	Игроки.Ссылка = &КомандаИлиИгрок
	      	               |	И ГОД(Игроки.ДатаРождения) >= 1900");
	Если ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Команды") Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	РАЗНОСТЬДАТ(Игроки.ДатаРождения, &Дата, ДЕНЬ) КАК Количество
		               |ИЗ
		               |	РегистрСведений.ПеремещенияИгроков.СрезПоследних(
		               |			,
		               |			Игрок.ПометкаУдаления = ЛОЖЬ
		               |				И ГОД(Игрок.ДатаРождения) >= 1900) КАК Перемещения
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Игроки КАК Игроки
		               |		ПО Перемещения.Игрок = Игроки.Ссылка
		               |ГДЕ
		               |	Перемещения.Команда = &КомандаИлиИгрок
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Количество";
	КонецЕсли;
	Запрос.УстановитьПараметр("КомандаИлиИгрок",	Отчет.КомандаИлиИгрок);
	Запрос.УстановитьПараметр("Дата",				НачальнаяДата(Отчет.НачальнаяДата));
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку(0);
КонецФункции

&НаКлиенте
Процедура ПриОткрытии(Отказ)
	Если НЕ ЗначениеЗаполнено(Отчет.КомандаИлиИгрок) Тогда
		Отчет.КомандаИлиИгрок	= МояКоманда();
		Отчет.НачальнаяДата		= НачальнаяДата(Отчет.НачальнаяДата);
	КонецЕсли;
	
	ГрафикОбновитьНаСервере();
	
	Если ЗначениеЗаполнено(Отчет.КомандаИлиИгрок)
	И График.Точки.Количество() = 0
	Тогда
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура КомандаИлиИгрокПриИзменении(Элемент)
	ГрафикОбновитьНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура КомандаИлиИгрокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ДанныеВыбора = Новый Структура("ТолькоПросмотр", Истина);
	Если ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Игроки") Тогда
		Если ЗначениеЗаполнено(Отчет.КомандаИлиИгрок) Тогда
			ДанныеВыбора.Вставить("Отбор", Новый Структура("ТекущаяКоманда", КомандаИгрока(Отчет.КомандаИлиИгрок, НачальнаяДата(Отчет.НачальнаяДата))));
		КонецЕсли;
		НаКлиенте.ОбработкаКомандыОткрытьФорму("Справочник.Игроки.ФормаВыбора", ДанныеВыбора, Элемент, СтандартнаяОбработка);
	ИначеЕсли ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Команды") Тогда
		Если ЗначениеЗаполнено(Отчет.КомандаИлиИгрок) Тогда
			ДанныеВыбора.Вставить("ТекущаяСтрока", Отчет.КомандаИлиИгрок);
		Иначе
			ДанныеВыбора.Вставить("ТекущаяСтрока", МояКоманда());
		КонецЕсли;
		НаКлиенте.ОбработкаКомандыОткрытьФорму("Справочник.Команды.ФормаВыбора", ДанныеВыбора, Элемент, СтандартнаяОбработка);
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Функция КомандаИгрока(Игрок, Дата)
	Возврат СерверныйФункции.КомандаИгрока(Игрок, Дата);
КонецФункции

&НаСервереБезКонтекста
Функция МояКоманда()
	Возврат СерверныйФункции.МояКомандаПолучить();
КонецФункции
