	...
	Для тЭлемент=1 По 3 Цикл
		...
		Пока Шагомер < Отчет.КоличествоНедель * 7 Цикл
			...
			Для Каждого ТекСтрока Из Выборка Цикл
				КолИтого	= КолИтого + 1;
				ВесИтого	= ВесИтого + Биоритмы.ФазаРасчитать2(ТекСтрока.Количество + Итера, тЭлемент);
			КонецЦикла;
...
Процедура КомандаИлиИгрокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Игроки") Тогда
		...
		Если ЗначениеЗаполнено(Отчет.КомандаИлиИгрок) Тогда
			ДанныеВыбора.Вставить("Отбор", Новый Структура("ТекущаяКоманда", Команда(Отчет.КомандаИлиИгрок, Отчет.НачальнаяДата)));
		КонецЕсли;
...
Функция Команда(Игрок, Дата)
	Возврат СерверныйФункции.Команда(Игрок, Дата);
КонецФункции
