...
Процедура ГрафикОбновитьНаСервере()
	...
	Для Фаза = 1 По 3 Цикл
		...
		Пока Шагомер < Отчет.КоличествоНедель * 7 Цикл
			Итера	= Итера + 1;
			Если Отчет.ТолькоВыходныеДни И ДеньНедели(НачальнаяДата(Отчет.НачальнаяДата) + Сутки(Итера)) < 6 Тогда
				Продолжить;
			КонецЕсли;
			Если Количество = 0 Тогда
				...
				КонецЦикла;
				Точка				= График.УстановитьТочку(НачальнаяДата(Отчет.НачальнаяДата) + Сутки(Итера));
