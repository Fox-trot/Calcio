...
&НаСервере
Функция НачальнаяДата()
	Возврат ?(ЗначениеЗаполнено(Отчет.НачальнаяДата), Отчет.НачальнаяДата, ТекущаяДатаСеанса());
КонецФункции
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Игрок") Тогда
		...
		Отчет.НачальнаяДата		= НачальнаяДата();
...
Процедура ГрафикОбновитьНаСервере()
	...
	Для Фаза = 1 По 3 Цикл
		...
		Пока Шагомер < Отчет.КоличествоНедель * 7 Цикл
			Итера	= Итера + 1;
			Если Отчет.ТолькоВыходныеДни И ДеньНедели(НачальнаяДата() + Сутки(Итера)) < 6 Тогда
				Продолжить;
			КонецЕсли;
			Если Количество = 0 Тогда
				...
				КонецЦикла;
				Точка				= График.УстановитьТочку(НачальнаяДата() + Сутки(Итера));
...
Процедура КомандаИлиИгрокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипЗнч(Отчет.КомандаИлиИгрок) = Тип("СправочникСсылка.Игроки") Тогда
		...
		Если ЗначениеЗаполнено(Отчет.КомандаИлиИгрок) Тогда
			ДанныеВыбора.Вставить("Отбор", Новый Структура("ТекущаяКоманда", КомандаИгрока(Отчет.КомандаИлиИгрок, НачальнаяДата())));
		КонецЕсли;
