...
Функция Победа()
	...
	ИначеЕсли Победа = 0 Тогда
		Победа = Справочники.ВидыСпорта.КоличествоОчков(Отчет.Чемпионат).Победа;
	КонецЕсли;
	Возврат Победа;
КонецФункции
...
&НаСервере
Функция ШагШкалы()
	Если Отчет.Данные.Количество() = 0 Тогда Возврат 1; КонецЕсли;
...
Процедура ГрафикОбновить()
	Если Элементы.График.Видимость Тогда
		...
		КонецЕсли;
		График.ОбластьПостроения.ШкалаЗначений.ШагШкалы = ?(Сравнить И ПоказыватьРазницуМячей, 0, ?(ПоказыватьРазницуМячей, 2 * Победа(), ШагШкалы()));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	ЕстьКлюч = НЕ ПустаяСтрока(КлючУникальности);
	Если ЕстьКлюч Тогда
...
Процедура ВидимостьУстановить()
	...
	Иначе
		...
		Элементы.ТипМатчей.Видимость 				= ГрафикВидимость;
		Элементы.График.Видимость					= ГрафикВидимость;
		Элементы.Сравнить.Видимость					= НЕ ЕстьКлюч И НЕ ЧемпионатПустой;
