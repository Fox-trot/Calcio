...
Процедура СформироватьНаСервере()
	Если ЗначениеЗаполнено(Отчет.Чемпионат) Тогда
		Результат = РеквизитФормыВЗначение("Отчет").ОтчетСформировать(Отчет, ПоказыватьРазницуМячей, Сравнить И Элементы.Сравнить.Видимость, Элементы.График.Видимость);
...
&НаСервере
Функция Победа()
	Если Отчет.Данные.Количество() = 0
		Тогда Возврат 1;
	ИначеЕсли Победа = 0 Тогда
		Победа = НаСервереПовторно.Победа(Отчет.Чемпионат);
	КонецЕсли;
	Возврат Победа;
КонецФункции
...
Функция ШагШкалы()
	...
	ИначеЕсли ПоказыватьРазницуМячей Тогда
		Возврат ?(Отчет.Данные.Получить(0).КоличествоОчков > 2 * Победа(), 2 * Победа(), Победа());
	КонецЕсли;
	Возврат ?(Отчет.Данные.Получить(0).КоличествоГолов + Отчет.Данные.Получить(0).КоличествоЗабито > 55, 10, 5);
КонецФункции
...
&НаКлиенте
Процедура КомандаСформировать(Команда=Неопределено)
...
Функция ЧемпионатыПолучить()
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               ...
	      	               |	НЕ Итоги.Чемпионат ЕСТЬ NULL КАК Пометка
	      	               |ИЗ
	      	               |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
	      	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Итоги КАК Итоги
	      	               |		ПО ЧемпионатыКоманды.Ссылка = Итоги.Чемпионат
	      	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Лига.Команды КАК ЛигаКоманды
	      	               |		ПО ЧемпионатыКоманды.Ссылка.Владелец = ЛигаКоманды.Ссылка
	      	               |ГДЕ
	      	               |	ЧемпионатыКоманды.Ссылка.ПометкаУдаления = ЛОЖЬ
	      	               |
	      	               |СГРУППИРОВАТЬ ПО
	      	               |	ЧемпионатыКоманды.Ссылка,
	      	               |	Итоги.Чемпионат,
	      	               |	ЛигаКоманды.Ссылка
	      	               |
	      	               |ИМЕЮЩИЕ
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("График") Тогда
		ГрафикВидимость	= Параметры.График;
	Иначе
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если СтрСравнить(ИмяСобытия, "ТурнирнаяТаблица") = 0 Тогда
...
Процедура ВидимостьУстановить()
	...
	Иначе
		ЧемпионатПустой			= НаСервереПовторно.ЧемпионатПустой(Отчет.Чемпионат);
		Элементы.ПоказыватьРазницуМячей.Видимость	= Истина;
		Элементы.ТипМатчей.Видимость 				= ГрафикВидимость;
		Элементы.График.Видимость					= ГрафикВидимость;
		Элементы.Сравнить.Видимость					= (ПоложениеКоманднойПанели <> ПоложениеКоманднойПанелиФормы.Нет И НЕ ЧемпионатПустой);
		
		Если Сравнить И ЧемпионатПустой Тогда Сравнить = Ложь; КонецЕсли;
	КонецЕсли;
КонецПроцедуры
