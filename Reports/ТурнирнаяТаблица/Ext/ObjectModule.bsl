...
Функция ОтчетСформировать(Объект, ПоказыватьРазницуМячей, ОставшиесяМатчи=Ложь, Сравнить=Ложь) Экспорт
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			Паст = Новый Структура("КоличествоОчков,КоличествоГолов", 0, 0);
			Если Сравнить Тогда
				...
				Иначе
					...
					Паст.КоличествоГолов	= ТекСтрока.КоличествоЗабито;
					Паст.КоличествоОчков	= 3 * ТекСтрока.Выигрыш + ТекСтрока.Ничья;
					Если ПоказыватьРазницуМячей Тогда
						ОбластьМакета.Параметры.КоличествоОчков	= Паст.КоличествоОчков;
					Иначе
						ОбластьМакета.Параметры.КоличествоОчков	= Разница(ТекСтрока);
					КонецЕсли;
				КонецЕсли;
				ТабДокумент.Присоединить(ОбластьМакета);
			КонецЕсли;
			
			ТекСтрока = Объект.Данные.Добавить();
			ЗаполнитьЗначенияСвойств(ТекСтрока, Выборка);
			ЗаполнитьЗначенияСвойств(ТекСтрока, Паст);
		КонецЦикла;
