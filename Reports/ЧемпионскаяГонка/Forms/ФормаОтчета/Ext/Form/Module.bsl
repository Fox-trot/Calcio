...
Процедура ГрафикОбновитьНаСервере()
	...
	График.ОбластьЗаголовка.Текст = ?(ТипГрафика = -1 Или ТипГрафика = 2, "Меньше - лучше", "");
	График.ОбластьПостроения.ШкалаЗначений.ШагШкалы = ?(ТипГрафика = -1 Или ТипГрафика >= 4, 0, ?(ТипГрафика=0, 2 * Справочники.ВидыСпорта.Победа(Отчет.Лига), 10));
	Для Каждого Детали Из Отчет.Данные Цикл
		Если ЗначениеЗаполнено(Отчет.Команда) И ТипГрафика = -1 Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Отчет.Команда) И ТипГрафика >= 4 Тогда
			Если Период = Детали.Период Тогда
				Количество	= Количество + 1;
			Иначе
				Количество	= 1;
				Период		= Детали.Период;
			КонецЕсли;
			График.УстановитьЗначение(УстановитьТочку(Детали), УстановитьСерию(Детали.Период), Окр(?(ТипГрафика = 4, Детали.КоличествоЗабито, Детали.КоличествоОчков) / Количество, 2), Детали.Матч);
		...
		ИначеЕсли ТипГрафика >= 4 Тогда
			График.УстановитьЗначение(УстановитьТочку(Детали), УстановитьСерию(Детали.Команда), ?(ТипГрафика = 4, Детали.КоличествоЗабито, Детали.КоличествоОчков) / 100);
		Иначе
...
Функция УстановитьТочку(Детали)
	Если ЗначениеЗаполнено(Отчет.Команда) Тогда
		...
		Точка.Расшифровка	= Отчет.Команда;
	Иначе
		Если ЗначениеЗаполнено(Детали.Тур) Тогда
			...
			КонецЕсли;
			Точка.Расшифровка	= Детали.Тур.Владелец;
		Иначе
			Точка = График.УстановитьТочку(ФорматЧГ(Детали.Период));
			Точка.Расшифровка	= ЧемпионатНайти(Отчет.Лига, Детали.Период);
		КонецЕсли;
	КонецЕсли;
	Возврат Точка;
КонецФункции
...
&НаСервере
Функция ЧемпионатНайти(Параметр, Дата)
	Возврат СерверныйФункции.ЧемпионатНайти(Параметр, Дата);
КонецФункции
...
Функция УстановитьСерию(Параметр)
	Если ТипЗнч(Параметр) = Тип("Дата") Тогда
		...
		Если ЗначениеЗаполнено(Отчет.Команда) Тогда
			Серия.Расшифровка = ЧемпионатНайти(Отчет.Команда, Параметр);
		КонецЕсли;
	Иначе
...
Функция Значение(Детали)
	Возврат ?(ТипГрафика = 3, Детали.КоличествоЗабито - Детали.КоличествоПропущено, ?(ТипГрафика = 1, Детали.КоличествоЗабито, ?(ТипГрафика = 2, Детали.КоличествоПропущено, Детали.КоличествоОчков)));
КонецФункции
...
Процедура ДанныеПолучить()
	...
	ИначеЕсли ТипГрафика >= 4 Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 5
		               ...
		               |	ВЫБОР
		               |		КОГДА ТурнирнаяТаблица.ТурКод = ВложенныйЗапрос.ТурКод
		               |			ТОГДА NULL
		               |		ИНАЧЕ ТурнирнаяТаблица.Тур
		               |	КОНЕЦ КАК Тур,
		               |	ТурнирнаяТаблица.Команда КАК Команда,
		               |	100 * ОКР(ТурнирнаяТаблица.КоличествоОчков / ТурнирнаяТаблица.Количество, 2) КАК КоличествоОчков,
		               |	100 * ОКР(ТурнирнаяТаблица.КоличествоЗабито / ТурнирнаяТаблица.Количество, 2) КАК КоличествоЗабито
		               |ИЗ
		               |	ТурнирнаяТаблица КАК ТурнирнаяТаблица
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		               |			МАКСИМУМ(ВременнаяТаблица.ТурКод) КАК ТурКод
		               |		ИЗ
		               |			Чемпионаты КАК ВременнаяТаблица) КАК ВложенныйЗапрос
		               |		ПО (ИСТИНА)
		               |ГДЕ
		               ...
		               |			ГДЕ
		               |				Клубы.Ссылка = &Лига)
		               |	И ТурнирнаяТаблица.КоличествоЗабито > 0
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Период,
		               |	Команда
...
Процедура НастройкиУстановить()
	...
	Элементы.ТипГрафика.СписокВыбора.Добавить(4, "Средняя результативность",, БиблиотекаКартинок.Goal);
	Элементы.ТипГрафика.СписокВыбора.Добавить(5, "Соотношение очки / матч");
