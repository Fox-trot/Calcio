...
Процедура ГрафикОбновитьНаСервере(ИспользоватьКеш=Ложь)
	...
	Для Каждого Детали Из Отчет.Данные Цикл
		Если ЗначениеЗаполнено(Отчет.Команда) Тогда
			График.УстановитьЗначение(График.УстановитьТочку(Детали.Тур), График.УстановитьСерию(ФорматЧГ(Детали.Период)), Значение(Детали), Детали.Матч, Детали.Матч);
		Иначе
...
Процедура ДанныеПолучить()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 3
	                      ...
	                      |	ТурнирнаяТаблица.Тур.Владелец.ДатаНачала КАК ДатаНачала,
	                      ...
	                      |	ТурнирнаяТаблица.Тур.Владелец.ДатаНачала,
	                      ...
	                      |		ПО ТурнирнаяТаблица.Период >= Чемпионаты.ДатаНачала
	                      |			И ТурнирнаяТаблица.Тур.Владелец = Чемпионаты.Чемпионат
