...
Процедура ГрафикОбновитьНаСервере()
	...
	Для Каждого Детали Из Отчет.Данные Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(Отчет.Команда) И ТипГрафика = 4 Тогда
			...
			КонецЕсли;
			График.УстановитьЗначение(УстановитьТочку(Детали), УстановитьСерию(Детали.Период), Окр(Детали.КоличествоОчков / НомерСтроки, 2), Детали.Матч);
		...
		ИначеЕсли ТипГрафика = 4 Тогда
			График.УстановитьЗначение(УстановитьТочку(Детали), УстановитьСерию(Детали.Команда), Детали.КоличествоОчков / 100);
		Иначе
...
Процедура ДанныеПолучить()
	...
	ИначеЕсли ТипГрафика = 4 Тогда
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 5
		               ...
		               |	КОНЕЦ КАК Тур,
		               |	ТурнирнаяТаблица.Команда КАК Команда,
		               |	100 * ОКР(ТурнирнаяТаблица.КоличествоОчков / ТурнирнаяТаблица.КоличествоЗабито, 2) КАК КоличествоОчков
		               |ИЗ
