...
Процедура ГрафикОбновитьНаСервере()
	...
	График.ОбластьЗаголовка.Текст = ?(Показатель = -1 Или Показатель = 2, "Меньше - лучше", "");
	График.ОбластьПостроения.ШкалаЗначений.ШагШкалы = ?(Показатель=-1, 0, ?(Показатель=0, 2 * Справочники.ВидыСпорта.Победа(Отчет.Лига), 10));
	Для Каждого Детали Из Отчет.Данные Цикл
		Если ЗначениеЗаполнено(Отчет.Команда) И Показатель = -1 Тогда
...
Функция УстановитьТочку(Детали)
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	Точка.Расшифровка	= ?(ЗначениеЗаполнено(Отчет.Команда), Детали.Тур, Детали.Тур.Владелец);
	Возврат Точка;
КонецФункции
...
Функция Значение(Детали)
	Возврат ?(Показатель=0, Детали.КоличествоОчков, ?(Показатель=1, Детали.КоличествоЗабито, ?(Показатель=2, Детали.КоличествоПропущено, Детали.КоличествоЗабито - Детали.КоличествоПропущено)));
КонецФункции
...
Процедура НастройкиУстановить()
	...
	Элементы.Показатель.СписокВыбора.Добавить(3, "Разница голов");
	Элементы.Показатель.СписокВыбора.Добавить(-1, "Занятое место");
КонецПроцедуры
