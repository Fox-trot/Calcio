...
Функция ОтчетСформировать(Объект, ПоказыватьРазницуМячей=Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Чемпионат) Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		ИНАЧЕ 0
		               |	КОНЕЦ КАК Выезд,
		               |	ЕСТЬNULL(Матчи.КоличествоГолов, 0) КАК КоличествоЗабито,
		               |	ЕСТЬNULL(Противник.КоличествоГолов, 0) КАК КоличествоПропущено,
		               |	ЕСТЬNULL(Матчи.КоличествоОчков, 0) КАК КоличествоОчков,
		               |	ВЫБОР
		               |		КОГДА Матчи.КоличествоОчков > Противник.КоличествоОчков
		               |			ТОГДА 1
		               |		ИНАЧЕ 0
