...
Процедура ГрафикОбновитьНаСервере(Пошагам=Ложь)
	Если Элементы.График.Видимость Тогда
		Если ?(Пошагам, Список.Количество() = 0, Истина) Тогда
			...
			КонецЦикла;
			График.ОбластьПостроения.ШкалаЗначений.ШагШкалы = Отчет.Чемпионат.Владелец.Владелец.Победа;
		КонецЕсли;
			...
			Для Итератор = 0 По 3 Цикл
				...
				Для Каждого ТекСтрока Из мСтроки Цикл
					Если ТурКод = ТекСтрока.ТурКод Тогда
						График.УстановитьЗначение(График.УстановитьТочку(ТекСтрока.Команда), График.УстановитьСерию(ТекСтрока.ТурКод), ТекСтрока.КоличествоОчков);
					Иначе
						График.УстановитьЗначение(График.УстановитьТочку(ТекСтрока.Команда), График.УстановитьСерию(ТекСтрока.ТурКод), ТекСтрока.КоличествоОчков, Новый Структура("Команда", ТекСтрока.Команда));
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
		Иначе
...
Процедура ГрафикОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		СтандартнаяОбработка = Ложь;
		Матчи = МатчиПолучить(Расшифровка.Команда);
