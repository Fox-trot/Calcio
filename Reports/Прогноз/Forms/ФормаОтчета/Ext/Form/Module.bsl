...
Процедура ГрафикОбновитьНаСервере(Пошагам=Ложь)
	Если Элементы.График.Видимость Тогда
		Если ?(Пошагам, Список.Количество() = 0, Истина) Тогда
			...
			КонецЦикла;
			График.ОбластьПостроения.ШкалаЗначений.ШагШкалы = 2 * Справочники.ВидыСпорта.Победа(Отчет.Чемпионат);
		КонецЕсли;
...
Процедура ПросмотрИсходныхДанных(Команда)
	...
	Элементы.ГруппаРезультаты.Видимость	= (НЕ Элементы.ПросмотрИсходныхДанных.Пометка);
	Элементы.ГруппаФазы.Видимость		= Элементы.ПросмотрИсходныхДанных.Пометка;
КонецПроцедуры
...
Процедура ФазыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	НаКлиенте.Показать(Элемент.ТекущиеДанные.Матч);
КонецПроцедуры
