...
Функция ОтчетСформировать(Отчет=Неопределено, Все=Ложь) Экспорт
	...
	Если ОлимпийскаяСистема Тогда
		ТабДокумент.Вывести(Макет.ПолучитьОбласть("Шапка1|НачалоП"));
	Иначе
	...
	Пока Команды.Следующий() Цикл
		...
		ОбластьМакета = Макет.ПолучитьОбласть(?(ОлимпийскаяСистема, "ДеталиП|НачалоП", "Детали1|Начало"));
		ОбластьМакета.Параметры.Заполнить(Команды);
		Если НЕ ПоказыватьРазницуМячей И НЕ ОлимпийскаяСистема Тогда
