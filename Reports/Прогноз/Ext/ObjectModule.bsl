...
Функция ОтчетСформировать(Отчет=Неопределено, Все=Ложь) Экспорт
	...
	Пока Команды.Следующий() Цикл
		...
		КонецЕсли;
		СерверныйФункции.КартинкаУстановить(ОбластьМакета, Команды.Команда);
					...
					Если мСтроки.Количество() = 0 Тогда
						...
						Иначе
							...
							Если ТекСтрока.КоличествоЗабито < 0 Тогда
								...
								Итого.КоличествоОчков = Итого.КоличествоОчков + ?(ТекСтрока.КоличествоОчков=Ничья(), ТекСтрока.КоличествоОчков, ?(ТекСтрока.КоличествоОчков=0, Победа(), 0));
							Иначе
	...
	Отчет.Фазы.Сортировать("ТурКод,Команда");
	
	ТабДокумент.ФиксацияСверху	= 3;
	Если Туры.Количество() > 3 Тогда
...
Функция ДанныеПолучить(Все=Ложь) Экспорт
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 5
	      	               ...
	      	               |	МатчКоманды.Команда КАК Команда,
	      	               |	ОКР(ЕСТЬNULL(Топы.Корреляция, 0) + ЕСТЬNULL(Прошлое.Корреляция, 1), 1) КАК Корреляция,
	      	               |	МатчКоманды.НомерСтроки КАК НомерСтроки,
	      	               |	МатчКоманды.Ссылка.Тур.ОлимпийскаяСистема КАК ОлимпийскаяСистема,
	      	               |	ЕСТЬNULL(МатчКоманды.Ссылка.Стадион.Ссылка, Стадионы.Ссылка) КАК Арена,
	      	               ...
	      	               |						ТОГДА 0
	      	               |					ИНАЧЕ 1
	      	               |				КОНЕЦ) / КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Регистратор) + СУММА(ВЫБОР
	      	               |					КОГДА ТурнирнаяТаблица.КоличествоГолов = 0
	      	               |						ТОГДА 0
	      	               |					ИНАЧЕ 1
	      	               |				КОНЕЦ) / КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТурнирнаяТаблица.Регистратор) - СУММА(ВЫБОР
	      	               ...
	      	               |						ТОГДА 0
	      	               |					ИНАЧЕ 1
	      	               |				КОНЕЦ) / КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Противники.Регистратор) КАК Корреляция
	      	               |		ИЗ
	      	               ...
	      	               |		ГДЕ
	      	               |			ТурнирнаяТаблица.Период МЕЖДУ &ДатаН И &Дата
	      	               |			И Туры.Владелец = &Чемпионат
	      	               |		
	      	               |		СГРУППИРОВАТЬ ПО
	      	               |			ТурнирнаяТаблица.Команда) КАК Прошлое
	      	               |		ПО МатчКоманды.Команда = Прошлое.Команда
	      	               |ГДЕ
	      	               ...
	      	               |					ГДЕ
	      	               |						Составы.Ссылка.Дата >= &ДатаН
	      	               |						И Составы.Ссылка.ПометкаУдаления = ЛОЖЬ
	      	               ...
	      	               |	ВЫБОР
	      	               |		КОГДА ОКР(ВТ.Фаза1 + ВТ.Фаза2 + ВТ.Фаза3 + ВТ.Корреляция, 1) - ОКР(ВТ2.Фаза1 + ВТ2.Фаза2 + ВТ2.Фаза3 + ВТ2.Корреляция, 1) >= 5
	      	               |			ТОГДА &Победа
	      	               |		КОГДА ОКР(ВТ.Фаза1 + ВТ.Фаза2 + ВТ.Фаза3 + ВТ.Корреляция, 1) - ОКР(ВТ2.Фаза1 + ВТ2.Фаза2 + ВТ2.Фаза3 + ВТ2.Корреляция, 1) <= -5
	      	               |			ТОГДА 0
	      	               |		ИНАЧЕ 1
	      	               ...
	      	               |	КОНЕЦ КАК Результат,
	      	               |	ВЫБОР
	      	               |		КОГДА ОКР(ВТ.Фаза1 + ВТ.Фаза2 + ВТ.Фаза3 + ВТ.Корреляция, 1) - ОКР(ВТ2.Фаза1 + ВТ2.Фаза2 + ВТ2.Фаза3 + ВТ2.Корреляция, 1) >= 0
	      	               |			ТОГДА ОКР(ВТ.Фаза1 + ВТ.Фаза2 + ВТ.Фаза3 + ВТ.Корреляция, 1) - ОКР(ВТ2.Фаза1 + ВТ2.Фаза2 + ВТ2.Фаза3 + ВТ2.Корреляция, 1)
	      	               |		ИНАЧЕ ОКР(ВТ2.Фаза1 + ВТ2.Фаза2 + ВТ2.Фаза3 + ВТ2.Корреляция, 1) - ОКР(ВТ.Фаза1 + ВТ.Фаза2 + ВТ.Фаза3 + ВТ.Корреляция, 1)
	      	               |	КОНЕЦ КАК Разница,
	      	               ...
	      	               |						ТОГДА -1
	      	               |					ИНАЧЕ 0
	      	               |				КОНЕЦ) + ЦЕЛ(СУММА(ВЫБОР
	      	               |						КОГДА ВТ.Фаза1 >= 0
	      	               |								И ВТ.Фаза2 >= 0
	      	               |								И ВТ.Фаза3 >= 0
	      	               |								И ВТ.Фаза1 + ВТ.Фаза2 + ВТ.Фаза3 > 0
	      	               |							ТОГДА 0.34
	      	               |						КОГДА ВТ.Фаза1 <= 0
	      	               |								И ВТ.Фаза2 <= 0
	      	               |								И ВТ.Фаза3 <= 0
	      	               ...
	      	               |			И Матчи.Команда <> Противники.Команда
	      	               |ГДЕ
	      	               |	Матчи.НомерСтроки = 1
	...
	Запрос.УстановитьПараметр("Победа",				Победа());
...
Функция Ничья()
	Возврат НаСервереПовторно.Ничья(Чемпионат);
КонецФункции
