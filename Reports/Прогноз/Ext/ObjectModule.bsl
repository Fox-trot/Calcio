...
Функция ОтчетСформировать(Отчет=Неопределено) Экспорт
	...
	Пока Команды.Следующий() Цикл
		...
		Команды.Команда, ТекущаяСтрока.ТурКод, Команды.КоличествоОчков));
		ЗаполнитьЗначенияСвойств(Отчет.Данные.Добавить(), Новый Структура("Команда,Тур,КоличествоОчков",
		Команды.Команда, ТурКод, КоличествоОчков - Команды.КоличествоОчков));
	КонецЦикла;
...
Функция ДанныеПолучить()
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               ...
	      	               |	И (&Все
	      	               |			ИЛИ МатчКоманды.Ссылка.Тур.Код В
	      	               ...
	      	               |				(ВЫБРАТЬ
	      	               |					МАКСИМУМ(ТурнирнаяТаблица.Регистратор)
	      	               |				ИЗ
	...
	Запрос.УстановитьПараметр("Дата",				ТекущаяДатаСеанса() - Сутки());
