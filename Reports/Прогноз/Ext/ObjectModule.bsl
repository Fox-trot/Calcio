...
Функция ОтчетСформировать(Отчет=Неопределено, Все=Ложь) Экспорт
	...
	Если ОлимпийскаяСистема Тогда
		ТабДокумент.Вывести(Макет.ПолучитьОбласть("Шапка1|Команды"));
		ТабДокумент.Присоединить(Макет.ПолучитьОбласть("Шапка1|LL"));
	Иначе
	...
	Пока Команды.Следующий() Цикл
		...
		Если НЕ ПоказыватьРазницуМячей Тогда
			ОбластьМакета.Параметры.КоличествоИгр	= СтрШаблон("%1-%2", Команды.КоличествоЗабито, Команды.КоличествоПропущено);
		КонецЕсли;
