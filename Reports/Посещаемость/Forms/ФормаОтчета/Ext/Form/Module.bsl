...
Процедура ДанныеПолучить()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 5
	                      ...
	                      |				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Туры КАК Т2
	                      |				ПО Туры.Владелец = Т2.Владелец
	                      |					И (ЦЕЛ(Туры.Код + 1) / 2 * 2 = Т2.Код)
	                      |		ГДЕ
	                      |			Туры.Владелец = &Чемпионат
	                      |			И ЦЕЛ(Туры.Код / 2) * 2 < Туры.Код
	                      |			И ЦЕЛ(Т2.Код / 2) * 2 = Т2.Код) КАК ВТ
	                      |		ПО (Матч.Тур = ВТ.Ссылка
	                      |				ИЛИ Матч.Тур = ВТ.Ссылка2)
	                      |ГДЕ
	                      |	Матч.Проведен = ИСТИНА
	                      |	И Матч.КоличествоЗрителей >= 1
	                      |	И Матч.Стадион В
	...
	Пока Выборка.Следующий() Цикл
		...
		И ЗначениеЗаполнено(Отчет.Чемпионат)
		И ТипГрафика = 0
		Тогда
			ТекСтрока = Отчет.Данные.Добавить();
			ЗаполнитьЗначенияСвойств(ТекСтрока, Выборка);
			ТекСтрока.Подсказка = СтрШаблон("%1-%2", Выборка.Код, Выборка.Код2);
		Иначе
