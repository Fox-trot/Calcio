	...
	Если ЗначениеЗаполнено(Отчет.Стадион) И ЗначениеЗаполнено(Отчет.Чемпионат) Тогда
		Если ТипГрафика = 1 Тогда
			Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 11
			               ...
			               |	ТурнирнаяТаблица.КоличествоГолов + ТурнирнаяТаблица.Пенальти КАК КоличествоЗабито,
			               |	Противник.КоличествоГолов + Противник.Пенальти КАК КоличествоПропущено
			               |ПОМЕСТИТЬ ВТ
			               |ИЗ
...
Процедура ГрафикОбновитьНаСервере()
	...
	Иначе
		Для Каждого Детали Из Отчет.Данные Цикл
			График.УстановитьЗначение(УстановитьТочку(Формат(Детали.Период, "ДФ=MMM")), УстановитьСерию(Детали.Стадион), Детали.КоличествоЗрителей, Детали.Матч);
		КонецЦикла;
	КонецЕсли;
