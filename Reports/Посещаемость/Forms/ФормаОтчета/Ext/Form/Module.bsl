...
Процедура ДанныеПолучить()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ ПЕРВЫЕ 5
	                      |			Матч.Стадион КАК Стадион,
	                      ...
	                      |			И Матч.КоличествоЗрителей >= Стадионы.КоличествоЗрителей
	                      |ГДЕ
	                      |	Матч.Проведен = ИСТИНА
	                      |	И Матч.Тур.Владелец = &Чемпионат
	                      |
		...
		Иначе
			...
			Для Каждого Детали Из Отчет.Данные Цикл
				Если Детали.КоличествоЗрителей > 0 Тогда
					График.УстановитьЗначение(УстановитьТочку(Детали.Период), Серия, Детали.КоличествоЗрителей);
