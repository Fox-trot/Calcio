...
Процедура ДанныеПолучить()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	МИНИМУМ(ДатаНачальная.Дата) КАК Дата
	                      |ПОМЕСТИТЬ ДатаНачальная
	                      |ИЗ
	                      |	(ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 11
	                      |		НАЧАЛОПЕРИОДА(Матч.Дата, НЕДЕЛЯ) КАК Дата
	                      |	ИЗ
	                      ...
	                      |	Матч.Тур.ДатаОкончания КАК Период,
	                      ...
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатаНачальная КАК ДатаНачальная
	                      |		ПО Матч.Дата >= ДатаНачальная.Дата
	                      |ГДЕ
	                      |	Матч.Проведен = ИСТИНА
	                      |	И Матч.КоличествоЗрителей >= 1
	                      |	И Матч.Тур.Владелец = &Чемпионат
...
Процедура ГрафикОбновить()
	График.Очистить();
	График.ТипДиаграммы	= ?(ТипГрафика = 1, ТипДиаграммы.ГистограммаСНакоплением, ТипДиаграммы.ГистограммаОбъемная);
	...
	Если ЗначениеЗаполнено(Отчет.Стадион) Тогда
		...
		Иначе
			...
			КонецЦикла;
			Если График.Точки.Количество() > 1 И БазовоеЗначение < 999 Тогда
				График.БазовоеЗначение	= Цел(-БазовоеЗначение * 0.999);
			КонецЕсли;
		КонецЕсли;
	...
	График.ПропускатьБазовоеЗначение	= (НЕ ЗначениеЗаполнено(Отчет.Стадион) Или График.БазовоеЗначение > 0);
