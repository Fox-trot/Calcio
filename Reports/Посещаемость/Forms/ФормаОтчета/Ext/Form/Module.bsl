...
Процедура ГрафикОбновить()
	...
	Иначе
		Для Каждого ТекСтрока Из Отчет.Данные Цикл
			Если НЕ ЗначениеЗаполнено(ТекСтрока.Матч) = 0 Тогда
				Серия	= График.УстановитьСерию(ТекСтрока.Стадион);
				
				Выборка	= Отчет.Данные.НайтиСтроки(Новый Структура("Стадион", ТекСтрока.Стадион));
