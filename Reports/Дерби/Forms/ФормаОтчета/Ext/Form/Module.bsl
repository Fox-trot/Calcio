...
Процедура КомандыЗагрузить()
	...
	Отчет.Клубы.Загрузить(Запрос.Выполнить().Выгрузить());
КонецПроцедуры
...
Процедура СформироватьНаСервере()
	Результат = РеквизитФормыВЗначение("Отчет").ОтчетСформировать(Отчет, Отчет.Клубы.Выгрузить(, "Команда").ВыгрузитьКолонку(0));
	ГрафикОбновить();
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.Свойство("Команды") Тогда
		Для Каждого Элемент Из Параметры.Команды Цикл
			ЗаполнитьЗначенияСвойств(Отчет.Клубы.Добавить(), Новый Структура("Команда", Элемент));
		КонецЦикла;
