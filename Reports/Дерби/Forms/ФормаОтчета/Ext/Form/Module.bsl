...
&НаСервере
Функция Победа()
	Возврат Справочники.ВидыСпорта.Победа(Отчет.Чемпионат);
КонецФункции
...
Процедура ГрафикОбновить()
	...
	График.ОбластьПостроения.ШкалаЗначений.ШагШкалы = ?(Отчет.Данные.Количество() > 0 И Отчет.Данные.Получить(0).Выигрыш > 1, Победа(), 1);
КонецПроцедуры
...
Процедура ГородаЗагрузить()
	Города.Очистить();
	Элементы.Город.СписокВыбора.Очистить();
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               |	Команды.Город КАК Ссылка,
	      	               |	НЕ Страны.Ссылка ЕСТЬ NULL КАК Пометка
	      	               |ИЗ
	      	               |	Справочник.Команды КАК Команды
	      	               ...
	      	               |			ЧемпионатыКоманды.Ссылка.Владелец.Страна КАК Ссылка
	      	               |		ИЗ
	      	               |			Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
	      	               |		ГДЕ
	      	               |			ЧемпионатыКоманды.Команда = &Команда
	      	               |			И (ЧемпионатыКоманды.Ссылка.Владелец.Страна ЕСТЬ NULL) = ЛОЖЬ) КАК Страны
	      	               |		ПО Команды.Город.Владелец = Страны.Ссылка
	      	               |ГДЕ
	      	               ...
	      	               |			ГДЕ
	      	               |				ТурнирнаяТаблица.Тур.Владелец.Владелец.Владелец = &ВидСпорта
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Пометка И Элементы.Город.СписокВыбора.Количество() < 11 Тогда
			Элементы.Город.СписокВыбора.Добавить(Выборка.Ссылка);
		КонецЕсли;
		Города.Добавить(Выборка.Ссылка);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("Город") И ЗначениеЗаполнено(Параметры.Город) Тогда
		ВидСпорта	= СерверныйФункции.ВидСпорта(МояКомандаПолучить());
