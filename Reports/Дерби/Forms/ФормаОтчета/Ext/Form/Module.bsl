...
&НаСервере
Функция ШагШкалы()
	Возврат ?(Отчет.Данные.Количество() > 0 И Отчет.Данные.Получить(0).Выигрыш > 1, Справочники.ВидыСпорта.Победа(Отчет.Чемпионат), 1);
КонецФункции
...
&НаСервереБезКонтекста
Функция ВидСпорта(Параметр)
	Возврат СерверныйФункции.ВидСпорта(Параметр);
КонецФункции

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("Город") И ЗначениеЗаполнено(Параметры.Город) Тогда
		ВидСпорта	= ВидСпорта(МояКомандаПолучить());
	...
	Иначе
		...
		ГородаЗагрузить();
	КонецЕсли;
	Если ЗначениеЗаполнено(Город) Или Клубы.Количество() >= 2 Тогда
		СерииЗаполнить();
		ТипГрафикаЗаполнить();
	Иначе
		Отказ = Истина;
