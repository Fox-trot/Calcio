...
Процедура КомандыЗагрузить(ГородМатч)
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	      	               ...
	      	               |ГДЕ
	      	               |	МатчКоманды.Ссылка = &Ссылка
	      	               |	И (МатчКоманды.Команда.Город, МатчКоманды.Команда.Город) В
	      	               |			(ВЫБРАТЬ
	      	               |				МИНИМУМ(МатчКоманды.Команда.Город),
	...
	Если ?(ТипЗнч(ГородМатч) = Тип("СправочникСсылка.Города"), Клубы.Количество() < 2, Клубы.Количество() <> 2) Тогда
		Клубы.Очистить();
	КонецЕсли;
КонецПроцедуры
