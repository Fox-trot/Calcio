...
Функция ОтчетСформировать(Отчет, Команды) Экспорт
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	      	               ...
	      	               |	ЧемпионатыКоманды.Ссылка.Владелец КАК Лига
	      	               |ИЗ
	      	               ...
	      	               |ГДЕ
	      	               |	ЧемпионатыКоманды.Команда В(&Команды)
	      	               |	И ЧемпионатыКоманды.Ссылка.ДатаНачала <= &Дата
	...
	Отчет.Чемпионат	= Выборка.Чемпионат;
	...
	Запрос.Текст	= "ВЫБРАТЬ
	            	  ...
	            	  |	СУММА(ЕСТЬNULL(ТурнирнаяТаблица.КоличествоГолов, 0)) - СУММА(ЕСТЬNULL(Противник.КоличествоГолов, 0)) КАК Разница
	            	  |ИЗ
	            	  |	Справочник.Чемпионаты.Команды КАК ЧемпионатыКоманды
	...
	Если ТурнирнаяТаблицаКакНаBBC() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Пятерка		= 1;
			Матчи = ВстречиПоследние5(Выборка.Команда, Команды, Отчет.Чемпионат);
...
Функция ТурнирнаяТаблицаКакНаBBC()
	Возврат НаСервереПовторно.ТурнирнаяТаблицаКакНаBBC();
КонецФункции
