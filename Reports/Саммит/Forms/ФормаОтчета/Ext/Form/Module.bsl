...
Процедура СформироватьНаСервере()
	Если ЗначениеЗаполнено(Отчет.Чемпионат) Тогда
		...
		График.Очистить();
		Проигрыш		= График.УстановитьСерию("Проигрыш");
		Проигрыш.Цвет	= ЦветаСтиля.ЦветОтрицательногоЧисла;
		Ничья			= График.УстановитьСерию("Ничья");
		Ничья.Цвет		= ЦветаСтиля.ЦветАктивности;
		Выигрыш			= График.УстановитьСерию("Выигрыш");
		Выигрыш.Цвет	= ЦветаСтиля.ЦветАкцента;
		Для Каждого Детали Из Отчет.Данные Цикл
			Точка	= График.УстановитьТочку(Детали.Команда);
			График.УстановитьЗначение(Точка, Проигрыш, Детали.Проигрыш);
			График.УстановитьЗначение(Точка, Ничья, Детали.Ничья);
			График.УстановитьЗначение(Точка, Выигрыш, Детали.Выигрыш);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
