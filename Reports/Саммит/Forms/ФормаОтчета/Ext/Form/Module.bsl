...
Процедура ГрафикОбновить()
	Если Элементы.ГруппаГрафики.Видимость Тогда
		...
		Для Каждого ТекСтрока Из _Команды Цикл
			...
			СерверныйФункции.ГрафикОбновить(мГрафики.Получить(Итератор), Отчет.Данные.НайтиСтроки(Новый Структура("Команда", ТекСтрока.Команда)), НЕ ПоказыватьРазницуМячей);
