...
&НаСервере
Процедура ХронологияОбновитьНаСервере()
	Если Лига = Отчет.Чемпионат.Владелец Тогда Возврат;
	ИначеЕсли НЕ ЗначениеЗаполнено(Отчет.Чемпионат) Тогда Возврат;
	КонецЕсли;
	...
	Хронология.Вывести(СерверныйФункции.ЗаголовокПолучить(Новый Структура("Лига,Чемпионат", Лига, Лига)));
	...
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 11
	      	               ...
	      	               |					И (ТурнирнаяТаблица.НомерСтроки <> ЕСТЬNULL(Противник.НомерСтроки, 0))
...
&НаКлиенте
Процедура ХронологияОбновить() Экспорт
	ХронологияОбновитьНаСервере();
КонецПроцедуры
...
&НаСервере
Функция Победа()
	Возврат НаСервереПовторно.Победа(Отчет.Чемпионат);
КонецФункции
...
Процедура Сформировать(Команда=Неопределено)
	Если ПроверитьЗаполнение() Тогда
		...
		ПодключитьОбработчикОжидания("ХронологияОбновить", 1, Истина);
	КонецЕсли;
КонецПроцедуры
