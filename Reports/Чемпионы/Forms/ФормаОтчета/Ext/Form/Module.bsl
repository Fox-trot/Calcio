...
Процедура ГрафикОбновить(ТолькоГрафик=Ложь)
	...
	ИначеЕсли ЗначениеЗаполнено(Отчет.Команда) Тогда
		...
		Для Каждого Детали Из Отчет.Данные Цикл
			...
			Иначе
				График.УстановитьЗначение(График.УстановитьТочку(ФорматЧГ(Детали.Год)), График.УстановитьСерию(Детали.Команда), Детали.КоличествоОчков);
			КонецЕсли;
		КонецЦикла;
	Иначе
		График.Очистить();
...
Процедура ЛигаПриИзменении(Элемент=Неопределено)
	КомандыЛиги.ЗагрузитьЗначения(КомандыЗагрузить());
	ГрафикОбновить();
КонецПроцедуры
...
Функция КомандыЗагрузить()
	Возврат ?(ЗначениеЗаполнено(Отчет.Лига), СерверныйФункции.КомандыЛиги(Отчет.Лига, НЕ Отчет.Лига.ОлимпийскаяСистема), Новый Массив);
КонецФункции
