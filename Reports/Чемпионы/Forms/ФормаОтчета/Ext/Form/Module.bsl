...
Процедура ГрафикОбновитьНаСервере()
	...
	ИначеЕсли ОтображатьТаблицуДанных Тогда
		Кондуит		= Новый Массив;
		Пока Выборка.Следующий() И Чемпионаты.Количество() <= 10 Цикл
			Если Чемпионаты.Найти(Выборка.Чемпионат) = Неопределено Тогда
				Чемпионаты.Добавить(Выборка.Чемпионат);
...
Процедура СформироватьНаСервере()
	Результат.Очистить();
	Результат.Вывести(ЗаголовокПолучить());
	
	Макет  = ПолучитьОбщийМакет("ТурнирнаяТаблица");
	Результат.Вывести(Макет.ПолучитьОбласть("Шапка2"));
	ОбластьМакета	= Макет.ПолучитьОбласть("Детали2");
	Для Каждого Выборка Из Отчет.Данные Цикл
...
Функция ЗаголовокПолучить()
	Возврат СерверныйФункции.ЗаголовокПолучить(Новый Структура("Лига,Чемпионат", Отчет.Лига, Отчет.Лига));
КонецФункции
...
Процедура ГрафикОбновить()
	...
	Иначе
		...
		Для Каждого Детали Из Отчет.Данные Цикл
			...
			Точка.Текст	= Формат(Детали.Год, "ЧГ=0");
