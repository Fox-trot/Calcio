		...
		Для Каждого Противник Из Команды Цикл
			...
			Иначе
				...
				Для Каждого Матч Из Матчи Цикл
					Если ЗначениеЗаполнено(Матч.Регистратор) Или НЕ ПустаяСтрока(Счет) Тогда
						Счет = ?(ПустаяСтрока(Счет), "", Счет + Символы.ПС) + СтрШаблон("%1-%2", Матч.КоличествоЗабито, Матч.КоличествоПропущено);
						Список.Добавить(Матч.Регистратор);
					Иначе
						ОбластьМакета = Макет.ПолучитьОбласть("Пятерка|Команда");
						Счет = ?(ПустаяСтрока(Счет), "", Счет + Символы.ПС) + Формат(Матч.Дата, "ДФ=dd.MM");
						Список.Добавить(Матч.Матч);
					КонецЕсли;
					Данные.Удалить(Матч);
				КонецЦикла;
				ОбластьМакета.Параметры.Счет = Счет;
				ОбластьМакета.Параметры.Регистратор	= ?(Список.Количество() = 1, Список.Получить(0).Значение, Список);
				ТабДокумент.Присоединить(ОбластьМакета);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
