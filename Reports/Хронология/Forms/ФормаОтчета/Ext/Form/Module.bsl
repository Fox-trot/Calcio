...
Процедура ГрафикОбновитьНаСервере()
	...
	Если ТипГрафика = 8 Тогда
			...
			|
			|ИМЕЮЩИЕ
			|	СУММА(ТурнирнаяТаблица.КоличествоГолов) > 0
			|
			|УПОРЯДОЧИТЬ ПО
...
Процедура ГрафикОбновить()
	...
	Если ТипГрафика = 8 Тогда
		Для Каждого Детали Из Отчет.Данные Цикл
			График.УстановитьЗначение(УстановитьТочку(Детали.Команда), УстановитьСерию(Детали.Команда), Детали.КоличествоЗабито);
		КонецЦикла;
...
Процедура ВидимостьДоступностьУстановить()
	Элементы.Команда.Доступность				= (ТипГрафика <> 8);
КонецПроцедуры
