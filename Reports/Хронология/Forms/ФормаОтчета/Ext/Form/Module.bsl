...
Процедура ГрафикОбновитьНаСервере()
	...
	ИначеЕсли ТипГрафика = 3 Тогда
		...
		График.ОбластьПостроения.ШкалаЗначений.ТекстЗаголовка = "Количество голов";
	Иначе
		График.ТипДиаграммы	= ?(ТипГрафика >= 4, ТипДиаграммы.Гистограмма, ТипДиаграммы.График);
	...
	Если ЗначениеЗаполнено(Отчет.Команда) Тогда
		...
		ИначеЕсли ТипГрафика = 3 Тогда
			...
			Для Каждого Детали Из Отчет.Данные Цикл
				...
				График.УстановитьЗначение(УстановитьТочку(Детали.Подсказка, Детали.Тур), Серия, Значение);
			КонецЦикла;
...
Функция УстановитьТочку(Значение, Расшифровка=Неопределено)
	Если ТипЗнч(Значение) = Тип("Дата") Тогда
		...
		Точка.Текст		= ?(ТипГрафика >= 10, Формат(Значение, "ДФ=MMMM"),ФорматД(Значение));
	Иначе
		Точка			= График.УстановитьТочку(Значение);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТипГрафика.СписокВыбора.Добавить(4, "Беспроигрышная серия");
	Элементы.ТипГрафика.СписокВыбора.Добавить(5, "Победная серия");
	Элементы.ТипГрафика.СписокВыбора.Добавить(6, "'Сухая' серия");
	Элементы.ТипГрафика.СписокВыбора.Добавить(7, "'Сухие' матчи");
	Элементы.ТипГрафика.СписокВыбора.Добавить(8, "'Сухие' победы");
	Элементы.ТипГрафика.СписокВыбора.Добавить(9, "Зрелищность");
	Элементы.ТипГрафика.СписокВыбора.Добавить(10, "Дома/Выезд");
	Элементы.ТипГрафика.СписокВыбора.Добавить(11, "Количество очков по месяцам");
	Элементы.ТипГрафика.СписокВыбора.Добавить(12, "Количество голов по месяцам",, БиблиотекаКартинок.Goal);
	Элементы.ТипГрафика.СписокВыбора.Добавить(13, "Количество 'верховых' побед");
	Элементы.ТипГрафика.СписокВыбора.Добавить(14, "Количество 'верховых' матчей");
	Элементы.ТипГрафика.СписокВыбора.Добавить(15, "Количество 'низовых' побед");
	Элементы.ТипГрафика.СписокВыбора.Добавить(16, "Количество 'низовых' матчей");
...
Процедура ГрафикВыбор(Элемент, ЗначениеДиаграммы, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ЗначениеДиаграммы) = Тип("ЗначениеДиаграммы") Тогда
		Если ЗначениеЗаполнено(ЗначениеДиаграммы.Расшифровка) Тогда
			//
		ИначеЕсли СтрНайти("3,4,5,6,11,12,13,14,15,16", ФорматЧГ(ТипГрафика)) > 0 Тогда
