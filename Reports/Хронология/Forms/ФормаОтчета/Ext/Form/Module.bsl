...
Процедура ДанныеПолучить()
	...
	Если ЗначениеЗаполнено(Отчет.Команда) Тогда
		Если ТипГрафика = 8 Тогда
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |
			               |ИМЕЮЩИЕ
			               |	СУММА(ТурнирнаяТаблица.КоличествоГолов + Противник.КоличествоГолов) >= 3
			               |
			               |УПОРЯДОЧИТЬ ПО
	...
	ИначеЕсли ТипГрафика = 8 Тогда
			...
			|	КОНЕЦ КАК Подсказка,
			|	МАКСИМУМ(Рекорд.Матч) КАК Матч,
			|	СУММА(ТурнирнаяТаблица.КоличествоГолов) КАК КоличествоЗабито
			|ИЗ
			|	РегистрНакопления.ТурнирнаяТаблица КАК ТурнирнаяТаблица
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
			|			Рекорд.Тур КАК Тур,
			|			МАКСИМУМ(Рекорды.Матч) КАК Матч
			|		ИЗ
			|			(ВЫБРАТЬ
			|				Рекорды.Тур КАК Тур,
			|				МАКСИМУМ(Рекорды.КоличествоЗабито) КАК КоличествоЗабито
			|			ИЗ
			|				Рекорды КАК Рекорды
			|			
			|			СГРУППИРОВАТЬ ПО
			|				Рекорды.Тур) КАК Рекорд
			|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Рекорды КАК Рекорды
			|				ПО Рекорд.Тур = Рекорды.Тур
			|					И Рекорд.КоличествоЗабито = Рекорды.КоличествоЗабито
			|		
			|		СГРУППИРОВАТЬ ПО
			|			Рекорд.Тур) КАК Рекорд
			|		ПО ТурнирнаяТаблица.Тур = Рекорд.Тур
			|ГДЕ
			|	ТурнирнаяТаблица.Тур.Владелец = &Чемпионат
			|
			|СГРУППИРОВАТЬ ПО
...
Процедура ГрафикОбновить()
	...
	Если ЗначениеЗаполнено(Отчет.Команда) Тогда
		Если ТипГрафика = 8 Тогда
			...
			Для Каждого Детали Из Отчет.Данные Цикл
				...
				График.УстановитьЗначение(Точка, 0, Детали.КоличествоЗабито);
				График.УстановитьЗначение(Точка, 1, Детали.КоличествоПропущено);
			КонецЦикла;
	...
	ИначеЕсли ТипГрафика = 8 Тогда
		СерииЗаполнить();
		График.Серии.Удалить(1);
		Для Каждого Детали Из Отчет.Данные Цикл
			График.УстановитьЗначение(УстановитьТочку(Детали.Подсказка), 0, Детали.КоличествоЗабито, Детали.Матч, Детали.Матч);
		КонецЦикла;
