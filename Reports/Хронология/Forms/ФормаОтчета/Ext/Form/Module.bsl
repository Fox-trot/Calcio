	...
	Если ЗначениеЗаполнено(Отчет.Команда) Тогда
		Если ТипГрафика = -1 Тогда
			Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			               ...
			               |УПОРЯДОЧИТЬ ПО
			               |	Период";
			Туры	= Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку(0);
			Клубы	= Новый Массив;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
	ИначеЕсли ТипГрафика = -1 Тогда
		...
		Туры	= Выборка.Получить(0).Выгрузить().ВыгрузитьКолонку(0);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
	Если ТипГрафика = -1 Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |			ИНАЧЕ 0
		               |		КОНЕЦ)";
		Для Каждого Итератор Из Туры Цикл
...
Процедура ГрафикОбновитьНаСервере()
	...
	Если ЗначениеЗаполнено(Отчет.Команда) Тогда
		Если ТипГрафика = -1 Тогда
			Серия	= График.УстановитьСерию("Место");
			Для Каждого Детали Из Отчет.Данные Цикл
				График.УстановитьЗначение(УстановитьТочку(Детали.Подсказка, Детали.Тур), Серия, Детали.КоличествоПропущено, Детали.Матч, Детали.Матч);
			КонецЦикла;
