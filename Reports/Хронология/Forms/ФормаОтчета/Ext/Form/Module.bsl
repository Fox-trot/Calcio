	...
	Если ЗначениеЗаполнено(Отчет.Команда) Тогда
		Если ТипГрафика = 6 Тогда
			Для Каждого Детали Из Отчет.Данные Цикл
				...
				График.УстановитьЗначение(Точка, График.УстановитьСерию("Забито"), Детали.КоличествоЗабито, Детали.Матч, Детали.Подсказка);
				График.УстановитьЗначение(Точка, График.УстановитьСерию("Пропущено"), Детали.КоличествоПропущено, Детали.Матч, Детали.Подсказка);
			КонецЦикла;
		...
		ИначеЕсли ТипГрафика = 2 Тогда
			...
			Для Каждого Детали Из Отчет.Данные Цикл
				...
				Итого	= Итого + 1;
				График.УстановитьЗначение(Точка, График.УстановитьСерию("Забито"), Детали.КоличествоЗабито);
				График.УстановитьЗначение(Точка, График.УстановитьСерию("Пропущено"), Детали.КоличествоПропущено);
				График.УстановитьЗначение(Точка, График.УстановитьСерию("Средняя результативность"), Окр(Забито / Итого, 1), Детали.Матч, Детали.Подсказка);
			КонецЦикла;
	...
	ИначеЕсли ТипГрафика = 2 Тогда
		...
		Для Каждого Детали Из Отчет.Данные Цикл
			Если ЗначениеЗаполнено(Детали.Период) Тогда
				...
				Забито	= Забито + Детали.КоличествоЗабито;
				Если Итого > 1 Тогда
					График.УстановитьЗначение(График.УстановитьТочку(ФорматД(Детали.Период)), Серия, ?(Итератор=0, Детали.КоличествоЗабито, Окр(Забито / Итого, 2)), Детали.Матч, Детали.Подсказка);
				КонецЕсли;
			Иначе
...
Функция МатчиПолучить(Отбор)
	...
	Если ТипЗнч(Отбор.Период) = Тип("Дата") Тогда
		...
		Пока Ответ.Количество() <=10 И Выборка.Следующий() Цикл
