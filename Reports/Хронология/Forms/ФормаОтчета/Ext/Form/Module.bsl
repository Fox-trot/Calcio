...
Процедура ГрафикОбновить()
	...
	График.Очистить();
	График.ТипДиаграммы	= ?(ТипГрафика = 3 Или ТипГрафика = 4, ТипДиаграммы.Гистограмма, ТипДиаграммы.График);
	Если ЗначениеЗаполнено(Отчет.Команда) Тогда
		Если ТипГрафика = 3 Или ТипГрафика = 4 Тогда
	...
	ИначеЕсли ТипГрафика = 3 Или ТипГрафика = 4 Тогда
		Для Каждого Детали Из Отчет.Данные Цикл
			График.УстановитьЗначение(График.УстановитьТочку(Детали.КоличествоЗабито), График.УстановитьСерию(Детали.Команда), Детали.КоличествоЗабито);
		КонецЦикла;
	...
	График.ОбластьЛегенды.Расположение	= ?(График.Серии.Количество() = 1, РасположениеЛегендыДиаграммы.Нет, РасположениеЛегендыДиаграммы.Авто);
КонецПроцедуры
